...
- ground station 通过目标 drone 名下的专属控制服务开启/停止图传；drone 在自己的命名空间下发布
  签名 NDN video packets，ground station 根据可预测名字预取 packets。
...
当前实现应该被理解为一个面向服务的 UAV prototype，其中已经接入了几类接近真实部署的能力：

- **Camera。** DroneAPP 支持文件、USB/V4L2 和 `auto` video source。`auto` 模式会用保守方式探测
  本机摄像头能力；如果没有可用摄像头，会为了本地 troubleshooting 回退到内置 sample video。
  Drone telemetry/readiness 会报告 camera availability、source 和 reason，因此 GS 能显示当前
  选中无人机是否真的有可用摄像头。
- **Flight controller。** DroneAPP 支持 mock、UDP MAVLink、serial MAVLink，以及作为 UDP 路径
  alias 的 `mavlink-router`。GPS 数据刻意从 PX4/ArduPilot 通过 MAVLink 发布的 telemetry 中读取；
  companion computer 不直接扫描独立 USB 或串口 GPS unit。后端提供相关字段时，GS 会显示
  heartbeat、armed、flight-controller readiness、GPS/EKF、电池、landed state 和 command ACK。
- **Mission。** Mission upload/start/stop 已经用 typed `MissionState`、`MissionPlan` 和
  `MissionProgressState` 表示。巡逻 waypoints 会按选中的 drone 做确定性聚类，每个 part 可以带上
  从 telemetry 采样到的 return-to-start 目标；progress active 时，GS 会阻止明显重复的 mission 操作。
- **Video。** Live video 使用 provider-specific video-control NDNSF service，高频视频流则是
  drone 自己命名空间下的签名 NDN Data packets。接收端会跟踪 adaptive RTT/timeout/backlog pressure、
  packet sequence、stream id 和 stream session metadata，因此旧 live session 的迟到 packets
  可以被丢弃，而不会被当成当前视频显示。
- **Repo。** Drone-side recording 到内嵌 `NDNSF-DistributedRepo` 是可选能力，由 drone 配置决定，
  不由 GS 的 Start Video 按钮决定。当前已经实现的具体 repo-backed UAV data product 是加密
  camera recording：GS 通过 recording manifest service 发现它，然后按名字 fetch/decrypt chunks
  并用于播放。Mission image、telemetry log、detection event 和 report 是同一 data-product model
  的后续扩展。
...
runtime 启动时，NDNSF 会一次性解析 certificate role：RSA 仍作为 NAC-ABE 和 permission
unwrap 的 encryption certificate；如果安装了 EC/ECDSA certificate，则优先作为 signing
certificate。Drone video 和 recording Data publisher 会缓存这个 signing choice，因此逐帧发布时
不会反复扫描 keychain。
...
大图、录像片段、报告以及其它大对象不应该作为大型 inline invocation payload 传输。
它们应该使用 NDNSF Core large-data abstraction：发布为 hybrid AES-GCM 加密、签名并
分段的 NDN Data，然后 request 只携带 reference；或者通过 `NDNSF-DistributedRepo` 存到
发布者自己的命名空间下，再用 manifest/name 引用。这样可以保持 service request 很小，
避免把 NDNSF invocation payload 变成临时文件传输通道。
...
和当前码率建议，而不用解析 packet log。主要压力来源会具体区分 timeout、loss、duplicate、
backlog、future-probe 和 decode-gap pressure，而不是全部隐藏在一个笼统的 congestion 标签下面。这个码率建议只走显式控制：GS 不会在后台静默改变 drone encoder；操作者可以点击
`Apply Bitrate`，这会先停止当前直播，等待 drone 的 Stop response，再用建议码率启动新的
stream，让 packet sequence、stream id 和 decoder state 保持清楚。GS 会记录完整
Stop-then-Start 闭环里的 requested bitrate 和 drone 返回的 accepted bitrate，因此 smoke test
可以确认重启后的 stream 真的使用了 drone 确认的码率。默认码率策略是 `manual`。
...
后续可以增加每个 stream 独立的小 SVS group 来发布 video-packet announcement，但它应该和主 UAV
control group 分开，避免高频视频信令影响 command、telemetry 和 mission traffic。同一个控制服务
以后也可以携带 H265、GOP 等参数。

Video packet data 发布在 drone 自己的 namespace 下，由 drone 签名，并可选通过
...
- 由 video-packet/prefetch pipeline 驱动的视频预览；
...
1. **收束状态模型。** 现在 telemetry、readiness、mission、video、command 和 safety state
   ...
   marker 渲染仍然留在窗口层。Drone telemetry 现在也会携带 camera availability/source/reason
   和 flight-controller backend/availability/readiness/state，因此 GS 能显示 drone 侧摄像头和飞控
   是否真的可用，而不只是显示收到了 telemetry response。telemetry live smoke 现在会为每个
   arm/takeoff/land telemetry sample 输出这些共享 action/summary/row/subsystem model，因此状态模型
   回归不只依赖 synthetic GUI injection，也会经过真实 NDNSF telemetry request 路径。后续新增
   mission/video/safety UI 路径时继续坚持这一点：只要有 typed state model，GUI 就不应该再从临时
   status string 推断状态。
...
4. **自适应视频服务质量。** 继续把 video 当作 NDNSF service workload：requested bitrate、
   ...
   `primary_pressure` 和 `policy_reason`。`primary_pressure` 会区分 timeout、loss、duplicate、
   backlog、probe 和 decode-gap pressure，方便操作者理解 receiver 为什么收缩或恢复。核心窗口、
   ...
   建议转换成显式 Stop-then-Start stream restart，并记录 drone 返回的 accepted bitrate
   作为闭环验证。MiniNDN smoke 也可以注入受控的
...
6. **Repo-backed UAV data products。** 当前已经实现的 repo-backed product 是通过
   `NDNSF-DistributedRepo` 保存的加密 camera recording，使用 publisher-owned name 和
   manifest-based discovery。Mission image、telemetry log、object-detection event 和 report
   应该复用同一套 data-product pattern。Camera recording manifest 现在会解析成
...
如果只想快速检查 launcher/config，不启动 MiniNDN、X11 或 APP 进程，可以运行：

```bash
python3 Experiments/NDNSF_UAV_GUI_Minindn.py --quick-smoke
```

成功时会输出 `NDNSF_UAV_GUI_MININDN_QUICK_SMOKE_OK`。
...
如果需要让 smoke test 覆盖显式 `Apply Bitrate` Stop-then-Start 路径，并验证重启后 drone
确认的 accepted bitrate，在同一命令后加 `--auto-apply-bitrate-test`。
...
启动后注入 timeout、backlog 和 probe 三类受控样本，并要求 GS 记录
`auto-video-pressure-timeout`、`auto-video-pressure-backlog` 和
...
2. 继续把 waypoint-sector patrol demo 扩展成完整 mission editor：已有 deterministic assignment、
   mission preview、return-to-start 和 progress model，后续可以补 polygon 绘制、按电量重新分配
   和 coverage report。
...
4. 针对有损无线链路调优 H264/H265 GOP、bitrate、chunk size、FEC 和 keyframe recovery，
   同时保留即时播放语义。
5. 当前 repo-backed product 是加密 camera recording；任务图像、telemetry log、detection event
