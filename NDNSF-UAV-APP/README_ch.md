...
backlog 和 future-probe pressure，而不是全部隐藏在一个笼统的 congestion 标签下面。这个码率建议只走显式控制：GS 不会在后台静默改变 drone encoder；操作者可以点击
...
4. **自适应视频服务质量。** 继续把 video 当作 NDNSF service workload：requested bitrate、
   ...
   backlog 和 probe pressure，方便操作者理解 receiver 为什么收缩或恢复。核心窗口、
