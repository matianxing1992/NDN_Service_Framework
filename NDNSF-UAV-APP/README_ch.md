...
packet log。这个码率建议目前只是 advisory：GS 不会在直播中静默重启 stream 或直接改变 drone
encoder。
当前 demo
...
4. **自适应视频服务质量。** 继续把 video 当作 NDNSF service workload：requested bitrate、
   ...
   `VideoAdaptiveState`，包括 advisory 的 decrease/hold/increase 码率建议；下一步可以把这些
   advisory 决策变成 operator-controlled 或 service-controlled 的显式码率调整。
