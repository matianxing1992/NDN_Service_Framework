#!/usr/bin/env bash
# Candidate-bound Spec170 D1 workload.
# This script intentionally consumes the same plan, manifest, Python driver,
# policy, trust schema, and artifacts as the passing current D0 bundle. The
# only D1 change is one CUDA Provider owning all four roles.
...
mkdir -p "$ROOT/run" "$LOG" "$STATUS" "$KEYCHAIN_ROOT" "$ROOT/cache-provider"
...
for path in "${required[@]}"; do
  if [[ ! -f "$BUNDLE/$path" ]]; then
    echo "SPEC170_D1_CURRENT_BUNDLE_FAIL missing=$BUNDLE/$path"
    exit 20
  fi
done
...
for role in nfd nfdctl controller provider user; do
...
EXPECTED_RUNNER_KIND="${SPEC170_EXPECTED_RUNNER_KIND:-onnxruntime-cuda}"

cat > "$ROOT/bootstrap.tokens" <<'EOF'
# identity token role
/NDNSF-DI/Tracer/user user0001 user
/NDNSF-DI/Tracer/provider/single single0001 provider
EOF
...
cleanup() {
  ...
  sleep 1
  # Preserve readiness failures without allowing an unresponsive Provider to
  # hold the Slurm job until its full allocation timeout.
  for pid in "${PIDS[@]}"; do kill -KILL "$pid" 2>/dev/null || true; done
  for pid in "${PIDS[@]}"; do wait "$pid" 2>/dev/null || true; done
  local evidence="/evidence/spec170/d1-current-${SLURM_JOB_ID:-incontainer}"
  mkdir -p "$evidence"
  ...
  printf 'SPEC170_D1_CURRENT_TERMINAL exit=%s bundle=%s\n' "$rc" "$BUNDLE"
  exit "$rc"
}
...
if [[ "$nfd_ready" -ne 1 ]]; then
  echo SPEC170_D1_CURRENT_NFD_READY_FAIL
  exit 10
fi
...
printf 'SPEC170_D1_CONTROLLER_PID=%s\n' "$controller_pid" >"$STATUS/controller-startup.txt"
controller_started=0
for _ in $(seq 1 1200); do
  # Do not open the Controller PIB from ndnsec while App_ServiceController is
  # still constructing its KeyChain. That startup race can report
  # "PIB database cannot be initialized: database is locked" and abort the
  # otherwise valid Controller.
  if grep -q 'ServiceController started...' "$LOG/controller.log" 2>/dev/null; then
    controller_started=1
    break
  fi
  if ! kill -0 "$controller_pid" 2>/dev/null; then
    echo SPEC170_D1_CURRENT_CONTROLLER_EXITED >"$STATUS/controller-startup.txt"
...
if [[ "$controller_started" -ne 1 ]]; then
  echo SPEC170_D1_CURRENT_CONTROLLER_START_FAIL
  exit 10
fi
controller_ready=0
for _ in $(seq 1 120); do
...
if [[ "$controller_ready" -ne 1 ]]; then
  echo SPEC170_D1_CURRENT_CONTROLLER_CERT_FAIL
  exit 10
fi
...
(
  ...
  exec env "$provider_pib" "$provider_tpm" \
    ...
    --provider /NDNSF-DI/Tracer/provider/single --roles all --workers 1 \
...
if [[ "$provider_ready" -ne 1 ]]; then
  echo SPEC170_D1_CURRENT_PROVIDER_READY_FAIL
  echo '=== provider=single ==='
...
env "$user_pib" "$user_tpm" python3 "$BUNDLE/user_driver.py" \
  ...
  --permission-wait-ms 5000 --ack-timeout-ms 3000 --timeout-ms 30000 \
  --requests 1 \
  --role-provider-preference \
  '/Backbone=>/NDNSF-DI/Tracer/provider/single;/Head/Shard/0=>/NDNSF-DI/Tracer/provider/single;/Head/Shard/1=>/NDNSF-DI/Tracer/provider/single;/Merge=>/NDNSF-DI/Tracer/provider/single;' \
  >"$LOG/user.log" 2>&1
user_rc=$?
set -u
...
grep -q 'event=SELECTION_RECEIVED' "$LOG/provider.log" || selection_ok=0

dependency_ok=1
for scope in backbone-to-head0 backbone-to-head1 head0-to-merge head1-to-merge; do
  grep -q "NDNSF_DI_DEPENDENCY_OUTPUT_TIMING.*scope=$scope " \
    "$LOG/provider.log" || dependency_ok=0
  grep -q "NDNSF_DI_DEPENDENCY_INPUT_TIMING.*scope=$scope " \
    "$LOG/provider.log" || dependency_ok=0
done
...
if [[ "$user_rc" -eq 0 ]] \
   ...
   && grep -q "runnerKind=${EXPECTED_RUNNER_KIND}\\|runnerKind\":\"${EXPECTED_RUNNER_KIND}\"" "$LOG/provider.log" \
   && grep -q 'cpuFallbackUsed=false\|cpuFallbackUsed":"false"' "$LOG/provider.log" \
   && grep -q 'NDNSF_DI_FINAL_RESPONSE\|NDNSF_DI_NATIVE_FINAL_RESPONSE_DECISION' "$LOG/provider.log"; then
  response_ok=1
fi
...
if [[ "$response_ok" -ne 1 ]]; then
  echo "SPEC170_D1_CURRENT_NETWORK_FAIL user_rc=$user_rc"
  exit 12
fi
echo "SPEC170_D1_CURRENT_NETWORK_PASS job=${SLURM_JOB_ID:-incontainer} request_ack_selection_response=PASS"
exit 0
