...
class GithubSealedWorkflowTests(unittest.TestCase):
    ...
    def test_qwen_slurm_uses_canonical_least_privilege_runner(self) -> None:
        ...
        self.assertIn('RUN_ID=@@RUN_ID@@', template)
        self.assertIn('scratch="/tmp/ndnsf-di-${SLURM_JOB_ID}-${RUN_ID}"', template)
        self.assertNotIn('scratch="/tmp/$USER/ndnsf-di/$SLURM_JOB_ID"', template)
