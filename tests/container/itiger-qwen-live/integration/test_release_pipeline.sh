...
foundation="$repo/packaging/ndnsf-di-container/oci/Dockerfile.foundation"
...
grep -q 'SOURCE_SEAL_MANIFEST_TAMPERED' "$foundation"
grep -q 'FROM scratch AS foundation' "$foundation"
# shellcheck disable=SC2016
grep -Fq 'FROM ${FOUNDATION_IMAGE} AS local-foundation' "$dockerfile"
...
grep -q 'foundation_image:' "$workflow"
