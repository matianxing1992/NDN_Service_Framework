...
def check_dependencies(nac_prefix='/usr/local', onnx_prefix='/usr/local'):
    missing, optional = [], []
    for command in ('g++', 'ar', 'readelf', 'protoc'):
    ...
    required = ('libndn-cxx >= 0.8.0', 'libndn-svs >= 0.1.0',
                'libnac-abe', 'ndnsd', 'openssl', 'protobuf', 'gtkmm-3.0',
                'onnxruntime >= 1.26.0')
    if not pkg:
        ...
        if subprocess.run([pkg, '--exists', *gst.split()], env=env).returncode:
...
if __name__ == '__main__':
    ...
    errors, warnings = check_dependencies(args.nac_abe_prefix, args.onnx_prefix)
