...
already supply User permission decryption keys or challenge processing.
...
credentials. It does not imply one credential per DNMP service: one role can
permit several commands. Nor does it establish a universal advantage over
Trust Schemas, which can express other credential arrangements. A single DKEY
does not have fixed size or update cost; policy complexity, distribution, and
rekeying still matter. The benefit concerns independently managed credentials,
not a demonstrated reduction in total key bytes or runtime cost.
...
This is an NDNSF application-level value, not the NDN Interest Nonce.
...
In the revised dissemination design, the Controller publishes updated public
parameters as signed named Data and uses Sync to announce their availability.
