...
determine access to ProviderChallenge. NDNSF checks the returned challenge
...
This aggregation complements Reason 2: it organizes multiple service permissions
separately from signing credentials within the reused authorization infrastructure.
It is not an exclusive capability of ABE. DNMP roles and signing credentials can
also cover several permissions~\cite{dnmp}. A single DKEY does not imply fixed
size or cost: policy complexity, key distribution, updates, and revocation still
matter. The benefit is a convenient organization of service permissions, not
proof of lower total cost than Trust-Schema-based authorization.
...
ACK content. The User supplies the corresponding challenge response in Selection;
the Provider checks that response before starting the selected task.
