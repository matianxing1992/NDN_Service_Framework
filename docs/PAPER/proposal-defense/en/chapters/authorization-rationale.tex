...
\paragraph{Reason 2: reusing service-permission management.}
Different entities may hold overlapping service entitlements. For example,
one User may invoke observation and detection services, while another may
invoke detection and mapping services. NDNSF can encode these service sets
in separately issued policy keys without changing the Users' identity-signing
credentials. Service-scoped content remains separately encrypted; aggregating
permissions in a key does not merge all services into one encryption scope.
...
The schema by itself does not restrict who can read copied Content~\cite{trustschema}.
It can express fine-grained signing rules and be combined with encryption.
NAC also protects distributed content through encryption and key
distribution~\cite{nac}. The rationale for ABE here is permission aggregation
and reuse of the associated management mechanism, not exclusive support for
cached ciphertext or credential reuse.
...
The comparison therefore uses the DNMP-inspired alternative in
Section~\ref{sec:dnmp-alternative}, with separate content encryption.
ABE separates service-permission policies from
