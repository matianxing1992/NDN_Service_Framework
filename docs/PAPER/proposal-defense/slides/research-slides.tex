...
\begin{frame}{RQ1: Why Protect Discovery Content?}
\begin{itemize}
...
\end{tabularx}
\vspace{3mm}\bluebox{Trust Schema [16] alone does not encrypt Content. Trust Schema plus encryption is a valid alternative, not an impossible design.}
\end{frame}

\begin{frame}{Why Reuse ABE Capability for Authorization?}
\begin{itemize}
\item Enhanced NAC-ABE [19] supports key-policy ABE (KP-ABE): ciphertext attributes meet a policy in an individually issued decryption key (DKEY).
\item Service permissions can be aggregated while identity-signing credentials remain unchanged.
\item Challenges reuse those service permissions instead of introducing additional service-role signing credentials.
\end{itemize}
\vspace{3mm}\bluebox{This saves a second entitlement representation in this design---not all key work. User DKEYs, challenge processing, policy growth, status and rekeying still cost resources.}
\end{frame}
...
\begin{frame}{Selected Exchanges and Permission Withdrawal}
\begin{itemize}
\item Symmetric request-scoped keys protect input and result Content. Selection supplies necessary keys only to the selected Provider through recipient encryption.
\item The requesting User and selected Provider share the result key; other Users are excluded under key-secrecy assumptions.
\item Controller-signed status affects authorization after retrieval and validation. ABE rotation limits subsequent encrypted exchanges.
\end{itemize}
\vspace{3mm}\bluebox{Current rotation is Controller-wide. Disconnected revocation is not immediate; old plaintext and keys cannot be recalled. Runtime tests do not establish lower cost than certificate revocation. [28]}
\end{frame}
