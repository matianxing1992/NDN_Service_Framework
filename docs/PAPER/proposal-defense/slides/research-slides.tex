...
\begin{frame}{Compare Authorization and Encryption Separately}
\small
\begin{tabularx}{\linewidth}{p{.27\linewidth}XX}
\toprule Design & Execution authorization & Discovery confidentiality\\\midrule
DNMP-inspired roles [12] & Validator checks role/command signing rules; add invocation-state checks & Add content encryption and key distribution\\\midrule
...
\end{tabularx}
\vspace{3mm}\bluebox{Trust Schema [16] defines permitted Data-name/signing-key relationships; validators apply its rules. It does not encrypt Content. Encryption and request-state checks are additions to the DNMP-inspired comparison, not capabilities attributed to the original DNMP implementation.}
\end{frame}
