...
\end{itemize}
\vspace{3mm}\bluebox{UAV and distributed inference are validation workloads for the same questions; implementing two applications alone does not establish two research contributions.}
\end{frame}
...
\begin{itemize}
\item Nodes may retrieve the same Request ciphertext; only holders of matching decryption keys can read its descriptor.
...
\end{itemize}
\vspace{3mm}\bluebox{Dissemination and read access are separate. Combining permissions in one key does not require encrypting all services' content under one shared condition.}
\end{frame}
...
\item The Provider encrypts its challenge in ACK content. The User answers in Selection; the Provider checks that response before execution.
\end{enumerate}
