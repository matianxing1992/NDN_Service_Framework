...
\item Nearby unmanned aerial vehicles (UAVs) offer suitable views.
...
\item \textbf{Check task inputs:} a valid image signature does not show that its producer was assigned to this task's dependency.
\end{enumerate}
\vspace{4mm}\bluebox{The research connects participation decisions to authorized execution and task-specific input checks, then measures the cost.}
\end{frame}
...
\end{itemize}
\vspace{3mm}\bluebox{UAV sensing tests dependencies between observations; distributed inference tests ordered computation and state. Both evaluate the same framework.}
\end{frame}
...
\end{tabularx}
\vspace{2mm}\bluebox{Proposed contribution: each selected Provider checks whether input Data matches the task's runtime role assignments and declared dependencies.}
\end{frame}
...
\begin{enumerate}
\item Each sensing UAV signs ViewImage Data containing encrypted image content and publishes it in its own authorized namespace.
\item The compute Provider retrieves images over assigned edges and publishes Annotation Data.
\item The compute Provider includes original image names and permitted image keys in its encrypted result for the requester.
