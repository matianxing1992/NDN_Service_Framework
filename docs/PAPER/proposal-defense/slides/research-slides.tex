...
\begin{enumerate}
\item The User puts a fresh random RequestNonce in ABE-protected Request content. This is not an Interest Nonce.
\item The Provider returns RequestNonce in ACK Data; the User checks it against the active request.
\item The Provider includes ProviderChallenge in ABE-protected ACK content and checks its challenge response in Selection before execution.
\end{enumerate}
\vspace{3mm}\bluebox{Both sides also check signatures under trust rules, service permissions and request state. Matching challenge responses show access to protected values, not exclusive key possession.}
\end{frame}
