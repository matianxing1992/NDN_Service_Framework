...
`UAV-Experimental` is a retained application integration branch explicitly
authorized by Tianxing. New `NDNSF-UAV-APP` features, including Spec 176, MUST
be specified, implemented, tested, and evidenced on `UAV-Experimental`. If a
short-lived UAV Spec branch is useful, it merges back into `UAV-Experimental`,
not directly into `Experimental` or `main`. At deliberate synchronization
points, `UAV-Experimental` may incorporate validated NDNSF changes from
`Experimental`; incomplete UAV application work does not flow in the opposite
direction. Promotion from `UAV-Experimental` requires an explicit decision by
Tianxing after application validation.
...
5. Do not infer that a branch was merged from its name; verify ancestry and the
   resulting tree. Preserve `main`, `Experimental`, `UAV-Experimental`, and
...
`UAV-Experimental` 是 Tianxing 明确授权保留的 UAV 应用集成分支。新的
`NDNSF-UAV-APP` 功能（包括 Spec 176）必须在 `UAV-Experimental` 上完成规格、
实现、测试和证据整理。若为了隔离而创建临时 UAV Spec 分支，该分支应合并回
`UAV-Experimental`，不能直接合并到 `Experimental` 或 `main`。在明确的同步
节点，`UAV-Experimental` 可以吸收 `Experimental` 中已经验证的 NDNSF 改动；
未完成的 UAV 应用工作不能反向进入 NDNSF 集成主线。只有在应用验证完成且
Tianxing 明确决定后，才可以把 `UAV-Experimental` 的内容晋升到其他集成分支。
...
5. 不能根据分支名称推断是否已经合并，必须检查 ancestry 和合并后的文件树。
   只保留 `main`、`Experimental`、`UAV-Experimental` 以及 Tianxing 明确要求
