...
\vspace{1.5em}
\begin{itemize}
\item A camera command acknowledgement is not capture success: check the image and measured pose.
\item Match each image to the same car and requested view; reject stale or unusable frames.
...
\begin{frame}{Test Camera Benefit and Framework Benefit Separately}
\small
\renewcommand{\arraystretch}{1.35}
\begin{tabular}{>{\RaggedRight\arraybackslash}p{.25\textwidth}>{\RaggedRight\arraybackslash}p{.33\textwidth}>{\RaggedRight\arraybackslash}p{.32\textwidth}}
\textbf{Question}&\textbf{Comparison}&\textbf{Measure}\\\hline
Are extra UAVs useful? & One UAV visits all views vs. multiple UAVs & Useful-view coverage, completion time, total energy\\
Does selection help? & Fixed assignments vs. runtime selection; same fleet & Completion under Provider loss, deadline misses, network bytes\\
\end{tabular}
\vspace{.5em}
\textbf{Route:} camera bench $\rightarrow$ flight simulation $\rightarrow$ one real UAV $\rightarrow$ multiple UAVs.
\claim{Fix the scene, views and deadline. Label visibility independently; include failed runs.}
\evidence{Planned ablation, no measured advantage. Match cameras, models and workload; include ground compute. This is not a comparison with other frameworks.}
\end{frame}
