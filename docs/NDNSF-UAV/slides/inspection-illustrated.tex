...
\column{.48\textwidth}
\begin{block}{SIYI A8 mini}
\begin{itemize}
\item Integrated camera + three-axis gimbal.
\item SDK control over Ethernet UDP or UART; Ethernet video output.
\item Candidate for the first camera-control bench experiment.
\end{itemize}
\end{block}
\column{.48\textwidth}
\begin{block}{Gremsy Pixy family}
\begin{itemize}
\item Gimbals for compatible camera payloads.
\item SDK/MAVLink control; camera support depends on the model.
\item Candidate when a specific imaging payload is required.
\end{itemize}
\end{block}
\end{columns}
\vspace{.6em}
\textbf{Verify before selection:} angle commands, attitude feedback, image access, payload weight, power and mounting.
\claim{Pointing at a car's GPS location still needs aircraft pose and camera calibration; a controllable gimbal supplies only part of the loop.}
\evidence{\href{https://siyi.biz/siyi_file/A8\%20mini/A8\%20mini\%20User\%20Manual\%20v1.6.pdf}{SIYI A8 mini manual}; \href{https://docs.gremsy.com/gremsysdk}{Gremsy SDK}. Candidate devices, not purchased or validated hardware.}
\end{frame}
