...
manifest = repo.put(
    object_name="/NDNSF-DI/ARTIFACT/AI/YOLO/2x2/Stage/0/Shard/0/model",
    payload=model_bytes,
...
The manifest records the object hash, size, replication factor, and selected
repo nodes. Providers still fetch signed/encrypted artifacts through NDNSF
today; the manifest gives the next storage layer enough information to place
and retrieve artifacts from controlled repo replicas instead of storing them
randomly.
