...
generated by a future planner from an ONNX graph. The current YOLO examples
ship ready-to-run policy files and splitter scripts.
...
contains artifacts or repo manifests, the provider can be selected for a role
and then download the role-specific model/runtime artifact.
...
`distributed_inference(...)` refers to model/runtime artifacts stored in
