...
ready-to-run policy files and splitter scripts. The YOLO layout splitter can
export a real Ultralytics YOLO model into a requested stage-by-shard layout
...
The `yolo_2x2` example now acts as the YOLO layout example. Its historical
default is 2x2: two pipeline stages and two sequential shards inside each
stage. The same splitter accepts custom layouts such as `1x3`, `2x3`, `3x2`,
and `3x3`. It uses the real Ultralytics YOLO nano model and exports one ONNX
chunk per generated role.
