...
from ..adapters.onnx.graph import analyze_onnx_graph, estimate_split_candidates
...
class NdnsfSvsEnvConfig:
    ...
    def to_env(self) -> dict[str, str]:
        ...
        protocol = str(self.protocol_version or "v3").strip().lower()
        if protocol in {"2", "v2"}:
            protocol = "v2"
        elif protocol in {"3", "v3"}:
            protocol = "v3"
        else:
            raise ValueError("protocol_version must be v2 or v3")
        values["NDNSF_SVS_PROTOCOL_VERSION"] = protocol
        mapping = {
...
def run_headless_qwen_minindn(args: argparse.Namespace) -> dict[str, Any]:
    ...
    summary = {
        ...
        "runnerClassification": harness_summary.get("runnerClassification", "invalid-evidence"),
        "executionEvidence": harness_summary.get("executionEvidence", []),
...
def repo_root() -> Path:
    configured = os.environ.get("NDNSF_REPO_ROOT", "").strip()
    candidates = ([Path(configured)] if configured else []) + [
        Path.cwd(),
        *Path(__file__).resolve().parents,
    ]
    for candidate in candidates:
        if (candidate / "Experiments/NDNSF_DI_NativeTracer_Minindn.py").is_file():
            return candidate.resolve()
    # Preserve the historical fallback for installed applications that only
    # use process-local GUI roles and do not launch a source-tree experiment.
...
class DirectRoleTab(ttk.Frame):
    ...
    def _common_env_frame(self, parent) -> ttk.LabelFrame:
        ...
        self._field(frame, row, "SVS protocol (v2/v3)", "env_protocol_version", "v3"); row += 1
        self._field(frame, row, "SVS suppression override ms", "env_suppression_ms", ""); row += 1
    ...
    def env_from_fields(self) -> NdnsfSvsEnvConfig:
        return NdnsfSvsEnvConfig(
            ...
            protocol_version=self.value("env_protocol_version"),
    ...
    def apply_env(self, env: NdnsfSvsEnvConfig) -> None:
        ...
        self.fields["env_protocol_version"].set(env.protocol_version)  # type: ignore[union-attr]
        ...
        for result in results:
            ...
            try:
                ...
                compact_runs.append({
                    ...
                    "runnerClassification": data.get("runnerClassification", "invalid-evidence"),
                    "executionEvidence": data.get("executionEvidence", []),
