...
class DISelectionParticipant:
    ...
    def prepare(
        ...
        if context.get("request_id") != assignment.request_id:
            binding_mismatches.append("request_id")
        if int(context.get("attempt", 0)) != assignment.attempt:
            binding_mismatches.append("attempt")
        if offer is None:
            binding_mismatches.append("offer_digest")
        elif context.get("service_name") != offer.service:
            binding_mismatches.append("service_name")
        if context.get("provider_identity") != self.provider:
            binding_mismatches.append("provider_identity")
        if context.get("provider_boot_epoch") != self.boot_epoch:
            binding_mismatches.append("provider_boot_epoch")
        if (context.get("selection_payload_digest")
                != self._digest_bytes(payload)):
            binding_mismatches.append("selection_payload_digest")
        if assignment.provider != self.provider:
            binding_mismatches.append("assignment_provider")
        if assignment.provider_boot_epoch != self.boot_epoch:
            binding_mismatches.append("assignment_provider_boot_epoch")
        if assignment.deadline_ms <= now_ms:
            binding_mismatches.append("assignment_deadline")
        if (int(context.get("expires_at_unix_ms", 0))
                < assignment.deadline_ms):
            binding_mismatches.append("core_expiry")
