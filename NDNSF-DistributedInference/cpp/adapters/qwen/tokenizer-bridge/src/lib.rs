...
fn decode_bytes(tokenizer: &Tokenizer, ids: &[u32], skip: bool, final_output: bool)
    -> Result<Vec<u8>, String>
{
    ...
    // Rust tokenizers materializes incomplete ByteLevel/ByteFallback runs as
    // replacement characters.  They are not stable output: a later token can
    // complete the byte sequence.  Keep the replacement suffix pending and
    // flush it only on final=true.  A literal U+FFFD is therefore delayed,
    // never deleted, which is the safe streaming boundary.
    let mut stable = text;
    while stable.ends_with('\u{fffd}') {
        stable.pop();
    }
    Ok(stable.into_bytes())
}
...
pub unsafe extern "C" fn ndi_token_decode(
    ...
    boundary(|| {
        ...
        let tokenizer = &*handle.cast::<Tokenizer>();
        decode_bytes(tokenizer, borrowed(ids, count, TOKEN_LIMIT)?, flag(skip_special)?, true)
    })
}
...
pub unsafe extern "C" fn ndi_token_decode_stable(
    ...
    boundary(|| {
        ...
        decode_bytes(
            tokenizer,
            borrowed(ids, count, TOKEN_LIMIT)?,
            flag(skip_special)?,
            flag(final_output)?,
        )
    })
