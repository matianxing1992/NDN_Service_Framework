...
`results/yolo_2x2_unified_selection_60s_minimal` 中的代表性运行输出
`YOLO_2X2_NATIVE_PROVIDERS_MININDN_OK`，60 个 measured warm requests 的
p50 为 84.95 ms，p95 为 114.46 ms，mean 为 85.96 ms。丢弃第一个 measured
warm request 后，steady warm p50 为 84.78 ms，p95 为 114.29 ms，mean 为
85.27 ms。NFD 在 warm-side 63 个被脚本计数的请求上观察到 819 个 incoming/outgoing
Data packets，即每个 observed request 13.00 个 Data packets；按 measured window
折算，每次 inference 约 13.65 个 NFD out Data packets，约 427 KB NFD
...
`results/yolo_2x2_control_svs_timing_60s`。它使用同样的 60 秒形状并打开
`--control-timing`，60 个 measured warm requests 的 p50 为 74.73 ms，p95 为
95.64 ms，steady-after-first p50 为 77.10 ms。该运行测得 p50 request latency
为 76.95 ms，provider dataflow 为 20.00 ms，outer-control residual 为
50.13 ms。Provider 侧 request handling 是亚毫秒级：
`requestObservedToAckPublishedMs` p50 为 0.15 ms；response decrypt/callback
也都是亚毫秒级，ONNX execution 不是瓶颈。剩余成本主要在 SVS/NFD delivery
和 segmented activation fetch：request publication 到 provider observation
约 11.55 ms p50，ACK publication 到 user pre-decrypt 约 6.70 ms p50，
selection publication 到 provider observation 约 5.83 ms p50，response
publication 到 user observation 约 7.81 ms p50。Compact multi-provider
SelectionMessage 仍然在 message level 携带共享的 scope-key large-data
reference，因为所有被选中的 DI providers 都需要同一组 encrypted activation scope
metadata；应把这类 reference passing 视为 correctness 所需成本，而不是继续压缩
payload 的优化目标。
