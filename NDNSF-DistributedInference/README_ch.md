...
`Experiments/Topology/AI_testbed.conf`。这个拓扑使用 1 ms 链路，是当前 DI
性能实验的可复现 testbed。不要在 MiniNDN 命令外层设置全局 `NDN_LOG`：NFD
会继承这个环境变量，而应用风格的 logging filter 可能导致 NFD 启动失败。低噪声
benchmark 应使用下面的脚本参数和 `NDNSF_TIMELINE_TRACE_SAMPLE_RATE`。
...
`results/yolo_2x2_native_provider_active_put_60s_minimal` 中的代表性运行输出
`YOLO_2X2_NATIVE_PROVIDERS_MININDN_OK`，60 个 measured warm requests 的
p50 为 100.70 ms，p95 为 129.93 ms，mean 为 100.27 ms。丢弃第一个 measured
warm request 后，steady warm p50 为 100.80 ms，p95 为 129.94 ms，mean 为
100.63 ms。NFD 在 warm-side 62 个被脚本计数的请求上观察到 913 个 incoming/outgoing
Data packets，即每个 observed request 14.73 个 Data packets；按 measured window
折算，每次 inference 约 15.22 个 NFD out Data packets，约 430 KB NFD
`nOutBytes`。和前面一样，byte average 只是近似 transport-size ratio，因为 NFD
byte counters 包含测量 face 上的所有包类型。

当前 native provider 路径默认使用 deterministic activation names 和 active-put
segment delivery（`NDNSF_COLLAB_LARGE_ACTIVE_PUT=1`）。Provider 生成 activation
segments 后会同时放入 IMS，并立即 `put` 到 NFD，使已经预挂的 exact segment
Interests 能直接被满足，而不再等待额外的应用层通知路径。只有做 A/B 诊断时才建议用
`NDNSF_COLLAB_LARGE_ACTIVE_PUT=0` 关闭它。
...
`results/yolo_2x2_native_provider_active_put_60s_control`。它使用同样的 60 秒
形状并打开 `--control-timing`，60 个 measured warm requests 的 p50 为
99.76 ms，p95 为 126.51 ms，steady-after-first p50 为 100.58 ms。该运行测得
p50 request latency 为 100.45 ms，provider dataflow 为 21.00 ms，role run
window 为 20.00 ms，ONNX run sum 为 2.39 ms，outer-control residual 为
78.56 ms。Activation reference wait 对所有 planned inputs 都是 0.00 ms，252 个
native dataflow inputs 和 outputs 全部匹配 deterministic plan，activation publish
每条 edge 的 p50 约 1.20 ms。剩余成本主要在 control propagation：request SVS 到
provider request observation 约 15.77 ms p50，ACK SVS 到 user pre-decrypt 约
7.46 ms p50，selection SVS 到 provider selection observation 约 32.98 ms p50，
response SVS 到 user observation 约 16.86 ms p50。Provider 侧 request admission、
selection decrypt/dispatch 和 response publication 都是亚毫秒级。换句话说，ONNX
和 activation publication 已经不是瓶颈；最大的 residual 是外层
ACK/Selection/Response control path，尤其是 SelectionMessage 通过 SVS/NFD
到达 provider 的过程。
...
SelectionMessage payload，也不应默认修改 ndn-svs timing。真正有价值的优化应该是
NDNSF 层的设计，例如 predictable SelectionMessage prefetch，或者其它有测量依据的
control-path shortcut；前提是保持同一个 selection name、permission、token 和
hybrid encryption 语义。
...
最小 C++ ONNX Runtime runner 是本地 role computation native 化的里程碑，但还没有
替换当前 MiniNDN end-to-end 脚本里的 Python providers。只有等 native provider
executable 能加载 generated plan、materialize artifacts、注册
`OnnxRuntimeModelRunner`，并直接处理 NDNSF collaboration request 后，网络级
latency 才能代表最终 C++ hot path。当前已发布的 MiniNDN 数字应理解为：
Python provider performance + native plan/timing scaffold，而不是最终 C++ hot-path
performance。
