...
作为存在时优先使用的 signing certificate。如果没有 ECDSA key，NDNSF 会自动回退到
RSA certificate 签名，保持兼容。只有在比较 dual-certificate signing path 与旧
RSA-only 行为时，才使用 `--single-rsa-certs` 做对照运行。
...
60 个 measured warm requests 的 p50 为 53.75 ms，p95 为 79.49 ms，max 为
93.49 ms。同一次运行记录到每次 measured warm inference 约 23.98 个 NFD Data
