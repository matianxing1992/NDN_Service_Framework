...
printUsage(const char* program)
{
  ...
  NDN_LOG_ERROR("  --max-suppression-ms N maximum SVS suppression delay, default library value");
...
main(int argc, char** argv)
{
  ...
  const int maxSuppressionMs = getIntArg(argc, argv, "--max-suppression-ms", -1);
  ...
  if (count <= 0 || intervalMs <= 0 || startupMs < 0 || timeoutMs <= 0 ||
      syncBatchMs < 0 || maxSuppressionMs < -1) {
    NDN_LOG_ERROR("count, interval-ms, startup-ms, timeout-ms, sync-batch-ms, "
                  "and max-suppression-ms must be valid");
    return 2;
  }
  ...
  if (maxSuppressionMs >= 0) {
    svs.getSVSync().getCore().setMaxSuppressionTime(
      ndn::time::milliseconds(maxSuppressionMs));
    NDN_LOG_INFO("App_SvsLatency maxSuppressionMs=" << maxSuppressionMs);
  }
  if (syncBatching) {
