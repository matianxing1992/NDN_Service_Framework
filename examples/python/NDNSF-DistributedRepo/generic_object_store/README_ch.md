...
示例 policy 还在 `repo_control_plane.object_classes` 下定义了一个部署特定的
`configured-demo` class。object policy smoke 会检查配置化 min/max replicas、TTL、
repair flag、`autoDelete` 和 `deletePolicy` 是否进入 catalog lookup response。
...
回原始 payload。测试还会按 object class、publisher、tags 和 mission metadata 发送
`CATALOG_QUERY`。这是 UAV recording/log 产品的 repo-level browsing prototype；完整
GS UI 仍然和这个 repo control-plane 测试分开。
`--uav-browse-smoke` 命令是命令行 browsing prototype：它从某个 Persistent repo catalog
查询匹配的 UAV objects，并通过正常的 manifest-based repo path fetch 每个 recording/log
object。
