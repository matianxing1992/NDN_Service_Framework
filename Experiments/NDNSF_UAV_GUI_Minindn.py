...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--auto-dashboard-refresh-buttons-test", action="store_true",
                        help="Start the GS GUI and verify the Preflight and Analyze refresh buttons "
                             "update the dashboard detail panels.")
    ...
    parser.add_argument("--auto-authority-alert-history-test", action="store_true",
                        help="Have the GS verify operator authority override and refresh alerts are retained.")
    parser.add_argument("--auto-authority-audit-query-test", action="store_true",
...
def main() -> int:
    ...
    try:
        ...
        if (args.auto_authority_persistence_test or
                ...
                args.auto_authority_alert_history_test or
                args.auto_authority_audit_query_test) and authority_state_file.exists():
        ...
        if args.auto_analyze_snapshot_test:
            gs_argv += [
        ...
        if args.auto_dashboard_panel_test:
            gs_argv += [
        ...
        if args.auto_dashboard_refresh_buttons_test:
            gs_argv += [
        ...
        if args.auto_authority_arbitration_test:
            gs_argv += [
                "--auto-authority-arbitration-test",
                "--operator-id", "/example/uav/operator/test-arbitration",
                "--ack-timeout-ms", "700",
                "--timeout-ms", "3000",
            ]
        ...
        if args.auto_authority_refresh_test:
            gs_argv += [
        ...
        if args.auto_authority_alert_history_test:
            gs_argv += [
        ...
        if args.auto_authority_audit_query_test:
            gs_argv += [
        ...
        if not (args.auto_patrol_test or args.auto_single_mission_test or
                ...
                args.auto_dashboard_refresh_buttons_test or
                ...
                args.auto_authority_refresh_timer_test or
                args.auto_authority_alert_history_test or
                args.auto_authority_audit_query_test or
        ...
        if args.auto_patrol_test and args.no_cli:
        ...
        elif args.auto_analyze_snapshot_test and args.no_cli:
            try:
                gs_proc.wait(timeout=70)
            except subprocess.TimeoutExpired as e:
                raise RuntimeError(f"ground station analyze snapshot smoke did not finish; see {gs_log}") from e
            if gs_proc.returncode != 0:
                raise RuntimeError(f"ground station exited with {gs_proc.returncode}; see {gs_log}")
            require_log(gs_log, "Analyze snapshot drone=" + args.drone_id)
        ...
        elif args.auto_dashboard_panel_test and args.no_cli:
            try:
                gs_proc.wait(timeout=90)
            except subprocess.TimeoutExpired as e:
                raise RuntimeError(f"ground station dashboard panel smoke did not finish; see {gs_log}") from e
            if gs_proc.returncode != 0:
                raise RuntimeError(f"ground station exited with {gs_proc.returncode}; see {gs_log}")
        ...
        elif args.auto_dashboard_refresh_buttons_test and args.no_cli:
            try:
                gs_proc.wait(timeout=90)
            except subprocess.TimeoutExpired as e:
                raise RuntimeError(f"ground station dashboard refresh buttons smoke did not finish; see {gs_log}") from e
            if gs_proc.returncode != 0:
                raise RuntimeError(f"ground station exited with {gs_proc.returncode}; see {gs_log}")
            require_log(gs_log, "PREFLIGHT_REFRESH_BUTTON_RESULT ok=true")
            require_log(gs_log, "ANALYZE_REFRESH_BUTTON_RESULT ok=true")
            require_log(gs_log, "DASHBOARD_REFRESH_BUTTONS_RESULT ok=true")
            print("NDNSF_UAV_DASHBOARD_REFRESH_BUTTONS_MININDN_SMOKE_OK")
        ...
        elif args.auto_authority_arbitration_test and args.no_cli:
            try:
                gs_proc.wait(timeout=45)
            except subprocess.TimeoutExpired as e:
                raise RuntimeError(f"ground station authority arbitration smoke did not finish; see {gs_log}") from e
            if gs_proc.returncode != 0:
                raise RuntimeError(f"ground station exited with {gs_proc.returncode}; see {gs_log}")
            require_log(gs_log, "AUTHORITY_LEASE_REJECTED")
            require_log(gs_log, "reason=lease-conflict")
            require_log(gs_log, "overridden_leases=1")
            require_log(gs_log, "AUTHORITY_ARBITRATION_RESULT ok=true")
            require_log(gs_log, "monitor=accepted")
            require_log(gs_log, "conflict=lease-conflict")
        ...
        elif args.auto_authority_refresh_test and args.no_cli:
            try:
                gs_proc.wait(timeout=45)
            except subprocess.TimeoutExpired as e:
                raise RuntimeError(f"ground station authority refresh smoke did not finish; see {gs_log}") from e
            if gs_proc.returncode != 0:
                raise RuntimeError(f"ground station exited with {gs_proc.returncode}; see {gs_log}")
            require_log(gs_log, "AUTHORITY_LEASE_REFRESH revoked=true")
            require_log(gs_log, "revoker_operator=/example/uav/operator/two")
            require_log(gs_log, "AUTHORITY_REFRESH_RESULT ok=true")
        ...
        elif args.auto_authority_alert_history_test and args.no_cli:
            try:
                gs_proc.wait(timeout=45)
            except subprocess.TimeoutExpired as e:
                raise RuntimeError(f"ground station authority alert history smoke did not finish; see {gs_log}") from e
            if gs_proc.returncode != 0:
                raise RuntimeError(f"ground station exited with {gs_proc.returncode}; see {gs_log}")
            require_log(gs_log, "AUTHORITY_ALERT type=admin-override")
            require_log(gs_log, "AUTHORITY_ALERT type=lease-revoked-detected")
        ...
        elif args.auto_authority_audit_query_test and args.no_cli:
            try:
                gs_proc.wait(timeout=45)
            except subprocess.TimeoutExpired as e:
                raise RuntimeError(f"ground station authority audit query smoke did not finish; see {gs_log}") from e
            if gs_proc.returncode != 0:
                raise RuntimeError(f"ground station exited with {gs_proc.returncode}; see {gs_log}")
            ...
            print("NDNSF_UAV_AUTHORITY_AUDIT_QUERY_MININDN_SMOKE_OK")
            ...
            require_log(gs_log, "LINK_STATE_GATE sample=initial")
            require_log(gs_log, "LINK_STATE_GATE sample=stale")
            require_log(gs_log, "arm_reason=link-stale")
            require_log(gs_log, "LINK_STATE_GATE sample=lost")
            require_log(gs_log, "arm_reason=link-lost")
            require_log(gs_log, "emergency_stop=true emergency_reason=ok")
            ...
            require_log(gs_log, "start_reason=blocked-A:waiting-heartbeat,B:waiting-heartbeat can_stop=true stop_reason=ok")
            ...
            require_log_any(gs_log, ["marker=Drone A", "marker=A P", "marker=A U P"])
            require_log(gs_log, "MISSION_CONTROL_STATE phase=progress-completed can_upload=true upload_reason=ok can_start=true start_reason=ok can_stop=true stop_reason=ok")
            ...
            require_log_any(gs_log, ["marker=Drone A", "marker=A C", "marker=A U C"])
            require_log(gs_log, "MISSION_CONTROL_STATE phase=final can_upload=true upload_reason=ok can_start=true start_reason=ok can_stop=true stop_reason=ok")
            ...
            require_log(gs_log, "can_manual=true can_panel=true mission_can_start=false mission_start_reason=no-uploaded-mission mission_can_stop=false mission_stop_reason=no-active-mission")
            ...
            require_log(gs_log, "mission_can_start=true mission_start_reason=ok mission_can_stop=true mission_stop_reason=ok mission_phases=A:uploaded")
            require_log(gs_log, "SELECTED_VIEW_STATE phase=not-ready selected=")
            require_log(gs_log, "readiness=not-ready mission=idle mission_progress=idle")
            require_log(gs_log, "video=stopped")
            require_log(gs_log, "safety_attention=false can_arm=false arm_reason=waiting-heartbeat")
            require_log(gs_log, "can_takeoff=false takeoff_reason=waiting-heartbeat")
            require_log(gs_log, "can_manual=false manual_reason=not-armed")
            require_log(gs_log, "SELECTED_VIEW_STATE phase=mission-uploaded selected=")
            require_log(gs_log, "readiness=ready mission=uploaded mission_progress=idle")
            require_log(gs_log, "video=stopped")
            require_log(gs_log, "can_arm=true arm_reason=ok can_takeoff=false takeoff_reason=not-armed")
            require_log(gs_log, "can_manual=false manual_reason=not-armed can_panel=false panel_reason=not-armed")
            require_log(gs_log, "marker=A U")
            require_log(gs_log, "DRONE_ROW_STATE phase=not-ready selected=A")
            require_log(gs_log, "readiness=not-ready armed=false gps=false mission=idle mission_progress=idle video=stopped")
            require_log(gs_log, "DRONE_ROW_STATE phase=mission-uploaded selected=A")
            require_log(gs_log, "readiness=ready armed=false gps=true mission=uploaded mission_progress=idle video=stopped")
            ...
            if args.auto_video_pressure_profile_test:
                ...
                require_log(gs_log, "GS_VIDEO_ADAPTIVE_STATE reason=pressure-profile-frame-gap")
                require_log(gs_log, "VIDEO_ADAPTIVE_VIEW_STATE phase=auto-video-pressure-frame-gap selected=" + args.drone_id)
                require_log(gs_log, "primary_pressure=decode-gap policy_reason=pressure-decode-gap")
                require_log(gs_log, "frame_gap_pressure=")
                require_log(gs_log, "decoded_frame_gap=")
            if args.auto_apply_bitrate_test:
