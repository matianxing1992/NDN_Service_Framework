#!/usr/bin/env python3
"""Freeze and execute the immutable Spec 153 six-rate successor campaign."""
...
BASE_PATH = ROOT / "Experiments/run_spec152_predictive_uav_rate_sweep.py"
RUNNER = Path(__file__).resolve()


def load_base() -> Any:
    spec = importlib.util.spec_from_file_location("spec152_runner_base", BASE_PATH)
...
BASE = load_base()
BASE.ENGINE.RUNNER = RUNNER
BASE.ENGINE.SOURCE_PATHS = BASE.ENGINE.SOURCE_PATHS + tuple(
    Path(value) for value in (
        "Experiments/run_spec153_uav_decode_stop_repair.py",
        "specs/153-uav-capture-boundary-and-stop-race/spec.md",
        "specs/153-uav-capture-boundary-and-stop-race/plan.md",
        "specs/153-uav-capture-boundary-and-stop-race/tasks.md",
        "specs/153-uav-capture-boundary-and-stop-race/contracts/successor-matrix.md",
    )
)
