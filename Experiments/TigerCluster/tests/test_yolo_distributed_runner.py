...
def test_two_ranks_share_one_preparation_and_probe_without_partial_success(tmp_path, monkeypatch, frozen_bundle, scratch_double, fault):
    ...
    plan = dict(schema='tiger-yolo-run-plan-v1', runId='two-test', case='two-node-gpu',
        ...
        requests=[dict(index=n, requestId='/two-test/'+str(n), output=str(root/'node0/user/requests'/str(n)))
                  for n in range(4)])
    ...
    monkeypatch.setattr(operator, '_normal_reference', lambda *a: (object(), tmp_path, tmp_path))
    ...
    def rank(**kw):
        launched.append(kw)
        assert kw['probe_id'] == 'c'*32 and kw['mode'] == 'two-node-gpu'
...
def test_final_join_consumes_both_actual_receipt_files_without_plan_candidate(tmp_path, monkeypatch):
    ...
    plan['requests'] = [dict(index=n) for n in range(4)]
    ...
    assert set(result['nodes']) == {'0','1'} and len(result['references']) == 4
