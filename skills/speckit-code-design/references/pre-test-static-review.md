...
## Dependency-Scoped Dispatch

门禁按依赖生效：依赖该任务的工作必须等待相应静态门通过；无依赖、文件边界清晰且自身前置已满足的任务可以继续。接口、状态、不变量、生成输入和构建配置依赖同样算依赖，不能仅凭文件名不同判断独立。既有 acceptance dependency 必须实际验收通过，不因异步审查降级。

单主会话可由主代理编码、独立只读 review-agent 子代理异步审查。发送审查后，主代理优先推进已登记的独立任务，不立即阻塞等待；没有合格任务、存在写入冲突或下一工作依赖审查结果时才等待，并记录原因。没有子代理能力时明确采用串行回退，不声称并行。

调度前在同一批次 evidence 记录任务对、已满足前置、独立性依据、各自写入文件和共享接口 owner。共享文件由单一 owner 修改；需要重排或细分任务时先更新 plan/tasks 和 Design binding，不自动删除 Depends。审查输入必须是固定 commit 或不可变快照，包含未跟踪的新源码、完整 diff、周边源码及内容摘要；只读代理不能追着变化中的工作树审查。记录派发/返回时间及实际重叠工作，不能把两个代理的存在当并行证据。

审查失败时阻塞该任务及其依赖闭包；无关工作可继续。修复形成新快照，复审受影响范围；共享接口或不变量变化时重新评估独立性并使受影响审查结果失效。所有成员仍须逐任务静态通过，再对汇合后的实际候选做组合审查，才可共享构建/测试。不能用旧快照 PASS 验收后来修改的代码，也不能让编码并发污染正在验证的候选或竞争同一 build tree。
...
3. 在同一审查结果中填写 [batch-quality-gates.md](batch-quality-gates.md) 的 Coverage matrix：逐项核对真实生产入口/调用方、实现与 wire、测试/harness/oracle、构建注册/source closure、迁移/证据路径；对复算 canonical bytes/digest/identity 的 helper 还要对照生产 serializer 的字段集合、顺序、规范化和 source identity。只要目标有新增入口、跨库调用或历史链接漏项，build lane 还要列出 project-symbol definition map（符号→定义文件→target/library）及 `rg`/CodeGraph、`nm -C`/`readelf` 查询。不适用项写理由，无法确认项写 `gap`。至少给出实际文件/符号和查询或检查命令，不能用“目录已看”代替。
...
5. 覆盖充分且没有控制性问题，记录 `STATIC_PASS / TESTS_DEFERRED / Batch ID`，并在同一结果
   ...
   已解锁任务，不启动常规构建/测试；无依赖任务在审查期间按 Dependency-Scoped Dispatch 继续。状态仍为 PARTIAL，保持未勾选并登记待执行选择器。
