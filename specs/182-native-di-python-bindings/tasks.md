...
| --- | --- | --- | --- | --- |
| [R4-B4 Authenticated Conversation Chain](evidence/r4-b4-conversation-chain-20260908.md) | PARTIAL | R4-B3; T011-C acceptance retained | CC-1/CC-2 focused C++ 9 cases、CC-3A projection、CC-3B requester/provider transaction wiring 与 runtime+coordinator 构造均已构建验证；49 个 requester/conversation/provider/stream 回归 cases PASS；`integration-tests -j4` 链接与 `Spec182GrantClientFlow/*` 2 cases PASS。R4-B5 已补本地公开 requester FULL_CONTEXT 成功出口，但 receipt/ACK 仍预置；公开两轮、真实跨进程 receipt/control、T011-C/T016 资格仍未完成 | 2026-09-08 |
| [R4-B6 Real Provider Conversation](evidence/r4-b6-provider-conversation-20260908.md) | READY | R4-B4/R4-B5; T011-C/CC-4 | 已登记真实 Provider 两轮稳定出口：公开 FULL_CONTEXT→receipt/control/commit→APPEND_DELTA，并覆盖恢复或单次 replacement 首个失败边界；实现依赖与 selector 已冻结，尚未编码或运行；完成后更新五 lane Coverage matrix 和 T011-C evidence | 2026-09-08 |
...
| R4-B6 | planned: production entry/callers (`NativeInferenceClient::request` plus `NdnsfIntegrationEnvironment`); implementation/wire (`NativeProviderHandler` receipt/control and coordinator continuation); test/harness/oracle (real SVS two-turn selector and recovery negative cases); build/source closure (`integration-tests` source list and case manifest); migration/evidence (T011-C evidence and raw run directory) | not started; static gate pending implementation | not run | no runtime result yet; seeded local records are excluded from this batch | planned `integration-tests -j4` named selector; exact closure and elapsed recorded after implementation | planned; no PASS label before execution | [R4-B6 plan/evidence](evidence/r4-b6-provider-conversation-20260908.md); remaining: implementation, static review, build and real two-turn run |
