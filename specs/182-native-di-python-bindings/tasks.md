...
| [R11-B8-G39 Compute Preflight Scratch Parity](evidence/r11-b8-g39-compute-preflight-scratch-parity-20260910.md) | CLOSED_FOR_VALIDATION | R11-B8-G38; Spec110 topology contract | **Deployment harness:** `preflight-compute.sh` now accepts only `/tmp/ndnsf-di-<SLURM_JOB_ID>` or its run-id suffix, matching both active Slurm templates and `run-container.sh`; nested and other-job paths fail before Apptainer. Five focused node-script tests, shell syntax and diff check pass. Real Slurm/SIF/NDN route qualification remains open | 2026-09-10 |
