...
**Revision**: 114 | **Status**: DRAFT / T001 DONE
...
| R10-B78 | production entry/callers: `di-native-provider --serve` → `ServiceProvider::serve`/Face loop; implementation/wire: bounded `--run-for-ms` serve lifetime and explicit installation-thread join/drain; test/harness/oracle: Provider CLI parser/source regression plus bounded check-only/serve lifecycle probe; build/source closure: current Provider target and registered source list under system-first `-j2`; migration/evidence: establishes a deterministic finite Provider process for the next independent requester/Provider case, without claiming transport or qualification | IN_PROGRESS | allocation basis: finite daemon exit and callback lifetime are shared prerequisites for P1/P2; stable exit is `SERVE_READY` followed by run-limit shutdown with joined install thread; no requester/Provider request is claimed in this batch | source/static checks complete; target build passed; finite serve probe is still pending, and static review found the join can wait for the full permission deadline | implementation and review are partial; no bounded behavior result yet | `STATIC_PASS`/`BUILD_PASS`/`FOCUSED_BEHAVIOR_PASS` only after bounded probe and source closure; parent P1/P2/T010/T016 remain PARTIAL | [R10-B78 evidence](evidence/r10-b78-provider-run-limit-20260909.md); next: repair the bounded wait and use the finite Provider in an independent requester/Provider manifest |
| R10-B79 | production entry/callers: current Spec182 C++/Python/qualification call graph; implementation/wire: requester/provider/stream/continuation contracts and caller migration; test/harness/oracle: CodeGraph, Python AST, Cppcheck, structure/validator, and registration-vs-runnable checks; build/source closure: current R10-B78 Provider build, `readelf`/`ldd`, and source registration; migration/evidence: read-only large static audit of implementation distance and static miss classes | OPEN_FOR_NEXT_BATCH | allocation basis: whole-chain audit before the next implementation batch; no parent task status advanced; stable audit exit is one evidence record with prioritized findings and a closure decision | audit complete; source and harness findings are recorded, while runtime transport, cross-process, continuation/recovery, no-Python migration, and deployment qualification remain unobserved | `STATIC_PASS` only for the audit/inventory boundary; it is not a product or protocol qualification result | [R10-B79 evidence](evidence/r10-b79-large-static-audit-20260909.md); next: repair SA-04/SA-03, then run one independent requester→Core→Provider case |
...
发现 requester 输入/输出路径与声明契约不一致、generation/stream budget 上限互相冲突、Provider
`--run-for-ms` 不能约束 permission-install join、冻结 manifest 无法被 canonical runner 加载、维护 caller
仍保留旧 API，以及 artifact 仍绑定主机依赖路径。该批没有推进 P1--P7 或任何父任务；静态检查和目标构建
