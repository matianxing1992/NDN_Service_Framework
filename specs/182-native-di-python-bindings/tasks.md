...
**Revision**: 60 | **Status**: DRAFT / T001 DONE
...
| R10-B16 | production entry/callers: `run_case` → `make_launch` in `tests/standalone/run-spec182-native-closure.py`; implementation/wire: validated `nodes` context, held `/proc/.../ns/net` descriptor and `nsenter` → bubblewrap/strace launch; test/harness/oracle: node inode/PID/start-ticks/socket/peer validation and missing/stale-context negatives; build/source closure: Python harness only, no product rebuild; migration/evidence: runner now has an explicit node-context boundary, while MiniNDN topology/case definitions and T016 qualification remain open | IN_PROGRESS; stable preflight/launch exit pending tests and evidence | R10-B15; native-isolation-design node contract | declared-node cases must fail closed without valid context and use the held namespace FD when valid; no host-netns masquerade or qualification promotion | static review must cover node validation, fd lifetime, `nsenter` argv, process cleanup, test registration and source closure; final lane gaps remain for owner topology and business cases | Python selectors for node context plus `py_compile`; no native build | `TESTS_DEFERRED / R10-B16`; not `QUALIFICATION_PASS` | evidence to be added after implementation; next owner batch must provide actual MiniNDN node contexts |
...
2026-09-09 R10-B16 native closure node-context boundary / **IN_PROGRESS**：runner 的下一项
修复已登记，目标是校验 MiniNDN 提供的 namespace inode、owner PID/start ticks、NFD socket
和 peer metadata，并以 held FD 通过 `nsenter` 接到既有 bubblewrap/strace launch。缺 context
必须在业务启动前 `UNQUALIFIED`；本批不新增 DI qualification case、不创建 host-netns
替身。完成后补独立 evidence 和静态/测试结果。
