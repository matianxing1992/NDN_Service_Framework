...
R11-B8 现开始按 caller group 批次执行；G4 已形成 native checkpoint handle export 出口，G5 已接通 Qwen caller 的 FULL_CONTEXT/APPEND_DELTA DTO 映射；R11-B8-G47 已完成 examples C++ target source/link closure；R11-B9-G5 已刷新当前 checkpoint 的 compatibility manifest provenance；R11-B9-G3 已用当前构建独立验证真实 Provider 的第二轮、恢复和 replacement 边界。已有局部 PASS 及下面历史记录
保留，父任务不因本轮局部实现升级。
...
| [R11-B8-G30 Port Availability Preflight](evidence/r11-b8-g30-port-availability-preflight-20260910.md) | CLOSED_FOR_VALIDATION | R11-B8-G29; Spec110 topology contract | **Deployment harness:** duplicate address/transport endpoints are rejected and target-node IPv4 TCP/UDP bind probes run before NFD; injected occupied-port failure exits 4 with `survivors: 0` and no NFD log. 22/22 topology unit, network integration and shell syntax pass. Dynamic cross-job allocation and real Slurm/SIF qualification remain open | 2026-09-10 |
...
| [R11-B8-G39 Compute Preflight Scratch Parity](evidence/r11-b8-g39-compute-preflight-scratch-parity-20260910.md) | CLOSED_FOR_VALIDATION | R11-B8-G38; Spec110 topology contract | **Deployment harness:** `preflight-compute.sh` now accepts only `/tmp/ndnsf-di-<SLURM_JOB_ID>` or its run-id suffix, matching the canonical `ndnsf-di.sbatch.in` template and `run-container.sh`; nested and other-job paths fail before Apptainer. Five focused node-script tests, shell syntax and diff check pass. Real Slurm/SIF/NDN route qualification remains open | 2026-09-10 |
| [R11-B8-G40 Scratch Symlink Boundary](evidence/r11-b8-g40-scratch-symlink-boundary-20260910.md) | CLOSED_FOR_VALIDATION | R11-B8-G39; Spec110 topology contract | **Deployment harness:** compute preflight, canonical SIF runner, topology supervisor and direct launcher now resolve scratch before writing evidence, sockets or process homes and reject a job-looking symlink or parent-path alias. 42 focused tests, shell syntax, network integration and design checks pass. Real Slurm/SIF/NDN route qualification remains open | 2026-09-10 |
...
| [R11-B8-G48 Qwen Native Observer Type Guard](evidence/r11-b8-g48-qwen-observer-type-guard-20260911.md) | CLOSED_FOR_VALIDATION | R11-B8-G47; T013-D | **Python caller edge:** decoded native observer payload must be a mapping before schema access; JSON array regression, legacy/native compatibility and Qwen entrypoint checks pass (36 tests), `py_compile` and diff check pass. This closes one caller fail-open only; maintained caller full migration, Provider/no-Python and T016/T017 remain open | 2026-09-11 |
| [R11-B9-G6 Compatibility Manifest Provenance Refresh](evidence/r11-b9-g6-compatibility-manifest-refresh-20260911.md) | CLOSED_FOR_VALIDATION | R11-B8-G48; O-004 inventory | **Documentation/generator:** regenerated the 344-entry compatibility manifest after the `04337f4c` observer type guard checkpoint; `sourceCommit` matches the current source, generator and design validator pass. This remains routing evidence only; semantic parity, maintained caller migration, legacy zero-use, no-Python and T015--T017 remain open | 2026-09-11 |
| [R11-B9-G7 T016 PO-001 Current Native Stream](evidence/r11-b9-g7-t016-po001-current-native-20260911.md) | CLOSED_FOR_VALIDATION | R11-B9-G6; T014-A/B; T016-A | **Native qualification subset:** current `build-nac182/integration-tests` executed the frozen `PO-001-stream` case inside a real MiniNDN requester namespace with staged ELF closure, bubblewrap and syscall observation; C++ `SPEC182_NATIVE_DI_REQUEST_RESULT_OK`, seven evidence classes, no Python/libpython trace and runner `PASS` (rc=0, 5.587 s). This closes one PO-001 stream sub-exit only; complete I01--I08/PO matrix, maintained callers, no-Python zero-use, T015 and T016 parent remain open | 2026-09-11 |
| [R11-B9-G8 Client Close Pending-Operation Registry](evidence/r11-b9-g8-client-close-operation-registry-20260911.md) | CLOSED_FOR_VALIDATION | R11-B9-G7; T010-A | **C++ lifecycle:** a strong pending-operation registry now keeps dropped public handles cancellable by `NativeInferenceClient::close()`; terminal operations unregister so completed fire-and-forget requests are not retained. A fresh `-j3` unit build (190/190, 57.934 s, no sustained swap) and the affected client suite (19/19 cases, 376/376 assertions) pass. This closes only the client close ownership boundary; complete lifecycle, maintained callers, no-Python, T015/T016 and T017 remain open | 2026-09-11 |
| [R11-B9-G9 Compatibility Manifest Provenance Refresh](evidence/r11-b9-g9-compatibility-manifest-refresh-20260911.md) | CLOSED_FOR_VALIDATION | R11-B9-G8; O-004 inventory | **Documentation/generator:** regenerated the 344-entry compatibility manifest after the C++ operation-registry checkpoint; `sourceCommit=729555fa`, generator output and source identity match. This remains routing evidence only; semantic parity, maintained caller migration, legacy zero-use, no-Python and T015--T017 remain open | 2026-09-11 |
| [R11-B9-G10 Current C++ Spec182 Regression](evidence/r11-b9-g10-current-cpp-spec182-regression-20260911.md) | CLOSED_FOR_VALIDATION | R11-B9-G8; T016-A | **C++ primary:** same fresh `unit-tests` binary passes the full `*Spec182*/*` selector with 260/260 cases and 7107/7107 assertions. This is a regression lane only; maintained callers, no-Python zero-use, complete I01--I08/PO matrix, exact-SIF/multi-machine and T017 remain open | 2026-09-11 |
| [R11-B9-G11 Current Whole-Chain Static Audit](evidence/r11-b9-g11-current-whole-chain-static-audit-20260911.md) | CLOSED_FOR_VALIDATION | R11-B9-G10; R10-B82 audit | **Current-source static audit:** SA-01 standalone conversation loading and SA-02 authority-key separation are resolved in current code; SA-03 maintained caller migration (16 calls/5 files), SA-04 complete independent qualification, SA-05 exact-SIF closure and SA-06 cross-process request-ID qualification remain open. No parent task is promoted | 2026-09-11 |
| [R11-B9-G12 Current C++ Spec182 Integration Selector](evidence/r11-b9-g12-current-cpp-spec182-integration-20260910.md) | CLOSED_FOR_VALIDATION | R11-B9-G11; T010/T016-A | **C++ primary:** current `build-nac182/integration-tests` active selector passes 2/2 cases and 21/21 assertions for grant publication/provider unwrap. This refreshes only the narrow integration lane; full process matrix, maintained callers, no-Python and T016/T017 remain open | 2026-09-10 |
| [R11-B10-G13 Current Cross-Process Native Chain](evidence/r11-b10-g13-current-cross-process-chain-20260910.md) | CLOSED_FOR_VALIDATION | R11-B9-G12; T010/T011; R11-B2--B6 | **C++ primary:** current `build-nac182` independent requester/authority/Provider runs pass unary numerical, stream token oracle, two-turn conversation, wrong-parent rejection, Provider-restart state-missing rejection, replacement `attempt-2`, and no-backup terminal failure. Driver is Python lifecycle tooling, so no-Python qualification and maintained caller/T014--T017 gates remain open | 2026-09-10 |
| [R11-B9-G5 Compatibility Manifest Provenance Refresh](evidence/r11-b9-g5-compatibility-manifest-refresh-20260911.md) | CLOSED_FOR_VALIDATION | R11-B8-G47; O-004 inventory | **Documentation/generator:** regenerated the 344-entry compatibility manifest after the current checkpoint; `sourceCommit` now equals `489d606c`, generator and design validator pass, and the only tracked delta is provenance. This remains routing evidence only; semantic parity, maintained caller migration, legacy zero-use, no-Python and T015--T017 remain open | 2026-09-11 |
...
| [R11-B8 Maintained Callers](contracts/native-first-execution.md#dispatch-cards) | PARTIAL | R11-B7; corresponding T012 ABI | G1/G2 generic facade 与 G48 Qwen observer guard 已形成局部出口；仍需 15 个 caller group 的 native entry、实际行为、兼容 wrapper 及旧路径零使用证据，不能按子批次数量计全量完成 | 2026-09-11 |
...
的 `mobility_wording_revision`；保留原实验结果，不改变产品验收状态。

文档旁路记录（2026-09-10，CHALLENGE TERMINOLOGY）：摘要改用 ABE-backed challenge
verification，避免与 Response Data 混淆；中英文四入口同步重建。检查记录见
[validation](../../docs/PAPER/proposal-defense/research-revision-validation.json)的
`challenge_terminology_revision`；仅修正文档措辞，不改变产品验收状态。
...
校验，旧 `ndnsf-di.sbatch.in` 仍只启动一个 `run-container.sh`，没有 process-map、NFD route
或 topology launcher 接线。现在 adapter 在任何 preflight、SIF materialization 或 state
directory 创建前返回 `SLURM_MULTINODE_TOPOLOGY_RUNNER_REQUIRED`；单元测试同时确认失败不调用
...
2026-09-11 R11-B8-G48 Qwen native observer type guard / **CLOSED_FOR_VALIDATION**：修复
Qwen native caller 在 JSON array/scalar observer payload 上直接调用 `.get()` 的
fail-open 边界；caller 现在要求 decoded payload 为 mapping，并以既有 observer error
汇总拒绝。36 个 Python/legacy/entrypoint checks、`py_compile` 和 `git diff --check`
通过。该修复不改变 C++ wire/Provider，maintained caller 全量迁移、legacy zero-use、
no-Python、T015/T016/T017 仍保持原状态。详见
[R11-B8-G48 evidence](evidence/r11-b8-g48-qwen-observer-type-guard-20260911.md)。

2026-09-11 R11-B9-G5 compatibility manifest provenance refresh / **CLOSED_FOR_VALIDATION**：
当前 checkpoint `489d606c` 修改了 Waf/docs 身份后，重新生成 344-entry manifest，
`sourceCommit` 与当前提交一致；generator、design validator 和 diff check 通过。只刷新
路由索引身份，不改变维护 caller 的语义、legacy zero-use、no-Python 或资格状态。详见
[R11-B9-G5 evidence](evidence/r11-b9-g5-compatibility-manifest-refresh-20260911.md)。

2026-09-11 R11-B9-G6 compatibility manifest provenance refresh / **CLOSED_FOR_VALIDATION**：
在 `04337f4c` 的 Qwen observer type guard 之后重新生成 344-entry manifest，
`sourceCommit` 与当前源码 checkpoint 一致；generator、design validator 和 diff check
通过。该批只更新 caller-edge 路由索引身份，不改变 maintained caller、legacy zero-use、
no-Python 或 T015--T017 资格状态。详见
[R11-B9-G6 evidence](evidence/r11-b9-g6-compatibility-manifest-refresh-20260911.md)。

2026-09-11 R11-B9-G7 T016 PO-001 current native stream / **CLOSED_FOR_VALIDATION**（仅限
单个 qualification 子出口）：当前 `build-nac182/integration-tests` 在真实 MiniNDN
requester namespace、staged ELF 闭包、bubblewrap 和 strace 观察中执行冻结的
`PO-001-stream`，C++ `SPEC182_NATIVE_DI_REQUEST_RESULT_OK`、七类 evidence、cleanup
和 no-Python trace 均通过，runner `PASS`（rc=0，约 5.587 秒）。完整 I01--I08/PO
matrix、maintained caller/no-Python zero-use、T015/T016 parent 与 T017 仍开放。详见
[R11-B9-G7 evidence](evidence/r11-b9-g7-t016-po001-current-native-20260911.md)。

2026-09-11 R11-B9-G8 client close pending-operation registry / **CLOSED_FOR_VALIDATION**：
静态审查发现 `NativeInferenceClient` 原先只以 weak operation index 跟踪 live
handles；调用方丢弃 handle 后，queued preparation 仍可能在 `close()` 之后执行。
现在由 strong pending registry 保留未终态 operation，`markTerminal()` 终态注销，
`close()` 在 client mutex 与 registry mutex 下摘出全部 pending operation 后统一取消。
首轮 `-j3` 仅在新测试 fixture 的 constness 转换处失败，修复后 fresh 190/190 build
（57.934 s）及 `Spec182ClientState` 19/19（376/376 assertions）通过，`git diff --check`
通过。该批只关闭 client close ownership boundary，完整 lifecycle、maintained callers、
legacy zero-use、no-Python、T015/T016/T017 仍开放。详见
[R11-B9-G8 evidence](evidence/r11-b9-g8-client-close-operation-registry-20260911.md)。

2026-09-11 R11-B9-G9 compatibility manifest provenance refresh / **CLOSED_FOR_VALIDATION**：
在 `729555fa` 的 C++ operation registry checkpoint 后重新生成 344-entry manifest，
`sourceCommit` 与当前源码一致，`dynamicAppSdk=67`。该批只刷新 inventory 身份，不改变
semantic parity、maintained caller、legacy zero-use、no-Python 或 T015--T017 资格状态。
详见 [R11-B9-G9 evidence](evidence/r11-b9-g9-compatibility-manifest-refresh-20260911.md)。

2026-09-11 R11-B9-G10 current C++ Spec182 regression / **CLOSED_FOR_VALIDATION**：
在 `729555fa` operation registry 修复后的 fresh `unit-tests` binary 上，完整
`*Spec182*/*` selector 通过 260/260 cases、7107/7107 assertions（`rc=0`）。详细
日志中的 verbose timeout 仅是前一次 `--log_level=test_suite` 运行边界；安静日志和
本次汇总均完成。该批只关闭 C++ regression lane，不推进 maintained callers、
no-Python、完整 PO matrix 或 T016/T017。详见
[R11-B9-G10 evidence](evidence/r11-b9-g10-current-cpp-spec182-regression-20260911.md)。

2026-09-11 R11-B9-G11 current whole-chain static audit / **CLOSED_FOR_VALIDATION**：
当前源码复查确认 standalone C++ conversation loader 与 authority-key separation
已经接线；维护 examples 仍有 5 个文件、16 个 compatibility/automatic-planner
调用，SA-04 independent qualification、SA-05 exact-SIF closure、SA-06 cross-process
request-ID qualification 仍开放。该批只刷新当前静态结论，不推进任何 parent task。
详见 [R11-B9-G11 evidence](evidence/r11-b9-g11-current-whole-chain-static-audit-20260911.md)。

2026-09-10 R11-B9-G12 current C++ Spec182 integration selector / **CLOSED_FOR_VALIDATION**：
当前 `build-nac182/integration-tests` 的 active `*Spec182*/*` selector 通过 2/2
cases、21/21 assertions，覆盖 grant exact-name publication/provider unwrap。该批只
刷新窄 integration lane，不推进完整 process matrix、maintained callers、no-Python
或 T016/T017。详见 [R11-B9-G12 evidence](evidence/r11-b9-g12-current-cpp-spec182-integration-20260910.md)。
...
ACK-driven Python planner 组合，且一节点 Slurm runner 没有执行 case config 声明的多节点
`runtime.nodes`。现已把 `SPEC180_YOLO_NATIVE_REQUESTER_CONFIG` 纳入 workload allowlist，
在 renderer 预检 config/topology 文件，强制 native requester，移除 native 分支拒绝的
legacy lifecycle/request-id 参数，并在多节点声明时 pre-start fail closed；render manifest
记录 single-node scope 与 topology digest；inventory input identity 同时绑定 native config。
39 个 dispatcher/release/Tiger 合同测试、53 个含 legacy/Qwen 回归测试及 97 个组合回归测试、
migration hash、py_compile、shell syntax、Spec Kit structure
和 diff check 通过。真实 Tiger/SIF、多节点 NDN、C++ no-Python 和 T016/T017 仍未执行。
详见 [R11-B8-G45 evidence](evidence/r11-b8-g45-tiger-yb-native-topology-binding-20260910.md)。

2026-09-10 R11-B9-G4 compatibility manifest provenance / **CLOSED_FOR_VALIDATION**（仅限
路由证据）：维护 caller 批次前发现 `contracts/compatibility-manifest.json` 的
`sourceCommit` 停留在旧提交 `f661e3fc`，可能把静态映射指向过期行号和哈希。已在当前
checkpoint `400d8126` 重新生成，保留 344 个 entries（`dynamicAppSdk=67`）；generator、
design validator（`ok=true`）、11/11 Spec Kit sync 和 diff check 通过。manifest 仍不证明
字段/错误/状态 parity、native ownership、维护 caller 迁移、legacy zero-use、no-Python 或
最终资格。详见 [R11-B9-G4 evidence](evidence/r11-b9-g4-compatibility-manifest-provenance-20260910.md)。
...
中的地址直接写进 `allocationAddresses`，使地址一致性检查在真实路径上变成自证；同时任意
Slurm 环境都可通过 `NDNSF_SPEC110_PROBE_OBSERVATION` 跳过 allocation-order 与 socket probe，
且多机 map 允许不同节点复用同一 IP（只要端口不同）。现已由目标节点 UDP route probe 产生
实际 source IPv4，按 rank 精确比较；nodes array 乱序、重复 node address、real-job observation bypass 和
无界 `srun` probe 均 fail closed。workdir/identity canonical digest 现在还绑定 root/regular-file
permission bits，避免可执行或可读模式在节点间分叉。84 个相关 Python 测试、network integration、
shell syntax 和 diff check 通过；该门不替代真实 Slurm/SIF、跨节点 NDN、GPU、no-Python 或
T016/T017 资格。详见
[R11-B8-G43 evidence](evidence/r11-b8-g43-network-probe-observation-integrity-20260910.md)。

2026-09-10 R11-B8-G42 cross-node input content digest / **CLOSED_FOR_VALIDATION**（仅限
deployment harness）：本轮发现 supervisor 只检查每个节点存在 `workdir`/`identityRef`，却不
检查挂载内容是否相同；不同节点的 bundle revision 或身份数据库会让 MiniNDN 单机通过、真实
多机在模型、配置或签名身份处产生分叉。现已在 NFD 启动前对 workdir 及每个非 NFD identity
root 按相对路径、目录项和文件 bytes 计算摘要，并在每个目标节点重新计算比对；缺失、特殊文件、
符号链接或摘要不一致均 fail closed，摘要写入 `workdir-digest.txt` 和
`identity-digests.tsv`。29/29 topology unit、network integration（含注入的 mismatch）和
shell/diff 检查通过；该门不替代真实 Slurm/SIF、跨节点 NDN、GPU、no-Python 或 T016/T017
资格。详见 [R11-B8-G42 evidence](evidence/r11-b8-g42-cross-node-input-digest-20260910.md)。
...
该目录传给 canonical `run-container.sh`；真实作业会在容器启动前触发 scratch preflight。模板现在
显式设置 `RUN_ID` 并生成 `/tmp/ndnsf-di-${SLURM_JOB_ID}-${RUN_ID}`，与 runner、supervisor 和
Spec110 contract 一致。`pytest -q tests/container/itiger-qwen-live/unit/test_github_sealed_workflow.py`
通过 31 tests，diff check 通过。该门不替代真实 Slurm/SIF、跨节点 NDN route、GPU、no-Python 或
T016/T017 资格。详见
[R11-B8-G37 evidence](evidence/r11-b8-g37-qwen-template-scratch-wiring-20260910.md)。

2026-09-10 R11-B8-G36 runner scratch name parity / **CLOSED_FOR_VALIDATION**（仅限 deployment
harness）：修复批处理模板生成 `ndnsf-di-$SLURM_JOB_ID-$RUN_ID`、而 canonical `run-container.sh`
只接受 `ndnsf-di-$SLURM_JOB_ID` 的跨入口契约分叉。runner 现在接受无后缀或以当前 job ID 开头的
`-<RUN_ID>` 后缀，并继续拒绝其他作业目录。`pytest -q tests/python/test_spec170_sif_build_record.py`
通过 9 tests，shell syntax 与 diff check 通过。该门不替代真实 SIF/Slurm、跨节点 NDN route、GPU、
no-Python 或 T016/T017 资格。详见
[R11-B8-G36 evidence](evidence/r11-b8-g36-run-container-scratch-parity-20260910.md)。

2026-09-10 R11-B8-G35 scheduler allocation order binding / **CLOSED_FOR_VALIDATION**（仅限
deployment harness）：修复 supervisor 与独立 route retry 直接使用 `srun --relative=<nodeRank>`
却不核对 Slurm allocation hostname 顺序的隐含约束。现在在任何 NFD 或 route command 前读取
`scontrol show hostnames "$SLURM_JOB_NODELIST"`，并和 process map 的 rank 顺序逐项比较；查询失败、
空结果或顺序不一致都会 pre-start 失败，反例返回 `SPEC110_ALLOCATION_NODE_ORDER_MISMATCH`，不创建
NFD 状态。25/25 topology unit、network integration、shell syntax 和 diff check 通过。该门不替代
真实 Slurm/SIF、跨节点 NDN route、GPU、no-Python 或 T016/T017 资格。详见
[R11-B8-G35 evidence](evidence/r11-b8-g35-allocation-order-binding-20260910.md)。
...
`identityRef` 符号链接未被 G31 检查覆盖的问题。非 test mode 现在要求 scratch basename 绑定
`SLURM_JOB_ID`；supervisor 与 launcher 均拒绝 identity root symlink。24/24 topology unit、
network integration、shell syntax 通过，最终 `NETWORK_SCRIPT_PASS`。详见
[R11-B8-G33 evidence](evidence/r11-b8-g33-job-scoped-scratch-identity-root-20260910.md)。

2026-09-10 R11-B8-G32 stale NFD socket boundary / **CLOSED_FOR_VALIDATION**（仅限 deployment
harness）：修复复用 scratch 中旧 NFD socket 使 readiness 错误放行的隐患。每个目标节点启动前
删除 socket，并按 rank 保存 `srun` PID；readiness 同时要求 PID 存活和新 socket。23/23
topology unit、network integration、shell syntax 通过，最终 `NETWORK_SCRIPT_PASS`。详见
[R11-B8-G32 evidence](evidence/r11-b8-g32-stale-nfd-socket-boundary-20260910.md)。

2026-09-10 R11-B8-G31 identity symlink boundary / **CLOSED_FOR_VALIDATION**（仅限 deployment
harness）：修复 `cp -a` 保留 `.ndn` 符号链接、让 scratch `HOME` 仍可能回到共享存储的隔离
绕过。supervisor 在 NFD 启动前、launcher 在 copy 前均拒绝 identity source `.ndn` 树中的
symlink；unit 23/23、network integration 和 shell syntax 通过，反例分别在 provider exec 前
和 NFD 启动前失败。详见 [R11-B8-G31 evidence](evidence/r11-b8-g31-identity-symlink-boundary-20260910.md)。

2026-09-10 R11-B8-G30 port availability preflight / **CLOSED_FOR_VALIDATION**（仅限
deployment harness）：process map 拒绝重复 `(address,transport-port)` endpoint，supervisor 在
任何 NFD 启动前由目标节点执行 IPv4 TCP/UDP bind probe；占用端口反例以
`SPEC110_PORT_NOT_AVAILABLE` 退出 4，保留 `teardown.json`、`survivors: 0` 且没有 NFD 日志。
22/22 topology unit、network integration 和 shell syntax 通过。该 probe 不等于跨作业端口
租约，真实 Slurm/SIF/跨节点资格仍开放。详见 [R11-B8-G30 evidence](evidence/r11-b8-g30-port-availability-preflight-20260910.md)。
...
target node 在自己的 row 内建立有界窗口，避免前一节点的慢启动挤压后续节点。shell syntax、
integration 及防回归 source assertion 通过，最终输出 `NETWORK_SCRIPT_PASS`。这只修复 preflight
时限边界，不推进真实 Slurm/SIF/跨节点 NDN/GPU/no-Python 或 T016/T017；详见
[R11-B8-G28 evidence](evidence/r11-b8-g28-per-node-readiness-deadline-20260910.md)。

2026-09-10 R11-B8-G27 routable IPv4 address boundary / **CLOSED_FOR_VALIDATION**（仅限
多机 topology preflight）：静态审查发现 v1 map 接受 IPv6、loopback、unspecified 和 multicast，
但 NFD/route 实现固定为 `tcp4`/`udp4`；多机 loopback 还会把每个节点的本地地址误当成远端。
现已要求 IPv4，拒绝 unspecified/multicast，并在 multi-node 拒绝 loopback/link-local。21/21
topology unit、network integration 与 Python 静态检查通过；真实跨节点 route、端口、SIF/GPU、
no-Python、T016/T017 仍开放。详见 [R11-B8-G27 evidence](evidence/r11-b8-g27-routable-ipv4-boundary-20260910.md)。
...
进程验证 unary、stream、FULL_CONTEXT/APPEND_DELTA、alternate-provider replacement 和
no-backup fail-closed；C++ numerical oracle、grant verification、真实 ORT CPU execution
及错误 parent 拒绝均有原始日志。Python 只负责进程生命周期。tiny fixture 不代表 Qwen3-0.6B，
也不代表 MiniNDN 或 Slurm/SIF 多机资格；maintained callers、legacy zero-use、T014 I02--I08/
no-Python、exact-SIF、T015--T017 仍开放。详见 [R11-B9-G2 evidence](evidence/r11-b9-g2-cross-process-native-chain-20260910.md)。
...
digest、exact-SIF canonical runner 及真实 Slurm/GPU/no-Python 资格缺口。文档链接、结构审计、
design validator 和 diff 检查通过；不推进产品父任务或资格状态。详见
[R11-B8-G24 evidence](evidence/r11-b8-g24-multimachine-audit-synchronization-20260910.md)。

2026-09-10 R11-B8-G23 early input failure evidence / **CLOSED_FOR_VALIDATION**（仅限
Slurm topology 启动边界）：将 pre-start traps 从 scratch 创建之后提前到输入文件确认之后，覆盖
无效 workdir、scratch 和 allocation-mode 约束；无效 workdir 注入返回 `3` 并写出
`survivors: 0` 的 `teardown.json`。19/19 topology unit、network integration、shell/Python
静态检查通过；真实 Slurm/SIF 多机资格仍未运行。详见
[R11-B8-G23 evidence](evidence/r11-b8-g23-early-input-failure-evidence-20260910.md)。
...
`ServiceProvider` 不同的 `localProviderName`，并允许空 `providerBootId` 在首个 target
时固定进共享 lease。现已在安装 host state 前 fail closed，按底层 Provider identity
校验并保存 canonical URI；新增负例覆盖 missing/foreign identity、missing boot，合法
重试仍可注册 target；fresh unit 190/190 build，host selector 8/8 cases、67/67 assertions，
完整 `Spec182*` 259/259 cases、7104/7104 assertions 通过。该修正防止小型 MiniNDN fixture 掩盖多机部署中的 offer、evidence、
data-prefix 与 lease 绑定错误；不证明跨机传输、maintained callers、no-Python、MiniNDN
或 T016/T017。详见 [R11-B8-G12 evidence](evidence/r11-b8-g12-provider-identity-binding-20260910.md)。
...
2026-09-10 R11-B8-G9 native stream process revalidation /
**CLOSED_FOR_VALIDATION** for the independent C++ stream process boundary.
`run-spec182-native-stream-process.py` returned `STREAM_RC=0`; requester emitted
8/8 token oracle events and `NATIVE_REQUEST_SUCCEEDED`, while the Provider emitted
grant verification and real ORT CPU execution evidence. Maintained Qwen caller
migration, remaining callers, legacy zero-use, no-Python and T016 remain open.
...
提交的 authenticated opaque wire，终态 gate 保证 ordinary、in-flight、failed 和
cancelled request 不返回 checkpoint；真实 Provider conversation、`Spec182*` unit
257 cases、pybind extension rebuild/import 与 81 个 wrapper/compatibility tests 通过。
第一次 Python import 暴露共享 DI 库旧符号，已通过当前源重链 DI 库并重建 extension 修复。
R11-B8 仍有 APPEND_DELTA caller、15 个 maintained caller group、legacy zero-use、
no-Python 与 T016/T017 资格缺口，R11-B9 未开始。详见
[R11-B8-G4 evidence](evidence/r11-b8-g4-native-checkpoint-handle-20260910.md)。

2026-09-10 R11-B8-G3 native dynamic conversation binding / **CLOSED_FOR_VALIDATION**（仅限
首轮 `FULL_CONTEXT` placement metadata ownership）：C++ coordinator 允许空 role/provider
map 的首轮 continuation，sealed placement 返回后原子绑定 canonical digest 与角色集合；
fresh unit `Spec182*` 257 cases 与 native Core/Provider integration 10 cases 重建并重跑
通过。详见 [R11-B8-G3 evidence](evidence/r11-b8-g3-native-dynamic-conversation-binding-20260910.md)。
R11-B8 仍有 15 个 maintained caller group、Python conversation adapter、legacy zero-use、
no-Python 与 T016/T017 资格缺口，R11-B9 未开始。
