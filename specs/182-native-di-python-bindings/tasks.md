...
**Revision**: 39 | **Status**: DRAFT / T001 DONE
...
| [T013-E Native Conversation Owner Configuration](contracts/execution-units.md#t013-e-native-conversation-owner-configuration) | PARTIAL | T011-C; T012-B | R5-B9 正在把 operator-pinned journal/key owner 在 C++ 中注入 `NativeInferenceClient`；配置 schema、opaque binding、owner-only key/path 校验及 focused selector 尚待完成，真实 Provider/cross-process 两轮仍属 T016 | 2026-09-08 |
...
| [T013-F Maintained YOLO Native Payload Route](contracts/execution-units.md#t013-f-maintained-yolo-native-payload-route-r5-b10) | PARTIAL | T013-A; T008-A; T010-B | [R5-B10 evidence](evidence/r5-b10-yolo-native-payload-route-20260908.md)：显式 native config、YOLO package/model identity、inline tensor payload、task/options 与 fail-closed shutdown branch 已写入；`py_compile`、5 个 legacy/source checks、`git diff --check` PASS；Spec180 lifecycle、真实 Core/Provider 与 numeric parity 未验收 | 2026-09-08 |
...
| [T014-A Isolation Collector Semantics](contracts/execution-units.md#t014-a-isolation-collector-semantics) | PARTIAL | T013-B | [R6-B1 evidence](evidence/r6-b1-collector-verdict-20260908.md)：完整 trace 的 policy violation 保持 `FAIL`，trace integrity、超时和证据缺口返回 `UNQUALIFIED`；12 个 Python cases、`py_compile`、`git diff --check` PASS；真实 namespace/child/endpoint observation 仍留 T016 | 2026-09-08 |
| [T014-B Qualification Harness Registration](contracts/execution-units.md#t014-b-qualification-harness-registration) | PARTIAL | T014-A | [R6-B2 evidence](evidence/r6-b2-harness-registration-20260908.md)：I01--I08 与 PO-001--PO-014 已注册到冻结 case manifest，campaign owner 校验 canonical runner、T016 owner、deadline/cleanup limits，并输出 fresh-run `UNQUALIFIED` 记录；真实 namespace/网络执行仍留 T016 | 2026-09-08 |
...
| [T016-A Local Qualification](contracts/execution-units.md#t016-a-local-qualification) | PARTIAL | T015-A | [R6-B4 preflight evidence](evidence/r6-b4-t016-preflight-20260908.md)：工具存在但 `/run/nfd/nfd.sock` 缺失、无 MiniNDN node/netns context；I01 campaign 在启动前返回 exit 2 `UNQUALIFIED`，未产生协议结果；完整 unit/integration/MiniNDN/no-Python matrix 仍未运行 | 2026-09-08 |
...
| R4-B6 | production entry/callers: covered (`NativeInferenceClient::request`, `NativeConversationCoordinator`, `NdnsfIntegrationEnvironment`, real `ServiceProvider` handler); implementation/wire: covered (V2 structured names, SVS session/seq freshness, receipt/control/commit ACK, terminal stream); test/harness/oracle: covered for positive two-turn path and bounded single-provider CC-4c negative, alternate-provider recovery gap; build/source closure: covered (`integration-tests` and unit target, system-first `-j4`); migration/evidence: gap (T012--T016) | static review completed; findings were structured request identity loss, freshness replay, post-End retry, and one-worker control-plane waits, all repaired in this batch; a legacy event-wire compatibility regression was also repaired before final gates | R7-B1 initial success expectation exposed the recovery ACK/no-admitted-provider boundary; final build had no compile/link failure | final R4-B6 selector passed two real Provider turns; R7-B1 negative selector passed and preserved no-checkpoint boundary | R4-B6 `integration-tests -j4`, 16.65s elapsed, exit 0; R7-B1 `integration-tests -j2`, Waf 36.791s, 118/118, exit 0; final negative/positive selectors exit 0; stream suite 17/17 and name suite 16/16 PASS | FOCUSED_BEHAVIOR_PASS; PARTIAL; not QUALIFICATION_PASS | [R4-B6 evidence](evidence/r4-b6-provider-conversation-20260908.md), [R7-B1 evidence](evidence/r7-b1-r4b6-replacement-20260908.md); remaining: alternate-provider recovery, T012/T013, T014/T015/T016 |
| R7-B1 | production entry/callers: covered (`NativeInferenceClient::request`, real `ServiceProvider` handler and `NativeConversationCoordinator` lookup); implementation/wire: covered (ProviderFailure stream injection, replacement ACK planning, failed-provider exclusion, no-checkpoint fence); test/harness/oracle: bounded single-provider negative covered, alternate-provider recovery gap; build/source closure: changed integration source registered by existing Waf target; migration/evidence: T011-C cross-process and T016 remain open | static review checked failure injection, status/result ordering, no record lookup before failed status, test registration and positive helper preservation; no actionable finding | first run expected success and exposed the real `ACK_CLOSED`/`DI_NATIVE_NO_ADMITTED_PROVIDER` boundary; raw exit201 retained and test semantics corrected | final negative selector passed 1 case with `NATIVE_REQUEST_STAGE_FAILED`, `ACK_CLOSED`, one collaboration call and no checkpoint; positive real two-turn selector passed 1 case | system-first `PATH=/usr/bin:/bin:/usr/sbin:/sbin WAFLOCK=.lock-waf ./waf -o build-nac182 build --targets=integration-tests -j2`, Waf 36.791s, 118/118, exit 0; final negative/positive selectors exit 0; `vmstat` recorded pre-existing swap and no sustained `si/so` | STATIC_PASS; BUILD_PASS; FOCUSED_BEHAVIOR_PASS; PARTIAL; not QUALIFICATION_PASS | [R7-B1 evidence](evidence/r7-b1-r4b6-replacement-20260908.md); single-provider negative closed, alternate-provider recovery/T011-C/T016 remain |
| R7-B2 | production entry/callers: same public `NativeInferenceClient`/real Provider conversation entry, expanded to two Provider identities; implementation/wire: replacement attempt receives a new role-map digest while parent CAS still binds the old checkpoint; test/harness/oracle: positive alternate-provider replacement plus old-map/prefix/no-checkpoint negatives; build/source closure: coordinator/client/integration targets registered by existing Waf globs; migration/evidence: cross-process and T016 remain open | READY_FOR_IMPLEMENTATION; static gate must cover coordinator pending ownership, planner projection binding, receipt/control ACK map identity and ABI consumers | no implementation run yet; R7-B1 established the first boundary and is preserved | intended exit: failed Provider is excluded, alternate Provider completes one replacement, successor checkpoint stores the new role map and accepted prefix, stale/duplicate callbacks cannot commit | planned shared integration/unit build with system-first `-j4` unless swap pressure requires `-j2`; named selectors to be recorded in evidence | READY; not QUALIFICATION_PASS | next: register implementation evidence and run focused coordinator plus two-provider integration selectors |
...
| R5-B10 | production entry/callers: `examples/python/NDNSF-DistributedInference/yolo_2x2/user.py` explicit `--native-requester-config` branch; implementation/wire: `APPClient.configure_native_requester_from_config` + `request_native_payload` with inline native tensor bytes and native task/options; test/harness/oracle: source route checks, no planner fallback in the branch, payload/result boundary and shutdown ownership; build/source closure: Python-only `py_compile` (native closure unchanged from R5-B9); migration/evidence: real Core/Provider request, numeric parity, default route retirement and T016 remain open | pending final static review; branch must be ordered before ACK-driven Python planner and must fail closed on configuration/constructor errors | no network harness in this batch; all real requester/provider and numeric evidence stays deferred | route/source tests and `py_compile` after implementation; no C++ rebuild unless native headers change | Python-only batch; system-first `-j4` not applicable; native extension identity inherited from R5-B9 | `PARTIAL`; not QUALIFICATION_PASS until real request and caller migration are observed | evidence to be written at `evidence/r5-b10-yolo-native-payload-route-20260908.md`; next T013-B legacy retirement review and T014 collector/harness closure |
...
2026-09-08 R7-B2 alternate-provider replacement / **READY**：R7-B1 的真实单 Provider 负例
已经确定失败 Provider 会从恢复 ACK 中排除；进一步静态检查发现 replacement turn 尚无独立
的当前 `planRoleMapDigest`，切换到备用 Provider 会把旧父 checkpoint map 错当成新尝试 map。
本批登记修复 coordinator pending turn、planner projection/receipt binding 及双 Provider
integration selector；父 CAS 仍绑定旧 checkpoint，成功 successor 必须保存新 map 与已接受
prefix。尚未编码或运行，R7-B1 原始失败与负例证据保持不变。
