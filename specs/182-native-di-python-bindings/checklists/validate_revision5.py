#!/usr/bin/env python3
"""Read-only validation for the Spec182 revision 5 document contract."""
...
for task in ids:
    ...
    if block:
        for key in ["SymbolContracts", "Documentation", "Usage", "StaticReview", "PostTestReview"]:
...
# This validates the written gate, never an actual product code-review verdict.
review_text = texts["contracts/pre-test-static-review.md"]
for i in range(1, 14):
    require(f"## SR-{i:03d} " in review_text, f"missing static-review section SR-{i:03d}")
for field in ["ReviewId", "SourceIdentity", "DesignIdentity", "TestSubject",
              "ReviewedPathsAndSymbols", "DesignCodeMapping", "PathWalkthroughs",
              "Findings", "AllowedTestScope", "Verdict", "ExpectedDefect",
              "DesignDeviations", "ArchitectureInvariantViolations", "ExpectedDiffAssessment",
              "RiskPredictions", "RiskCountJustification", "KnownCorrectnessRisks",
              "NegativePathsChecked", "OwnershipLifetimeIssues", "CriticalDefectTests",
              "KnownUntestedBehavior", "PreTestDecision", "PreReviewId", "TestEvidence",
              "RiskOutcomes", "GreenButWrongScenarios", "UnverifiedObligations",
              "FinalDiffReview", "PostTestDecision",
              "InvalidationAndNextAction", "RuntimeTests"]:
    require(re.search(r"^" + field + ":", review_text, re.M) is not None,
            f"missing static-review evidence field: {field}")
for state in ["NOT_REVIEWED", "BLOCK", "PASS", "STALE"]:
    require(state in review_text, f"missing review state: {state}")
for name in ["spec.md", "plan.md", "tasks.md", "contracts/proof-design.md"]:
    require("S0" in texts[name], f"static gate absent from {name}")
require("unit→integration→MiniNDN" in texts["tasks.md"], "runtime stage order missing")
require("PO-015" in texts["traceability.md"], "static review proof untraced")
require("product STATIC_REVIEW NOT_RUN" in review_text,
        "must not claim product static-review PASS before implementation")
for marker in ["Design Conformance Review", "Correctness Review", "Test Adequacy Review",
               "Static review PASS != Behavior PASS"]:
    require(marker in review_text, f"missing adversarial rule: {marker}")
require("PO-016" in texts["traceability.md"], "post-test proof untraced")
require("同一问题两轮" in review_text, "bounded review rule absent")
require("S1" in texts["tasks.md"] and "S1" in texts["plan.md"], "post-test gate absent")
links = 0
...
report = {"schema": "spec182-revision5-document-validation-v1",
          ...
          "errors": errors, "runtime_tests": "NOT_RUN", "product_static_review": "NOT_RUN", "product_post_test_review": "NOT_RUN"}
