...
- **Owned ONNX graph inspection**: 在 A/onnx/NativeOnnxRecipeAssembler.{hpp,cpp} 新增 inspectNativeOnnxPlanningGraph(source, expectedModel, control) → NativeOnnxGraphInspection，复用 ownedSourceModel、原图 identity 与官方 InferShapes；不得重新实现 ONNX parser。按维护 analyze_onnx_graph/to_model_graph_snapshot 生成 adapter-bound graph digest、实际 node names、ONNX node indices 和 metadata JSON，检查 expectedModel.graphDigest。推断失败使用未修改原图，未知形状/类型保留，预算溢出明确拒绝。此入口不负责获取/认证网络来源；当前 ONNX node 一一映射不替代 Qwen semantic layer 的多节点映射。验证包含实际 Python ONNX fixtures、branch/dynamic/initializer/unnamed、adapter/source 篡改、deadline/source limit；同步 U/di-native-planning.t.cpp、fixture author/JSON 与 case-manifest。
...
- **B-G1-YOLO-SEMANTIC API**: `NativeYoloCatalogComponent` 包含既有 component identity 和完整 `semanticPartitionJson`；`NativeYoloComponentSplit::fromOnnxCatalog(model, source, control, candidates, postprocessingJson)` 直接调用实际 ONNX inspection，校验语义 partition 后建立 immutable splitter。工厂保留完整 model identity 与实际规划图；enumerate 使用保留图，不信任调用方重建的图事实，并拒绝另一 model/graph identity。原 typed constructor 接收已解析 planning IDs，不代表 catalog 校验。factory 不认证网络来源、不合成注册摘要，调用方须从既有 catalog owner 提供配置。`atomic-v1` 按维护算法将全图分配给首角色；非 atomic 完整检查名称、tensor interfaces、依赖、安全切点及角色输入输出后才产生候选。新增 class 状态需要 fresh ABI tree，批末统一构建。
...
- **Parent**: T012-A; **Depends**: T011-C; **Reviewer**: native DTO binding and ABI review
- **Outcome**: 让 Python requester 能构造并传递 native generation、Core stream、ControllerVersion 与 conversation continuation 值对象；协议校验、wire 编解码和事件状态仍由 C++ owner 执行。
- **Read**: `NativeRequestOptions.hpp`; `NativeExecutionPlanJson.hpp`; `NativeConversationCoordinator.hpp`; `InvocationStream.hpp`; T012-A binding/lifetime contract。
- **Write**: `pythonWrapper/src/ndnsf/di_bindings.cpp`; `tests/python/test_spec182_native_bindings.py`; 本执行卡 evidence。
- **Steps**: 注册带 `Native` 前缀的 enum/class；用安全的 wire-bytes property 转换 `eventKeyGrant`；暴露 `NativeRequestOptions.generation/stream/conversation`；不暴露 native worker callback，不复制 planner 或 protocol state。
...
- **Outcome**: 将已有 C++ `NativeInferenceHandle::observe` 以受控的 Python facade 暴露，并让 `APPClient.request_native_payload` 可选接收 observer；已接受的 stream token event 与 terminal event 均可被观察，事件顺序、replay、终态、异常隔离和 handle 寿命继续由 C++ owner 负责。
...
- **Steps**: 仅绑定非 secret 的 request id、bytes payload 和 terminal 标志；在 C++ stream acceptance 后发布有界 token event；在提交 native request 前校验 observer callable，提交后立即挂载 observer；不把 Python callback 变成 planner、strategy 或协议状态 owner，不改变无 observer 的返回和失败语义。
