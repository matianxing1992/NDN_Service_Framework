...
**Profile**: bounded-executor / Codex Spark
**Status**: DISPATCH_DESIGNED / product NOT_STARTED / Spark trial NOT_RUN
...
T001 的设计卡由设计者收口；Spark 执行已冻结的实现卡。Reviewer 指明需要更强模型复核的边界，
不是自动调用模型或派生 agent 的授权。T015 的整体语义审查由具备跨层审查能力的执行者负责。
T016 可由 Spark 按冻结命令操作，但不能自行改变 oracle、隔离边界或解释未分类失败为 PASS。
...
`"$UNIT_BIN" --list_content` 核对非空测试集合后才运行；冻结前不能把猜测命令交给 Spark 执行。
...
卡结果统一写 tasks 的 Spark Checkpoint：`Card | Status | Baseline | Evidence | Remaining`。
未完成卡保持 PLANNED/IN_PROGRESS/BLOCK，不能因本文件文档检查通过变成 COMPLETE。
上层任务只有全部卡和原父任务 LocalChecks 满足后勾选；产品最终验收仍由 T016 关闭。

初次 Spark 试用在 T001 release 后按依赖推进；选择一个冻结值行为（如 T003-A）、
一个含资源所有权行为（T007-A）及后续局部修复（T011-A）记录实际首次验收、审查修正和越界修改。
不是为了测试 Spark 而绕过依赖提前实施。无需用速度或额度消耗作为产品验收门。

可直接交给执行者的入口：

> 使用 speckit-implement 的 bounded-executor 模式执行 Spec182。先读 tasks 当前 checkpoint 和
> contracts/spark-execution.md 的分派规则；只选择一张依赖已满足的实现卡。
> 若 T001 尚未 release，报告具体缺口并交回设计者。按 Read/Write/Steps/Verify 完成该卡，
> 保存实际源码身份和检查结果，依仓库规则本地 checkpoint；不把父任务、T016 或整项 Spec 提前标完成。
