...
| C18 / CD-013 NativeRequestPreparation | 原生 task/graph/catalog/input/artifact I/O owner | registry、authenticated catalog/Repo/publication/role ports、request control | prepareInput/inspectModel/prepareRoles/ensureArtifacts；保留真实 source/manifest 和完整请求模型；已发布 immutable record 只按 TTL 过期 |
