...
`rc=127`，候选库重试又暴露相对 artifact cwd 的 `rc=2`。在 `examples/wscript` 增加
`$ORIGIN/..` RUNPATH（源码 checkpoint `085359eb`）后，从 bundle 根目录且不设置
`LD_LIBRARY_PATH` 重试，四个 ONNX role 全部执行，发布 4 个 dependency objects、产生
...
三次失败边界均在相应重试前写入 failure-log。该批不代表 Provider `--serve`、requester/
Provider transport、maintained caller/no-Python 或 T016 qualification；详见 [R10-B53 evidence](evidence/r10-b53-plan-onnx-smoke-20260909.md)。
...
3. G2 / T010--012（T005 authority prerequisite）：按 N1--N3 分离 authority，先完成原生跨进程 unary，再在同链验证 stream、continuation、recovery、replacement、cleanup；每批 C++ unit/integration/process 先于对应 Python 检查。
4. G3 / T013--014：N3 出口通过后迁移原 16 个调用方及薄绑定，清退旧默认运行时，再完成 no-Python/依赖闭包工具与验收准备；不再把首次真实跨进程请求推迟到此后。
...
新 config selector、完整 `Spec182*` 252 unit cases、9 个 native Core/Provider integration
cases、requester help、导出符号检查均通过；Python 72 项只验证薄 wrapper/contract 仍指向同一
native 实体，不替代 native 行为验收。首次扩展编译的 `ndn::Name`→`std::string` 接线错误已
记录并修复；首次 integration 构建选到没有该 target 的旧 Waf cache，显式指定 output 后重建
通过并已记录为工具边界。该批不推进父任务；authority 分离、
