...
| T006.b | T006 | 对 C++ binaries、`_ndnsf.so`、入口 `--help` 执行 import、`readelf -d`、`ldd -r`、RPATH/RUNPATH、SONAME closure | VERIFIED | `evidence/native-build-r64-20260915.md` records the current-source r64 `197/197` native target build, `SPEC180_NATIVE_IDENTITY_OK`, Python import/loader verification, provider/requester/authority help, assembly-worker mode status and the complete host `readelf`/`ldd -r` matrix; the expected raw Python C API symbols are interpreter-provided | repeat the complete loader matrix after exact SIF composition; host-only RUNPATH cannot be promoted | binary/loader/base change重做 |
