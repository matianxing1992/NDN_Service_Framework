...
| T006.a | T006 | 按锁定 builder 和 `-j4` 上限完成依赖、Core/Repo/DI/native extension 的完整 unit/integration 验证 | BLOCKED_AFTER_BOUNDARY | `evidence/native-abi-closure.md`; focused Python/Tiger tests pass, Waf build stops at NAC-ABE/ONNX dependency | matching ONNX 1.17 full-protobuf prefix missing | source/toolchain/ABI变更重建 |
| T006.b | T006 | 对 C++ binaries、`_ndnsf.so`、入口 `--help` 执行 import、`readelf -d`、`ldd -r`、RPATH/RUNPATH、SONAME closure | BLOCKED_AFTER_BOUNDARY | `evidence/native-abi-closure.md`; existing artifacts report unresolved symbols | clean rebuild and packaged loader closure required | binary/loader/base change重做 |
...
| T009.a | T009 | 进行 Tiger compute、Slurm、Apptainer、capacity、GPU UUID、NFD route、project storage 和 staged candidate preflight | WAITING_EXTERNAL_INPUT | local inventory has no `sbatch` or `nvidia-smi`; no Tiger endpoint/account | run on Tiger login/compute host with declared allocation | host/GPU/Apptainer变化重做 |
