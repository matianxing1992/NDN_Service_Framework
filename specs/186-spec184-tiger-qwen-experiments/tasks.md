...
| T006.c | T006 | 构建或复用稳定 base SIF，并在匹配 SDK/container 中构建只读 application bundle；记录 base/app 分层 hash | BLOCKED_AFTER_BOUNDARY | `evidence/application-bundle-r6-20260913.md` records the current-source read-only app bundle `04c2dd64b4f070cbd909a87f75a0372a0e3d4dae45c7e712641369cf76531d73`; r38 compute 1.5.3 built/imported final SIF `sha256:e890b5de2ffffb9e795cd9b2a7cce28b58cef4b7adcb9fe13844bcfa47c168dd` (4,167,950,336 bytes) from intermediate `sha256:328f2ffd2b0d8da1c19e4114c196bea255f7ee1f626f25f891398a046e012a27`, but no local-first promotion or exact Spec186 composition receipt exists | retain r38 SIF as an external fallback candidate only; complete local 1.5.3 build/smoke, immutable upload and same-SHA receive verification, or record a new compute-build exception | base ABI重建；app-only仅重建 app |
| T006.d | T006 | 按本地优先规则完成 SIF exact-SHA promotion：本地 1.5.3 build/import/CPU smoke → 封存 → 上传 → Tiger compute 1.5.3 接收核对；例外必须绑定首失败和新 candidate | BLOCKED_AFTER_BOUNDARY | `evidence/tiger-r38-build-boundary-20260914.md` records r38 compute build/import only; it has no local SIF smoke, immutable upload and same-SHA receive receipt | finish the local-first promotion chain; do not run MiniNDN/Tiger qualification on a SIF whose promotion identity is unverified | SIF bytes/definition/upload target/runtime change重做 |
...
| T009.a | T009 | 进行 Tiger compute、Slurm、Apptainer、capacity、GPU UUID、NFD route、project storage 和 staged candidate preflight | VERIFIED | `evidence/tiger-preflight-20260912.md`, `evidence/runtime-version-policy-20260913.md`, `evidence/host-m01-r55-g3-20260913.md` and `evidence/tiger-r38-build-boundary-20260914.md`; compute `/home/tma1/.local/bin/apptainer-1.5.3` 1.5.3、`itiger02` RTX 6000 Ada/GPU UUID、Slurm allocation、final SIF build and runtime import pass; login `/usr/bin/apptainer` 1.3.4 remains metadata-only | local-first promotion, staged case/catalogue identity and NFD route still need a run-bound receipt; r38 stopped before MiniNDN with `CANONICAL_CATALOGUE_VERIFY_FAILED` | host/GPU/Apptainer/staging变化重做 |
| T009.b | T009 | 使用 exact base+app 在单节点完成 YOLO 1 warmup + 1 measured；模型角色 observed CUDA、Merge observed CPU | BLOCKED_AFTER_BOUNDARY | `evidence/tiger-r38-build-boundary-20260914.md` records final SIF runtime import but the runner stopped before MiniNDN startup at canonical catalogue verification; no CUDA/model-role or numerical result exists | capture the raw validator exception, repair only the input/runner boundary, then rerun a new candidate/run | SIF/app/profile/GPU变化重跑 |
...
| T013.c | T013 | 生成 Spec186 closure/handoff，明确 implemented/wired/executed/measured、未完成 external rows 和下一步 | VERIFIED | `evidence/closure-handoff-20260914.md` records the r38 build/import boundary, local-first promotion correction, first runtime failure and ordered next gates; historical `closure-handoff-20260913.md` remains immutable | 任一必需 gate未完成则保持 IN_PROGRESS | final candidate变化重新闭合 |
