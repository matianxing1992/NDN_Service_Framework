...
| T006.b | T006 | 对 C++ binaries、`_ndnsf.so`、入口 `--help` 执行 import、`readelf -d`、`ldd -r`、RPATH/RUNPATH、SONAME closure | VERIFIED | `evidence/native-build-r61-20260915.md` records fresh r61 `197/197` native target build, `SPEC180_NATIVE_IDENTITY_OK`, Python import/loader verification, provider/requester/authority help and the complete host `readelf`/`ldd -r` matrix; the expected raw Python C API symbols are interpreter-provided | repeat the complete loader matrix after exact SIF composition; host-only RUNPATH cannot be promoted | binary/loader/base change重做 |
| T006.c | T006 | 构建或复用稳定 base SIF，并在匹配 SDK/container 中构建只读 application bundle；记录 base/app 分层 hash | BLOCKED_AFTER_BOUNDARY | `evidence/native-build-r61-20260915.md` records a current-source host bundle `4a13a2fbc40d5825c94ecef71bd4522321663567da2fc0c1cd339d8f9851de3c` whose tree digest is self-consistent, but its ELF RUNPATH contains host-only prefixes; no matching-SDK container app closure or local-first exact composition exists. The r6 and r38 receipts remain historical. | rebuild app inside the matching container, then complete local 1.5.3 SIF/import/CPU smoke, immutable upload and same-SHA receive verification | base ABI重建；app-only仅重建 app |
