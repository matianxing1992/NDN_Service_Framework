...
| T006.c | T006 | 构建或复用稳定 base SIF，并在匹配 SDK/container 中构建只读 application bundle；记录 base/app 分层 hash | BLOCKED_AFTER_BOUNDARY | `evidence/native-build-r61-20260915.md` records host bundle `4a13a2fbc40d5825c94ecef71bd4522321663567da2fc0c1cd339d8f9851de3c` from source `81096f3e`; its tree digest is self-consistent, but its ELF RUNPATH contains host-only prefixes and the later r63 source seal requires a rebuild. No matching-SDK container app closure or local-first exact composition exists. The r6 and r38 receipts remain historical. | rebuild app inside the matching container against the r63 seal, then complete local 1.5.3 SIF/import/CPU smoke, immutable upload and same-SHA receive verification | base ABI重建；app-only仅重建 app |
