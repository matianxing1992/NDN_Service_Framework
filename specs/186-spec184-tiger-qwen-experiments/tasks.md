...
| T009.a | T009 | 进行 Tiger compute、Slurm、Apptainer、capacity、GPU UUID、NFD route、project storage 和 staged candidate preflight | VERIFIED | `evidence/tiger-preflight-20260912.md`, `evidence/local-apptainer-20260912.md`, `evidence/application-bundle-r6-20260913.md` and `evidence/runtime-version-policy-20260913.md`; local `/usr/local/bin/apptainer` and compute `/usr/bin/apptainer` both pin 1.5.3, `itiger05` GPU/storage/Slurm preflight passes, and the current-source read-only r6 app bundle is staged with digest `04c2dd64b4f070cbd909a87f75a0372a0e3d4dae45c7e712641369cf76531d73`; the login node is control-plane metadata only and is not used for SIF | exact source-sealed SIF, NFD route and runtime composition remain; old SIF probe failed on missing `libboost_system.so.1.71.0` | host/GPU/Apptainer/staging变化重做 |
...
| T013.c | T013 | 生成 Spec186 closure/handoff，明确 implemented/wired/executed/measured、未完成 external rows 和下一步 | VERIFIED | `evidence/closure-handoff-20260913.md` records the current r6 `IN_PROGRESS` boundary, route/base-SIF/model recovery inputs and ordered next gates | 任一必需 gate未完成则保持 IN_PROGRESS | final candidate变化重新闭合 |
