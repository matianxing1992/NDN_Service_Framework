...
| T005.b | T005 | 修复 T005 controlling gaps 并运行 focused regression，重新审计直到 `PASS` | VERIFIED | checkpoints 7–8 record provider help repair, fresh native closure, collector identity repair and focused regression; 74 TigerCluster tests pass | qualification must not be promoted without T006–T012 receipts | 任一行为变更重新执行 |
...
| T006.b | T006 | 对 C++ binaries、`_ndnsf.so`、入口 `--help` 执行 import、`readelf -d`、`ldd -r`、RPATH/RUNPATH、SONAME closure | VERIFIED | `evidence/native-abi-closure.md` checkpoint 12 plus `evidence/t006-nac-abe-abi-20260913.md`; current `build-spec186-r5` identity verify, provider/requester/authority help, canonical import, readelf, RUNPATH, native ldd closure and binding lifecycle pass after explicit NAC-ABE prefix repair; all profile/runtime paths now pin Apptainer 1.5.3 in `evidence/runtime-version-policy-20260913.md` | repeat after exact SIF composition; extension raw Python C API symbols are interpreter-provided | binary/loader/base change重做 |
| T006.c | T006 | 构建或复用稳定 base SIF，并在匹配 SDK/container 中构建只读 application bundle；记录 base/app 分层 hash | BLOCKED_AFTER_BOUNDARY | `evidence/application-bundle-r5-20260913.md` records the local and staged read-only app bundle `687610de859155449c51ec2ba4bb7b57c77614cbf0a53f106bb65152f8c07129`; clean source handoff at `4751148375dad9149c7c185c9381d5734c733e13` verifies with source seal `sha256:597c44a97b34655dfb67b9fc3bff3693b844f5cc1f10624870554bdee8e658e2`; no exact Spec186 source-sealed base SIF or current host-gate/composition receipt | obtain a current host-gate manifest, build/verify the 1.5.3 source-sealed base SIF, and verify the exact base+app composition | base ABI重建；app-only仅重建 app |
...
| T009.a | T009 | 进行 Tiger compute、Slurm、Apptainer、capacity、GPU UUID、NFD route、project storage 和 staged candidate preflight | VERIFIED | `evidence/tiger-preflight-20260912.md`, `evidence/local-apptainer-20260912.md`, `evidence/application-bundle-r5-20260913.md` and `evidence/runtime-version-policy-20260913.md`; local `/usr/local/bin/apptainer` and compute `/usr/bin/apptainer` both pin 1.5.3, `itiger05` GPU/storage/Slurm preflight passes, and the read-only r5 app bundle is staged with digest `687610de859155449c51ec2ba4bb7b57c77614cbf0a53f106bb65152f8c07129`; login-node 1.3.4 is not used for SIF | exact source-sealed SIF, NFD route and runtime composition remain; old SIF probe failed on missing `libboost_system.so.1.71.0` | host/GPU/Apptainer/staging变化重做 |
...
| T013.b | T013 | 把每次非平凡失败写入 `docs/failure-log.md`，完成离线 hash/oracle/reconciliation 和 evidence index | VERIFIED | `docs/failure-log.md`, `evidence/README.md`, immutable stale-hash receipt and repaired all-profile pre-dispatch digest receipts including source/app path binding | 失败被覆盖或仅存在聊天记录阻断 | 新失败/证据 parser变化重做 |
| T013.c | T013 | 生成 Spec186 closure/handoff，明确 implemented/wired/executed/measured、未完成 external rows 和下一步 | VERIFIED | `evidence/closure-handoff-20260913.md` records the current r5 `IN_PROGRESS` boundary, route/base-SIF/model recovery inputs and ordered next gates | 任一必需 gate未完成则保持 IN_PROGRESS | final candidate变化重新闭合 |
