...
| T006.b | T006 | 对 C++ binaries、`_ndnsf.so`、入口 `--help` 执行 import、`readelf -d`、`ldd -r`、RPATH/RUNPATH、SONAME closure | VERIFIED | `evidence/native-abi-closure.md` checkpoint 12 plus `evidence/t006-nac-abe-abi-20260913.md`; current `build-spec186-r5` identity verify, provider/requester/authority help, canonical import, readelf, RUNPATH, native ldd closure and binding lifecycle pass after explicit NAC-ABE prefix repair | repeat after exact SIF composition; extension raw Python C API symbols are interpreter-provided | binary/loader/base change重做 |
...
| T009.a | T009 | 进行 Tiger compute、Slurm、Apptainer、capacity、GPU UUID、NFD route、project storage 和 staged candidate preflight | VERIFIED | `evidence/tiger-preflight-20260912.md`, `evidence/local-apptainer-20260912.md` and `evidence/application-bundle-r5-20260913.md`; local runtime is only `/usr/local/bin/apptainer` 1.5.3, compute `apptainer --version` is 1.5.3-1.el9, `itiger05` GPU/storage/Slurm preflight passes, and the read-only r5 app bundle is staged with digest `687610de859155449c51ec2ba4bb7b57c77614cbf0a53f106bb65152f8c07129`; login-node 1.3.4 is not used for SIF | exact source-sealed SIF, NFD route and runtime composition remain; old SIF probe failed on missing `libboost_system.so.1.71.0` | host/GPU/Apptainer/staging变化重做 |
