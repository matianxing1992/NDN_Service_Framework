...
| T006.b | T006 | 对 C++ binaries、`_ndnsf.so`、入口 `--help` 执行 import、`readelf -d`、`ldd -r`、RPATH/RUNPATH、SONAME closure | BLOCKED_AFTER_BOUNDARY | `evidence/native-abi-closure.md`; existing artifacts and forced temporary NAC-ABE load report unresolved symbols and header/library export mismatch | same-revision dependency rebuild and packaged loader closure required | binary/loader/base change重做 |
