...
**Status**: IN_PROGRESS. 本任务表只描述 Spec186 的新候选；Spec183 的 v56 PASS、575 提交前的旧 smoke 和任何 Spec185 文件都不能直接关闭本表任务。当前本机与 Tiger compute 的 Apptainer 运行边界已统一为 1.5.3；资格实验仍受 exact base SIF、仓库路由、模型和外部运行输入阻断。
...
| T005.b | T005 | 修复 T005 controlling gaps 并运行 focused regression，重新审计直到 `PASS` | VERIFIED | checkpoints 7–8, 11, 15–20 and `evidence/static-experiment-cycle-20260913.md` record provider help repair, native closure, collector identity repair, streamed collaboration repair, strict nested profile/manifest/terminal receipt validation, resource/run-root boundaries, harness format checks, terminal GPU-role policy and scheduler identity binding; 44 focused Spec186 tests pass | qualification must not be promoted without T006–T012 receipts | 任一行为变更重新执行 |
| T006.pre | T006 | 在原生编译前运行 `preflight-development-sif.py`，交叉核对 rendered definition、`workspace.tar` 的 Waf 输入、NumPy wheel-private DSO/RPATH 和 base-SIF 导入 | VERIFIED | `Experiments/TigerCluster/adapters/slurm-apptainer/scripts/preflight-development-sif.py` passes against the current rendered r6 definition, sealed handoff and repaired base SIF; `build-local-sif.sh` invokes it before `LOCAL_SIF_BUILD_START` | this gate prevents known input/closure failures from consuming a full compile; it does not qualify MiniNDN/Tiger | definition, handoff or base-SIF change重做 |
...
| T006.b | T006 | 对 C++ binaries、`_ndnsf.so`、入口 `--help` 执行 import、`readelf -d`、`ldd -r`、RPATH/RUNPATH、SONAME closure | VERIFIED | historical closure checkpoint 12 plus current r6 native identity verify (`SPEC180_NATIVE_IDENTITY_OK`), provider/requester/authority help, canonical import and native ldd closure after the streamed collaboration repair; all profile/runtime paths now pin Apptainer 1.5.3 in `evidence/runtime-version-policy-20260913.md` and the r6 bundle receipt | repeat the complete loader matrix after exact SIF composition; extension raw Python C API symbols are interpreter-provided | binary/loader/base change重做 |
| T006.c | T006 | 构建或复用稳定 base SIF，并在匹配 SDK/container 中构建只读 application bundle；记录 base/app 分层 hash | BLOCKED_AFTER_BOUNDARY | `evidence/application-bundle-r6-20260913.md` records the current-source local and staged read-only app bundle `04c2dd64b4f070cbd909a87f75a0372a0e3d4dae45c7e712641369cf76531d73`; the app bundle source seal is `sha256:f4676a0f937c903caebc8374893171890d0d6d0639be42a3ced1b101d7512a00`, while the corrected source-sealed SIF handoff (including `pc.in`) is `sha256:f221cd8bac8318d35f7ee8da67765cf05e60b5dd79513b25ecb74b00e3bca833`; source commit `6d143d3f0f7a7c627af2c1ef6810d79c0738b52d` and the current host M01 G3 manifest are fixed, but no exact Spec186 source-sealed base SIF or composition receipt exists | build/verify the 1.5.3 source-sealed base SIF and verify the exact base+app composition; the host G3 receipt is the prerequisite, not the SIF result | base ABI重建；app-only仅重建 app |
| T007.a | T007 | 使用新 candidate 运行 YOLO MiniNDN Y-A/Y-B；保留真实 ACK/Selection、依赖 Data、terminal response 和独立 numerical oracle | WAITING_EXTERNAL_INPUT | `evidence/direct-target-audit-20260913.md`; staged Y-A/Y-B package, trust/key maps, topology/config pass `validate_inputs` and process-vector construction; no MiniNDN startup yet | exact source-sealed base SIF and final native closure required | candidate/model/harness变化重跑 |
| T007.b | T007 | 运行 YOLO MiniNDN Y-N 注册拒绝和 dependency-negative；记录唯一 edge、native failure、无响应/重选和 cleanup | WAITING_EXTERNAL_INPUT | `evidence/direct-target-audit-20260913.md`; staged Y-N case bundle is sealed, but live negative path is not started | exact source-sealed base SIF and final native closure required | negative contract/collector变化重跑 |
...
| T009.a | T009 | 进行 Tiger compute、Slurm、Apptainer、capacity、GPU UUID、NFD route、project storage 和 staged candidate preflight | VERIFIED | `evidence/tiger-preflight-20260912.md`, `evidence/local-apptainer-20260912.md`, `evidence/application-bundle-r6-20260913.md`, `evidence/runtime-version-policy-20260913.md` and current host route/manifest receipt `evidence/host-m01-r55-g3-20260913.md`; local `/usr/local/bin/apptainer` 1.5.3 and allocated compute `/home/tma1/.local/bin/apptainer-1.5.3` 1.5.3 are the only SIF runtimes, `itiger05` GPU/storage/Slurm preflight passes, the current-source read-only r6 app bundle is staged with digest `04c2dd64b4f070cbd909a87f75a0372a0e3d4dae45c7e712641369cf76531d73`, and current host M01 route/identity validation is PASS; the login node's `/usr/bin/apptainer` 1.3.4 is control-plane metadata only | exact source-sealed SIF, Tiger NFD route and runtime composition remain; old SIF probe failed on missing `libboost_system.so.1.71.0` | host/GPU/Apptainer/staging变化重做 |
...
| T013.c | T013 | 生成 Spec186 closure/handoff，明确 implemented/wired/executed/measured、未完成 external rows 和下一步 | VERIFIED | `evidence/closure-handoff-20260913.md` records the current r6 `IN_PROGRESS` boundary, validated r55 host M01 G3 receipt, route/base-SIF/model recovery inputs and ordered next gates | 任一必需 gate未完成则保持 IN_PROGRESS | final candidate变化重新闭合 |
...
- [ ] T006 [US1] Run the cheap source/definition/base-SIF preflight, then build and verify the locked native runtime and layered base SIF plus read-only application bundle, including complete unit/integration validation and `import`, `--help`, `readelf`, `ldd -r`, RPATH and SONAME closure evidence.
