...
| T006.c | T006 | 构建或复用稳定 base SIF，并在匹配 SDK/container 中构建只读 application bundle；记录 base/app 分层 hash | NOT_STARTED | V05 composition manifest | base/app mismatch、错误 mode、缺 entrypoint阻断 | base ABI重建；app-only仅重建 app |
| T007.a | T007 | 使用新 candidate 运行 YOLO MiniNDN Y-A/Y-B；保留真实 ACK/Selection、依赖 Data、terminal response 和独立 numerical oracle | NOT_STARTED | V06 `LOCAL_CPU_PASS` receipts | fixture-only、shape/oracle缺失为 FAILED/INCOMPLETE | candidate/model/harness变化重跑 |
| T007.b | T007 | 运行 YOLO MiniNDN Y-N 注册拒绝和 dependency-negative；记录唯一 edge、native failure、无响应/重选和 cleanup | NOT_STARTED | V07 `EXPECTED_REJECTION_PASS` or bounded failure | completion budget/edge identity不闭合阻断 | negative contract/collector变化重跑 |
| T007.c | T007 | 离线重算 Y-A/Y-B/Y-N receipt，确认旧 Spec183 evidence 未被引用为当前候选 PASS | NOT_STARTED | `evidence/minindn-yolo-*.md` and hash report | candidate digest混杂时阻断 | evidence parser/oracle变化重做 |
| T008.a | T008 | 核对 Qwen3-0.6B 权重、tokenizer、stage manifest、格式（ONNX 或 GGUF/Q3）、backend、容量和 digest | NOT_STARTED | `evidence/minindn-qwen06b-inventory.md` | 权重缺失或 backend不兼容写 `WAITING_EXTERNAL_INPUT` | model/tokenizer/backend变化重做 |
| T008.b | T008 | 在真实模型可用时运行 `NDNSF_DI_Qwen06B_Native_Minindn.py` 的 `check → prepare → run/local`，完成冷请求和 follow-up conversation | NOT_STARTED | V08 token IDs, terminal, checkpoint, child exit, cleanup | 仅 tiny fixture标 `SMOKE_ONLY` | model/app/native runner变化重跑 |
| T008.c | T008 | 记录 Qwen 结果边界：`LOCAL_CPU_PASS`、`SMOKE_ONLY` 或 `WAITING_EXTERNAL_INPUT`；不得关闭 Qwen3.6-27B 外部资格 | NOT_STARTED | `evidence/minindn-qwen06b-*.md` | status未与证据一致时阻断汇总 | 资产可用/格式改变重评估 |
| T009.a | T009 | 进行 Tiger compute、Slurm、Apptainer、capacity、GPU UUID、NFD route、project storage 和 staged candidate preflight | NOT_STARTED | V09 environment/transport receipt | allocation、节点或容量未知时不提交 | host/GPU/Apptainer变化重做 |
...
| T011.a | T011 | 只有 T008 的模型/backend/资源门通过时，生成 Qwen Tiger profile、stage placement、GPU/CPU policy 和 bounded budget | NOT_STARTED | conditional readiness receipt | 缺 0.6B artifact或Tiger资源则 `WAITING_EXTERNAL_INPUT` | model/resource/backend变化重做 |
...
| T011.c | T011 | 若条件不满足，生成可重放的 waiting/blocked receipt，说明缺项、查询命令和恢复入口 | NOT_STARTED | `evidence/tiger-qwen06b-*.md` | 不得以 YOLO PASS 或 tiny fixture代替 | external input到位后新 run |
...
| T013.a | T013 | 更新 `spec.md`、`plan.md`、`tasks.md`、`validation-matrix.md`、contracts、quickstart 和 `AGENTS.md` managed pointer（如扩展可用） | NOT_STARTED | document consistency check | pointer或路径不一致阻断 handoff | spec/plan/task change重跑 analyze |
| T013.b | T013 | 把每次非平凡失败写入 `docs/failure-log.md`，完成离线 hash/oracle/reconciliation 和 evidence index | NOT_STARTED | failure-log + offline report | 失败被覆盖或仅存在聊天记录阻断 | 新失败/证据 parser变化重做 |
