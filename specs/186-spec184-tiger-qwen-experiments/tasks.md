...
| T009.a | T009 | 进行 Tiger compute、Slurm、Apptainer、capacity、GPU UUID、NFD route、project storage 和 staged candidate preflight | VERIFIED | `evidence/tiger-preflight-20260912.md` and `evidence/local-apptainer-20260912.md`; local runtime is only `/usr/local/bin/apptainer` 1.5.3, compute `apptainer --version` is 1.5.3-1.el9, `itiger05` GPU/storage/Slurm preflight passes, and the 9-file app bundle is staged with digest `badf6a0afb36e43d02f7103cba36383bf8f0336e2310a0fd546c33223734734d`; login-node 1.3.4 is not used for SIF | exact source-sealed SIF, NFD route and runtime composition remain; old SIF probe failed on missing `libboost_system.so.1.71.0` | host/GPU/Apptainer/staging变化重做 |
