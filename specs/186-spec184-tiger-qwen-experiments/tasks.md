...
| T005.a | T005 | 完成生产接线审计：CodeGraph caller、effective config、backend、security grant、oracle、collector 和终态逐项映射 FR/SC | BLOCKED_AFTER_BOUNDARY | `evidence/design-code-convergence.md` checkpoint 5 (`BLOCK`) | provider `--help` contract, source-sealed base SIF and external runtime evidence must close | 受影响行为修复后重审 |
...
| T006.a | T006 | 按锁定 builder 和 `-j4` 上限完成依赖、Core/Repo/DI/native extension 的完整 unit/integration 验证 | BLOCKED_AFTER_BOUNDARY | `evidence/native-abi-closure.md` checkpoints 4–10; pinned Rust bridge cache, explicit NDN-SVS/NAC-ABE pairing, Core/DI/worker/provider and Python extension builds recorded | source-sealed base SIF, complete unit/integration campaign and provider help repair remain | source/toolchain/ABI变更重建 |
| T006.b | T006 | 对 C++ binaries、`_ndnsf.so`、入口 `--help` 执行 import、`readelf -d`、`ldd -r`、RPATH/RUNPATH、SONAME closure | BLOCKED_AFTER_BOUNDARY | `evidence/native-abi-closure.md` checkpoints 8–10; native ELF closure and canonical extension import pass, provider `--help` returns 2 and base SIF is absent | zero-status provider help plus packaged loader/base closure required | binary/loader/base change重做 |
| T006.c | T006 | 构建或复用稳定 base SIF，并在匹配 SDK/container 中构建只读 application bundle；记录 base/app 分层 hash | BLOCKED_AFTER_BOUNDARY | cached SIF hash recorded but source seal unproven; no V05 composition | exact source/dependency SIF and app bundle required | base ABI重建；app-only仅重建 app |
...
| T009.a | T009 | 进行 Tiger compute、Slurm、Apptainer、capacity、GPU UUID、NFD route、project storage 和 staged candidate preflight | VERIFIED | `evidence/tiger-preflight-20260912.md` and `evidence/local-apptainer-20260912.md`; `itiger05` allocation, RTX 6000 Ada UUID/capacity, Slurm 24.05.2, project storage, local Apptainer 1.5.3 and compute Apptainer `--version` pass; compute `apptainer version` hang retained as boundary | exact source-sealed SIF/app, NFD route and candidate staging remain | host/GPU/Apptainer变化重做 |
