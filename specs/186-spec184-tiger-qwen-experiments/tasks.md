...
| T005.a | T005 | 完成生产接线审计：CodeGraph caller、effective config、backend、security grant、oracle、collector 和终态逐项映射 FR/SC | BLOCKED_AFTER_BOUNDARY | `evidence/design-code-convergence.md` checkpoints 5–6 (`BLOCK`); provider help repair is source-compiled but not yet full-closure tested | clean candidate rebuild, source-sealed base SIF and external runtime evidence must close | 受影响行为修复后重审 |
...
| T006.a | T006 | 按锁定 builder 和 `-j4` 上限完成依赖、Core/Repo/DI/native extension 的完整 unit/integration 验证 | BLOCKED_AFTER_BOUNDARY | `evidence/native-abi-closure.md` checkpoints 4–11; pinned Rust bridge cache and explicit NDN-SVS/NAC-ABE pairing were previously recorded, but the help repair requires a fresh clean build | source-sealed base SIF, complete unit/integration campaign and fresh provider rebuild remain | source/toolchain/ABI变更重建 |
| T006.b | T006 | 对 C++ binaries、`_ndnsf.so`、入口 `--help` 执行 import、`readelf -d`、`ldd -r`、RPATH/RUNPATH、SONAME closure | BLOCKED_AFTER_BOUNDARY | `evidence/native-abi-closure.md` checkpoints 8–11; prior native closure/import is historical, current source repair has only translation-unit compile evidence | zero-status provider help plus fresh packaged loader/base closure required | binary/loader/base change重做 |
