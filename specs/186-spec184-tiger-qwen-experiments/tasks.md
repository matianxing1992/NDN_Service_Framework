...
| T009.a | T009 | 进行 Tiger compute、Slurm、Apptainer、capacity、GPU UUID、NFD route、project storage 和 staged candidate preflight | VERIFIED | `evidence/tiger-preflight-20260912.md`; `itiger05` allocation, RTX 6000 Ada UUID/capacity, Slurm 24.05.2, project storage and compute Apptainer `--version` pass; compute `apptainer version` hang retained as boundary | exact source-sealed SIF/app, NFD route and candidate staging remain | host/GPU/Apptainer变化重做 |
