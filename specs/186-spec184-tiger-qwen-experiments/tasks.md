...
| T006.pre | T006 | 先逐项比对 `successful-tiger-gpu-template.md` 的 base/compiler/Apptainer/multi-stage tuple，记录差异，再在原生编译前运行 `prepare-development-handoff.py render`、shell/Python/boundary 和 `preflight-development-sif.py`，交叉核对 rendered definition、`workspace.tar` 的 Waf 输入、builder 实际消费的源码子路径、base-owned 编译器/SDK/Rust/头文件能力、NumPy wheel-private DSO/RPATH 和 base-SIF 导入 | VERIFIED | `evidence/successful-template-comparison-20260915.md` records the exact base SHA, r51 renderer output/diff, compiler/toolchain/stage invariants and compute preflight; `evidence/static-build-preflight-20260915.md` records the discovered base-resolution ordering defect, source-consumer/capability guards, `--verify-existing` reuse path and 55 focused tests; this gate still does not qualify MiniNDN/Tiger | native build follows only after this receipt; no SIF/runtime result is implied; existing SIF may be re-verified without recompilation when candidate identity is unchanged | definition, handoff, template tuple or base-SIF change重做 |
