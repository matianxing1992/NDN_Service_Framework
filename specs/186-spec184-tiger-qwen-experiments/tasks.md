...
| T006.a | T006 | 按锁定 builder 和 `-j4` 上限完成依赖、Core/Repo/DI/native extension 的完整 unit/integration 验证 | BLOCKED_AFTER_BOUNDARY | `evidence/native-abi-closure.md`; temporary ONNX archive probe passes, Waf reconfigure reaches the absent pinned Rust tokenizer toolchain before compilation | locked Rust cargo/cargo-home and source-sealed dependency inputs missing | source/toolchain/ABI变更重建 |
