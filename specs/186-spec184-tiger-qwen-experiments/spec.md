...
- **FR-006**: C++ and native extensions MUST be built in a declared matching builder with parallelism no greater than `-j4`; `readelf`, `ldd -r`, RPATH/RUNPATH, SONAME and import/help checks MUST be recorded. The exact replay entrypoint MUST also verify that installed compiled bindings are not shadowed by same-named source wrapper directories in launcher or child `PYTHONPATH`. Before an expensive SIF compile, the rendered definition, sealed source archives and their required entry points/extraction roots, every builder-consumed source path, pinned wheels, wheel-private libraries and base-SIF Python/RPATH composition MUST pass the repository-owned Spec186 preflight. Every local or compute-node SIF invocation MUST bind an explicit Apptainer 1.5.3 executable; the Tiger login-node 1.3.4 package is metadata-only and MUST never execute a SIF.
