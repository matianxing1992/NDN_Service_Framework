...
分段测量先行，不把总时长当decode时间，不引入自适应调度/新模型/Repo重构。
...
- Target Platform: 当前Linux开发机，两个MiniNDN执行节点ucla/arizona，Qwen3-0.6B；CPU缓存兼容范围。
- Storage: 复用固定内容缓存与现有conversation journal；新日志不包含密钥/模型bytes。
...
- Performance Goals: spec SC-001–006；1000ms ACK目标、实时首token、三轮中位端到端降低至少50%。
...
G1–G6、C1–C3、D1–D4、B1–B3、M1–M3均映射[design contract](contracts/design.md)。
...
关键签名/字段/失败流程以 [CD-01–05](contracts/design.md) 为单一设计源。
...
| B190-06 / T006 | 全真实路径收敛PASS，候选错误在启动前拒绝 | T001–T005 / all focused selectors | Waf注册/安装、C++oracle、launcher preflight；CD-05 | validation / evidence/b190-06.md planned |
...
生产修改限CD-01–05明确路径。维护Design/spec-design-changes.md，实施API变化随对应task同步中文契约和双PDF。
