# Archived Quickstart Through 2026-09-02

> Historical command record only. Use `../../quickstart.md`.
...
The experiment-control contract is
[`contracts/tiger-experiment-profile-v1.md`](contracts/tiger-experiment-profile-v1.md).
Use one checked-in launcher/profile and a gate-specific run record; do not
create a second wrapper or pass ambient overrides. Only the contract's
allowlisted identities and explicitly consumed stage-device mapping may vary.

**Mandatory design-to-code gate:** before treating any unit/integration run,
MiniNDN run, SIF replay, Tiger job, benchmark, or experiment as validation,
follow this order: (1) freeze the active `spec.md`, `plan.md`, contracts,
invariants, and effective configuration; (2) trace the actual production entry
points with CodeGraph and source inspection, including runtime wiring,
security/configuration resolution, terminal ownership, cleanup, logging, and
evidence emission; (3) record and repair every design/implementation gap in
[`evidence/design-code-audit-YYYYMMDD.md`](evidence/design-code-audit-20260831.md),
repair the specification/tasks or production code as appropriate, and attach a
focused regression for each repair; (4) re-run the audit and require a fresh
`PASS`; (5) only then proceed to complete tests and qualification. Focused
red/green tests are permitted only to close a finding; formal validation is
blocked while any controlling gap remains. Any later behavior-affecting source,
dependency, launcher, configuration, artifact, topology, harness, or
evidence-schema change invalidates that PASS and starts a new audit. Follow the
canonical procedure in
[`design-code-convergence.md`](../../.specify/memory/design-code-convergence.md).
The canonical document's **Required sign-off checklist** must be fully checked
for the exact subject before its result can be called validation; a passing
command cannot waive an unchecked design-to-code item.

The Spec175 launcher applies this normal logging filter to application
processes (MiniNDN and exact-SIF/Tiger paths use the same value):

```bash
NDN_LOG='*=WARN:ndn_service_framework.TimelineTrace=WARN:ndnsf.di.RuntimeEvidence=WARN'
```

This keeps the machine-readable timing/evidence records while suppressing
routine trace/info noise. For a bounded repair diagnostic, raise only the DI
component without changing the workload:

```bash
NDN_LOG='*=WARN:ndn_service_framework.TimelineTrace=WARN:ndnsf.di.RuntimeEvidence=TRACE'
```

Do not export an application `NDN_LOG` globally around the MiniNDN command;
NFD inherits its environment and may reject application logger syntax. Pass
the filter through the application child environment as the launcher does.
...
owner, selected-Provider-only event grants, complete 82-FR/21-SC mapping
(including FR-030a and FR-032a), and
...
single-writer conflict handling. These cases are implemented and mandatory in
the next current-source G2 qualification; the historical G2 pass predates them
and cannot qualify the current candidate.
...
bounded queues, packet lineage, and clean teardown. The runner broadcasts a
SIGINT to all owned children, waits one bounded graceful-drain window, and
records any required force-termination in `forcedShutdownSignals`; such a
harness-initiated teardown is distinct from a child self-abort. A failure returns to the
native or MiniNDN owner and does not trigger a SIF rebuild.

Historical checkpoint (2026-09-01; invalidated 2026-09-02): T020 G0/G1/G2 and T022 G3 passed for source seal
`results/spec175/g0/source-seal-g2-probe-fixed-20260901T221454Z.json`; the strict
42-process matrix is recorded at
`results/spec175/g3/qualification-manifest-post-profile-20260901T221454Z.json`.
The launcher must preserve one
`<run-dir>.runner.log` beside each repetition directory (captured production-
runner stdout). The strict manifest treats that file as required evidence; a
missing log blocks G3 and must not be synthesized from another run.

The three repetitions are independent processes but use fixed case seeds so
the protocol/fault subject is identical across repetitions:

```text
M01--M04, M10--M14: seed=1750001
M05--M09:           seed=1750002
campaignId=spec175-<case>-<seed>
```

Do not generate a seed series per repetition. A run with another seed is
diagnostic-only and must be excluded from the G3 manifest; rerun all 42
processes from a fresh output root under the fixed seed contract. A setup-layer
failure may be retried once only with the identical profile, seed, topology,
timeouts, and workload after the documented scoped cleanup; changing any of
those inputs creates a new subject.
...
an identical rerun passes. The historical M09 `returncode=-11` record in
`evidence/t022-g3-current-20260825n.md` remains a negative regression input and
must not be pooled with the new subject. The current host/CPU checkpoints already exercise M11-M14 for
...
export SPEC175_HOST_GATE_MANIFEST="$PWD/results/spec175/g3/qualification-manifest-<source-id>.json"
...
Before starting the six-entry sentinel, run one bounded M01 launcher smoke with
...
Replay M01, M04, M07, M10, M11, and M13 once each by keeping MiniNDN/Mininet/
OVS/NLSR on the host and launching each node's NFD, Controller, repository,
Provider, User, and ORT process from that exact SIF with
`apptainer exec --cleanenv`. This is a final packaging/reproducibility check,
not the primary debugging loop or a duplicate of G3. First
...
The replay driver's `--seed` is the healthy workload seed (`1750001`). It
automatically substitutes the registered fault seed (`1750002`) for M05--M09
and records the effective seed in every entry; do not pass one healthy seed to
all cases or hand-edit the resulting manifest.

The tracked host driver and its focused launcher-contract tests are implemented,
but no pytest or dry-run may be reported as G4. The current candidate must still
execute the driver so that the host creates the MiniNDN topology, invokes the
exact SIF for every NFD/application process, and emits a fresh six-entry G4
manifest. It must never start MiniNDN inside the SIF. In addition to the
service route probe, M01 must fetch one exact canonical source/role object named
by the accepted V3 projection and record publication, route, authentication,
decryption, and segment evidence. A repository receipt or catalog marker alone
does not satisfy FR-079. Expected:
M01/M04/M07/M10/M11/M13 each pass once (6/6), exact tiny-model
...
The matrix is case-isolated: the driver must prove zero owned processes,
namespaces, sockets, and temporary identities after each case; require all
four Provider readiness records before the User runs; and enforce a per-case
watchdog. If a campaign deadline is reached, the terminal manifest must name
the first incomplete case and mark every remaining case `NOT_RUN`. A partial
PASS count is diagnostic only and cannot authorize G4T/Tiger; permit at most
one byte-identical retry after the documented scoped cleanup.

The tracked driver owns the bounds (`CASE_WATCHDOG_SECONDS = 900` and
`CAMPAIGN_WATCHDOG_SECONDS = 14400`). Do not wrap it in an untracked shorter
`timeout`: an external interruption must still leave a terminal replay manifest
with `firstIncomplete` and `NOT_RUN` rows, and a partial count cannot authorize
G4 or Tiger. Changing the driver or these bounds invalidates the candidate
source seal and restarts at G0.

For a new campaign dimension, edit the checked-in profile's declared matrix,
not the shell command. Run-level rows (case, registered seed, output identity)
may share the sealed candidate. Subject-level rows (coverage, speed, timeout,
topology, Provider/GPU count, workload, or model/runtime) require a new profile
digest and the owning local gates before SIF/Tiger. An unlisted or unconsumed
environment variable is rejected before side effects.
...
The candidate record must bind one release tuple, not only the SIF:

```text
sourceSeal + sif + hostReplay + submitBundle + effectiveConfig +
modelArtifacts + validationContract
```

Before any SSH/upload/remote mutation/`sbatch`, generate and validate the
candidate-closure manifest. It must classify `changedPlane`, name the earliest
`restartGate`, bind terminal G0--G4 evidence, pass the historical-failure
mutation suite, and prove that a failed dry run makes zero external side-effect
calls. For functional/performance gates it must also bind a same-publisher,
non-mutating repository service request whose validated ACK returned over the
exact route before the publication barrier. Local registration/permission
markers and fixed sleeps do not satisfy that row.

Create one gate-specific run record from the checked-in
`packaging/ndnsf-di-container/jobs/spec175/proven-tiger-profile.json`. The
record contains absolute paths for the candidate closure, SIF, bundle,
workload, checklist, output directory, and (for model gates) the external
model manifest/root, plus the fixed per-gate provider/GPU/seed/resource values.
Only the stage-readiness device mapping is a runtime parameter delta because
the checked-in stage wrapper explicitly consumes it; changing the other values
requires a new profile/job and source seal.
Compute `profileSha256` from the canonical JSON before writing the record.
Render and inspect the exact submission once; rendering creates the
hash-bound delta report and does not contact Tiger:

```bash
PROFILE="$PWD/packaging/ndnsf-di-container/jobs/spec175/proven-tiger-profile.json"
RUN="$PWD/results/spec175/tiger/<candidate>/control.run.json"
python3 packaging/ndnsf-di-container/jobs/spec175/submit_profile.py \
  render "$PROFILE" "$RUN" "$PWD"
```

Only after the rendered report says `proven-baseline-exact-delta=PASS` and
the closure/checklist evidence is current may the same record be submitted:
...
Replace only the first argument and the run record for the registered
`stage-readiness`, `multi-provider`, `conversation-residency`, or `performance`
gate. Do not export ad-hoc values, call a `.sbatch` file directly, use a second
wrapper, or rely on `--export=ALL`; the submitter renders an explicit
`--export=NONE,...` environment and submits those exact bytes.
...
T024 implements and mutation-tests this `control` gate before the source
freeze. T025 executes it only after T023 produces the exact G4 candidate:
...
control only; it cannot satisfy G5U, G5, G6, or G7. Stop before model staging if it
fails.
...
## 10. G5U Tiger generic one-shot functional gate

After G4T passes, run the current candidate's ordinary one-shot ONNX path from
the same profile and submission entry point:

```bash
packaging/ndnsf-di-container/jobs/spec175/submit.sh \
  unary-functional "$PROFILE" "$RUN"
```

Expected: three fresh processes; one Request per process; four independent
Providers; four valid ACKs; one ACK-derived plan; every assigned role executes
once with CUDA ORT; one exact final Response; all child exits zero; no token
event, decode epoch, KV/recurrent/convolution cache, conversation checkpoint,
CPU model-compute fallback, or caller-supplied Provider map. This is the
generic-model functional row, not a Qwen or throughput result. The historical
small-model diagnostic wrapper/job cannot satisfy it.

## 11. G5 Tiger Qwen3.6-27B stage/cache-readiness gate

**Planned**:

```bash
packaging/ndnsf-di-container/jobs/spec175/submit.sh \
  stage-readiness "$PROFILE" "$RUN"
```
...
## 12. G6 Tiger three-Provider functional gate

```bash
packaging/ndnsf-di-container/jobs/spec175/submit.sh \
  multi-provider "$PROFILE" "$RUN"
```
...
## 13. G6C Tiger conversation-residency functional control

```bash
packaging/ndnsf-di-container/jobs/spec175/submit.sh \
  conversation-residency "$PROFILE" "$RUN"
```
...
## 14. G7 Tiger performance qualification
...
G5U unary PASS -> G5 readiness PASS -> G6 streaming PASS ->
G6C conversation/fallback PASS -> G7 verdict recorded
```
