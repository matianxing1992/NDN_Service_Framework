# Archived Experiment Plan Through 2026-09-02

> Historical audit record only. Use `../../experiment-plan.md` for the frozen
...
- Verification Status: IMPLEMENTATION_CONVERGENCE_PASS_G0_G3_PASS_G4_PENDING
- Version Label: spec175_code_plan_v5_implementation_first

## Experiment Overview

- **Title**: Multi-use-case distributed ONNX inference through NDNSF-DI
- **Objective**: Qualify one immutable candidate across generic one-shot
  inference, one-turn token streaming, multi-turn continuation, unavailable-
  state failure/fallback, and sustained warm generation. For the LLM rows,
  verify that one authorized placement can prefill once, produce
...
## Mandatory pre-test ordering

This plan does not authorize a test campaign by itself. Before any complete
unit/integration suite, MiniNDN run, benchmark, SIF replay, or Tiger job is
counted as validation, the active design, contracts, and effective
configuration must be traced through the production entry points and
dependencies. Record each design/code gap, repair the specification/tasks or
production path, run the focused regression, and re-audit to a fresh
code-aware `PASS`. A run started while that audit is `BLOCK` remains diagnostic
history only. Any behavior-affecting source, dependency, launcher,
configuration, artifact, topology, harness, or evidence-schema change restarts
the ordering gate.

The same ordering is a planning prerequisite: do not schedule or promise a
complete test campaign until the real production path has been traced and its
controlling gaps have been repaired or explicitly recorded as blockers. A
focused test may be planned only when it closes a named repair finding; it
must not be used to discover design/code distance or justify a broader run.
...
7. Can a completed turn finalize and atomically promote its request-local state,
   ...
   state over NDN?
8. Can the same candidate execute a generic one-shot multi-Provider ONNX plan
   and an autoregressive streamed plan without changing launcher, security,
   discovery, placement, or runtime backend?
...
| G5U-G7 | Slurm allocation, GPU/scratch/project storage, immutable model mounts, SIF hash/stage-once verification | NFD/NDNSF/ORT workload runtime, including G6C GPU/host state movement; no MiniNDN or NLSR |
...
same host MiniNDN harness run the six-row packaging-equivalence sentinel with
NFD and NDNSF/ORT applications launched from that image. One bounded current-
SIF D0-shaped Tiger deployment
control must then reproduce the proven launcher/lifecycle before model staging;
only after G4T passes may G5U-G7 run. Any later source/contract/workload change
...
| Unary functional control | `results/spec175/g5u/qualification-manifest-v1.json` | JSON | three exact one-shot four-Provider CUDA results with no stream/KV state |
...
- Exact-SIF G4: one fresh process for M01, M04, M07, M10, M11, and M13
  (6 total). G4 proves packaging equivalence; G3 remains the exhaustive
  semantic matrix.
...
- G5U: three fresh generic one-shot invocations using the deterministic small
  ONNX four-role plan and four independent Provider processes. Each Request
  obtains an ACK-derived role map, executes every role once through CUDA ORT,
  returns one exact final Response, emits no token events, allocates no decode
  or conversation state, and uses no CPU model-compute fallback. This row
  proves ordinary-model compatibility only; it is not a Qwen or performance
  result.
...
- G5U/G5/G6/G6C use exact digests and zero model-compute fallback. The 20 token/s
...
For G5U, the experimental unit is one one-shot Request. It fails if any token
event, prefill/decode epoch, KV-cache entry, conversation checkpoint, second
service Request, fixed offline Provider map, missing role, CPU model-compute
fallback, or more than one final Response is observed. Exact output alone is
insufficient unless all four independent Provider processes execute their
assigned role and the plan is derived from the accepted ACK snapshot.
