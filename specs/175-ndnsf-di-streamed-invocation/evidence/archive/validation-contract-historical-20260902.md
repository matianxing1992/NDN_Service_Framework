...
## Spec175 execution profile (2026-09-02)

The current normative route is `T020 -> T022 -> T025 -> T028`: one fresh
design/code seal, one local release gate, one candidate-bound Tiger deployment,
and one final closure. T022 uses the unit/Python/native checks, one
representative M01 host/CPU production-path smoke, and one exact-SIF smoke.
The former G0--G4 and M01--M14 matrix material below is retained for provenance
only; it is not an active Spec175 prerequisite and must not be rerun to unlock
Tiger. The names `G4T`, `G5`, `G6`, `G6C`, and `G7` are historical labels only.
If the launcher emits their checks, they are sections of the single
candidate-bound T025 deployment, never separate jobs.

**Operator stop point.** Read this current profile and the active route above,
then use `../quickstart.md`. The historical sections below are retained only
for provenance and compatibility tests; they are not instructions to run a
matrix, repeat a gate, or submit another job. T025's checked-in
`multi-provider` submission has already been consumed once for r5; the only
remaining action is T028 negative closure.

## Historical validation contract (reference only; not the current route)

The following detailed gate definitions document earlier validation subjects,
their failure evidence, and checks still embedded in compatibility drivers. They
are retained for provenance only. They must not be interpreted as additional
tasks, matrix rows, candidates, repetitions, or retry loops. The current queue,
candidate identity, and acceptance criteria are defined only by `spec.md`,
`plan.md`, and `tasks.md`.

For the current candidate, the only executable route is:

```text
T020 fresh seal and design/code convergence
  -> T022 one local release (unit/Python/native + one M01 host/CPU smoke
     + one exact-SIF smoke)
  -> T025 one candidate-bound Tiger deployment
  -> T028 final closure
```

In particular, the historical three-process healthy-case rule, M01--M14
matrix, six-row SIF replay, and G4/G5/G6/G7 labels below do not authorize a
second local campaign or a second Tiger job. If an existing validator still
requires one of those shapes, it is a tooling defect to repair before T022;
operators must not satisfy it by rerunning the historical matrix.

## Global rules

- The design-to-code convergence gate G-1 runs before the local release gate.
  Only a fresh G-1 `PASS` authorizes the local release and the one Tiger
  deployment G-T. Any
  required failure blocks later gates; no `--force` path exists for evidence
  acceptance. The downstream checks are bounded sections of G-T, not an
  open-ended diagnostic campaign.
- The registered deployment uses workload seed `1750001` and Greedy sampling
  for exact token comparison. Fault scheduling is not an active Spec175 axis.
...
- Required tests may not be skipped. A missing dependency is a failed gate, not
  a skip/pass.
- Historical gate tables may describe repeated OS processes, but the current
  route runs one registered host smoke and one exact-SIF smoke. Operators must
  not recreate those historical repetitions or manually repeat a passing case.
- If warm measurements are included in the registered deployment workload,
  cold preparation is recorded separately and warm samples are reported with
  their exact count. They remain descriptive evidence in G-T, not a separate
  performance campaign.
- There are no registered repetitions in the current route. A changed runtime
  parameter creates a new candidate and cannot be pooled with this deployment.
- **Implementation-first boundary**: while any Core/runtime/API/conversation or
  ...
  T029--T042 (including reopened T031) close, freeze one current
  source/workload subject and run G0--G3
  once. SIF creation, SIF preflight/replay, remote upload, Slurm allocation,
  and Tiger jobs are refused until the one real host/CPU smoke is `PASS`. A
  pre-smoke SIF is diagnostic-only. After the local release gate passes, build
  one final SIF; any later source, contract, or workload edit
...
- A signal exit, core dump, missing case-result record, or unclassified nonzero
  child status from the current subject blocks promotion. Preserve the first
  result, classify the owner, and do not submit a second Tiger attempt from
  Spec175.
...
- **Execution boundary (current route)**: the local host smoke is
  host-orchestrated. The host runs MiniNDN,
  ...
  G-T runs the SIF directly under Slurm and does not run MiniNDN.

### Current execution boundary

The authoritative status is `../audit.md` and the only open queue is
`../tasks.md`. Historical source seals, matrices, SIF digests, failed object
fetches, and Tiger jobs remain in `../evidence/` and `../evidence/archive/`;
none qualifies a new candidate. No current-candidate G-T PASS is claimed.

The allowed route was the single T020 -> T022 -> T025 -> T028 chain. T025 failed
before generation because the exact-SIF runtime rejected the planning-manifest
candidate digest; T028 records the unqualified result. Do not copy dated task
counts or checkpoint narratives into this contract, and do not submit another
job from this Spec. Conversation/performance clauses below are optional
observations inside the one deployment, not additional targets.
...
## G-1 - Design-to-code convergence gate

G-1 is a development gate and creates no qualification manifest. It prevents
formal resealing while the public contract and production path disagree. All of
the following focused positive and mutation tests must pass:

G-1 follows the project-wide checklist in
`.specify/memory/design-code-convergence.md`; this feature adds stricter
NDNSF-DI requirements but does not weaken that gate.

1. Spec175 streaming rejects V2, hybrid, TensorGroup/rank-role, tensor degree
   other than one, missing-role, and duplicate-Provider plans before Selection;
   the registered workload uses `PreSplitFirstStrategy`.
2. The native Provider starts without ready-made role artifacts and, after
   Selection, fetches/verifies the canonical artifact root and sealed recipe,
   assembles/caches the local role, and loads ORT. The G4 case must also prove
   that the exact canonical source/role object named by the projection is
   published, routed, fetched, authenticated, and decryptable; a repository
   receipt, catalog marker, or service ACK alone is insufficient. Preassembled
   compatibility is rejected by the formal launcher.
3. The real native final role executes the sealed sampler and standalone
   tokenizer, emits oracle-matching text deltas, stop reason, and final decoded
   text; token-ID-only and digest-only-sampling mutations fail.
4. The real distributed CUDA coordinator retains opaque device state across
   epochs and reports zero complete-state host round trips after prefill. A
   direct single-runner proof is insufficient.
5. Conversation pause/prefetch performs measured physical D2H/H2D movement and
   allocation/release. Enum-only or accounting-only transitions fail.
6. Named ndn-cxx components and severity filters control production logging;
   sensitive payload/state data remains absent.

G-1 passes only when the code-aware audit names the exact repaired source,
production entry points, effective configuration, traceability, focused
regressions, and zero unresolved controlling gaps. Only after that `PASS` may
T035--T036 close and G0 begin.
...
gate against the sealed subject to `PASS` before the current T022 local-release
gate can promote.
...
## Historical G3 - Host/CPU real MiniNDN gate (not current route)

The former contract ran the four-Provider M01-M14 matrix against the native/
Python build and tiny ONNX fixture before building a SIF. That matrix, its
topology/fault rows, and its repetitions are retained below as historical
debugging evidence only. The current T022 route runs one registered M01
production-path smoke and must not invoke this matrix.
...
The NFD readiness record must verify topology closure, not merely prefix text:
router `a` must have one distinct `/example/llm-pipeline/group` next hop for
each of `u`, `p0`, `p1`, `p2`, `p3`, and `repo`, and every member must have its
group route back to `a`. A prefix/strategy snapshot with one missing expected
face is FAIL. Routine lifecycle evidence exports
`NDN_LOG=*=WARN:ndn_service_framework.TimelineTrace=WARN:ndnsf.di.RuntimeEvidence=WARN`
and `NDNSF_CONTROL_TIMING=1`; global TRACE is reserved for a bounded diagnostic
and cannot be needed for a qualification PASS. Every machine-readable native
record must be completely assembled and submitted once through a single
ndn-cxx logger sink. A record emitted through multiple `std::cout` insertions
is invalid because merged stdout/stderr can interleave within a field. Keep
`NDN_LOG_NOFLUSH` unset when crash or terminal evidence is part of acceptance.

Pass: every registered case repetition passes, every same-subject attempt is retained
and no signal/native exit not initiated by the harness remains unclassified
(bounded forced teardown after graceful-drain expiry is recorded separately), process-tree cleanup succeeds, exact
...
Python module bind mounts are forbidden for G4 and G-T. External model artifacts are
...
Replay the representative M01, M04, M07, M10, M11, and M13 sentinel rows with
the host creating the MiniNDN topology and using the exact SIF as the command
provider for each node's NFD and NDNSF/ORT processes. This is a packaging/
reproducibility check, not the primary debugging loop or a second semantic
campaign. Any mismatch is first classified as host-substrate,
...
The table defines the full G3 matrix. G4 executes only the six named sentinel
rows once each. M11--M14 use the same topology and tiny ONNX bundle and add no
SIF or GPU dependency:
...
Historical command boundary (not the current T022 route):
...
sudo -E env \
  ...
  --cases M01,M04,M07,M10,M11,M13 --repeats 1 \
  --output results/spec175/g4/qualification-manifest-v1.json
```

Pass: the registered exact-SIF smoke passes, process-tree cleanup succeeds, exact
...
topology, fixture, Apptainer, and G3 manifest digests. G3 remains the exhaustive
registered semantic matrix; G4 proves that one representative protocol path
survives packaging without repeating the entire low-cost campaign.

## G4T/G5/G6/G6C/G7 - Internal checks within the single Tiger deployment (G-T)

The following sections define the evidence collected by the one T025/G-T
deployment. They are not separate Spec175 tasks, candidates, or retry loops.
If a check is not included in the registered deployment workload, record it as
not measured rather than creating a new job.

G4T reuses the proven Spec170 D0/r23 operational shape before any 27B artifact
is staged. A checked-in `proven-tiger-profile` records the exact common
wrapper/helper hashes, Apptainer command, explicit bundle `cwd`, mounts,
isolated HOME/PIB/TPM, identities/routes, child order, readiness barrier,
resource envelope, workload, and terminal oracle. It also records the only
fields each later gate may change. A machine-readable delta manifest compares
the fully rendered Spec175 command/effective configuration with this profile.
Every difference must be allowlisted and value-validated; an unknown,
ambient-only, or unlisted difference fails before `sbatch`.

The first internal check is one bounded CPU/no-GPU job with the exact current SIF: one Controller, one
...
MiniNDN/NLSR, build/materialize an image, or overlay source. Every child must
  ...
  and cannot satisfy the downstream stage, stream, conversation, or performance
  gates. Failure blocks model staging and must be reduced to
one delta from the historical control.

## Retired G5U row (local-only compatibility check)

The former Tiger G5U submission was removed from the release route because it
duplicated the local unary contract checks and had no registered job in the
single Tiger interface. Generic unary behavior remains required at G1/G2:
normal discovery, ACK-derived placement, one execution per role, one terminal
Response, no stream/KV/conversation state, and no hidden CPU model fallback.
Those local results are sufficient to establish API compatibility; Tiger is
reserved for the expensive stateful Qwen deployment path.

### G5 - Qwen3.6-27B stage and cache readiness check
...
### G6 - three-Provider streamed functional check
...
## Optional observations within G-T

The following clauses are active G6C/G7 gates after G6. They are invoked by the
same `submit.sh` candidate route and do not permit diagnostic rows, parameter
sweeps, or a second candidate.

### G6C - conversation-residency observation

When the registered workload includes continuation, it runs after the stream
check as a bounded functional observation, not a throughput campaign. The frozen continuation workload
...
### G7 - warm performance observation

G7 reuses the same candidate and workload without changes. For each warm
invocation report:
...
packaging/ndnsf-di-container/jobs/spec175/submit.sh \
  ...
  packaging/ndnsf-di-container/jobs/spec175/proven-tiger-profile.json \
  /absolute/path/to/candidate-gate-run.json
```

The run JSON carries the candidate closure, exact local/remote SIF identity,
workload, checklist, output/run identity, and gate-specific model/resource
values. The submitter rejects unknown keys and inherited values, renders an
explicit sealed job environment, and invokes Slurm with ambient export disabled.
The profile, run JSON, exact rendered configuration, and final `sbatch` argv are
hash-bound together. Direct `.sbatch` execution and post-validation command
...
The checklist is nested inside a repository-owned candidate-closure manifest.
That manifest MUST bind the source seal, exact SIF, host-replay harness,
submit-bundle tree, workload/effective configuration, model/artifact set, and
this validation contract. It MUST also record `changedPlane`, previous and new
component identities, `invalidatedGates`, `restartGate`, and the terminal G0--G4
manifest hashes. Any missing component or cross-candidate prerequisite fails
...
`functional-bundle-identity-closure` and
`repository-service-route-readiness`. The latter requires a non-mutating
same-identity repository request/validated-ACK record from the exact service
route before the publication barrier; registration, permission, or catalog
markers alone fail this row. The repository-owned functional preflight
...
All submit phases are cumulative sections of the one G-T candidate-bound
deployment. A phase may consume an earlier section's manifest, but a manifest
from another candidate, SIF, workload, model, or source seal is missing
evidence, not a reusable pass.
...
The mutation contract is part of the gate. It MUST remove or alter, one at a
time, a transitive helper, producer/consumer environment name, bundle `cwd`,
artifact/sidecar, identity/token entry, HOME/PIB/TPM assignment, Controller
start/liveness path, memory request, candidate digest, child exit, live
repository ACK, profile field, gate allowlist, rendered command, and ambient
environment value. It MUST also attempt direct `.sbatch` execution. Every
mutation MUST fail with zero observed SSH, upload, remote mutation, model
staging, or `sbatch` calls.

The script must then run login preflight, bounded compute-node preflight,
proven-profile semantic delta validation, SIF/config/model hash verification,
...
The `control` gate requires no model manifest or GPU; model/cache fields become
mandatory only for `stage-readiness`, `multi-provider`,
`conversation-residency`, and `performance`. The interface is exactly one
positional gate, one proven-profile path, and one run-record path.
