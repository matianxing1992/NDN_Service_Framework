...
2026-09-16 B187-AUTHORITY-CANDIDATE：authority 闭包提交 `bd5b2f3e` 纳入干净 source handoff，新的完整两阶段候选构建完成。Apptainer 1.5.3，base SHA-256 `8ebfc4646a5f96109a8480b120e684ee3923bf067d2e49aef53b42d29e5acdd9`，source seal `be8af5b9f765c51963b38b306c0d5d9a0ac79653a51cb3b5bf8f7ac017cfcb31`，Waf `295/295`（含 `DI_NativeArtifactAuthority`，`-j4`，19m31.89s），候选 SIF `Experiments/TigerCluster/images/spec187-complete-20260916-authority/candidate.sif` 为 4,139,462,656 bytes，SHA-256 `ec21657dfed64d4ac0d3ad435c4796ba2aab66172c7256813b032cabdd1fc1eb`。builder/final 原生 manifest、Python binding、`ldd` 和构建内验证通过；外部 `--cleanenv --containall --writable-tmpfs` verifier exit 0（5.94s）。同一候选上 C++ DI unit selector 通过（`DI_CPP_UNIT_SMOKE_PASS`，46.37s，binary SHA `b724eb792b9812bb8c76d48fcee4de68c30a87619ec30a8a136a28a533af721a`），C++ YOLO native runner 三次通过（`YOLO_CPU_NATIVE_RUNNER_PASS`，9.15s，binary SHA `f2d5d1cd6eddef9abec17829c63e13e7efde6da541c90b5bef05ca8f467703fd`）。这些是 cleanenv/局部行为门证据，候选仍为 `BUILT_UNQUALIFIED`；host-gate、APP pair、authority runtime 配置、through-MiniNDN 请求和 Tiger 尚未验收。
