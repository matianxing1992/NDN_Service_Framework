...
2. The Controller replaces only the target identity's complete policy. NAC-ABE
   generates the replacement DKEY lazily when that identity fetches it. After
   the target installs the newer signed status, the runtime SHOULD initiate one
   coalesced target-only fetch; an explicit fetch is the fallback when automatic
   refresh is unavailable. The replacement contains all of the identity's
   retained attributes plus the new one. No DKEY is issued, invalidated, or
   refetched for an unaffected identity.
