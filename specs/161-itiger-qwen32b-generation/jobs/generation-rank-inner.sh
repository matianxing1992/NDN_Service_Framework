...
print("SPEC161_CONTROLLER_READY", flush=True)
controller.run()
PY
  controller_pid=$!
  pids+=("$controller_pid")
  for _ in $(seq 1 240); do
    grep -q SPEC161_CONTROLLER_READY /scratch/log/controller.log && break
    kill -0 "$controller_pid" 2>/dev/null || exit 5
    sleep 0.5
  done
  grep -q SPEC161_CONTROLLER_READY /scratch/log/controller.log
...
if test "$rank" -eq 0; then
  ...
  /opt/venv/bin/python /source/llm_pipeline/user.py \
    ...
    --group /NDNSF-DistributeInference/example/group \
    --runtime qwen-transformers \
    --generation-campaign-manifest "$SPEC161_SMOKE_CAMPAIGN" \
    --generation-jsonl /scratch/log/generation-raw.jsonl \
    --qwen-tokenizer-dir "$(dirname "$SPEC161_SMOKE_CAMPAIGN")/tokenizer" \
    --request-id "${SPEC161_PREP_SUBMISSION_ID:-spec161-smoke}" \
    --stages 3 \
    --max-new-tokens 64 \
