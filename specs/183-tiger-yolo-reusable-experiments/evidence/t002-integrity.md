...
- 在既有builder增加显式Spec183 host-gate dispatch，保持旧Spec175合同和source/ABI门。
- 解析sourceLock/sourceSeal/wheels及所有harness/model文件的传递完整性，复用已有validator；当前最低文件集合不是完整闭包证明。
- 真实receipt内容/来源/数值/生命周期验证，拒绝仅手填PASS。
- 真实check/prepare/build/upload/submit边界接线和零外部调用mutation tests；当前完整性函数本身不做外部操作，不能据此称生产入口已被门控。
