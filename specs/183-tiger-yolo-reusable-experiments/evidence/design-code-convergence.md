...
| N2 / HIGH | `tools/spec183_minindn.py` still accepts only Y-B and has unbounded outer `subprocess.run`; three registered cases, owned cleanup and semantic manifest remain absent. [Input-binding repair](t010-input-binding.md) now verifies prepared/profile/harness/public preparation, actual per-run key pairs and maps; uses exclusive run output and actual package/epoch. Generic driver preserves explicit requester/authority keys. Marker/docstring now correctly say no qualification. | T004/T005/T010: finish bounded owned cleanup and three registered case selectors against the maintained driver. Produce N1's receipt from those same executions. Do not substitute an unrelated ingress failure for missing dependency Data. | Input checks 13 passed; driver environment checks 22 passed; final single wrapper assertion check passed (overlap). No network/runtime qualification. Add only remaining changed boundaries; after T007/T008/T009 run exactly three registered cases and collect evidence in those runs. |
