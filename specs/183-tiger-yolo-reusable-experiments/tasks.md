...
| T010.a | T010 | MiniNDN 正常 CPU 图、权限拒绝、缺依赖三个注册场景 | BLOCKED | [输入绑定](evidence/t010-input-binding.md)已验；[取消/清理](evidence/t010-cancellation.md)：信号进入finally，移除全局清理；6个组件用例最终有通过证据；仅Y-B，V11 NOT_RUN | 剩三场景、外层总时限、子进程强杀后reap/清理证据；T007前完成源码，T009后实跑 | 仅重跑2项信号枚举断言修复；复用其余4项及输入证据，不重复历史campaign |
