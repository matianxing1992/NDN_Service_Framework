...
| T004.b | T004 | `jobs/yolo/submit.py` 的远端 staging、run 接真实 worker | BLOCKED | `REMOTE_STAGING_NOT_WIRED`、`DISTRIBUTED_RUNNER_NOT_WIRED` 仍在；local 见 T004.d，单 GPU 私有入口见 T004.g | 完成暂存/双节点/负例/作业终态接线；在 T007 前完成审查 | 只验证新调用边界，复用既有 worker/barrier/journal 证据 |
