...
| T004.m | T004 | sender submit使用本机解释器；两端同名project路径保留验收原字节 | IN_PROGRESS | [submit origin](evidence/t004-submit-origin.md)；入口修复及相关68项组件检查通过；运输合同已定 | 仍需闭包枚举、SSH运输、接收发布/恢复；本机project镜像尚未创建；未执行实际运输 | 仅重测提交解释器和接收边界；不修改旧回执、不重跑模型或下载SIF |
