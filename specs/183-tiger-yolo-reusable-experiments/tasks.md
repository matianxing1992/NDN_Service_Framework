...
| T010.a | T010 | MiniNDN 正常 CPU 图、权限拒绝、缺依赖三个注册场景 | VERIFIED_REAL_YB_YN | [APP v33 Y-B/Y-N](evidence/minindn-v86-v88-exact-sif-yb-yn-v33.md)；v86 正常数值与 v88 八个注册子案全 PASS，绑定 exact-SIF、oracle 和 clean cleanup | 本行的 fresh aggregate 已完成；父 T010 仍受 T007/T008 正式顺序与 `qualification=NOT_EVALUATED` 约束；GPU/Tiger资格未完成 | base/app/harness 不变不重跑；仅 aggregate driver、base/ABI/app 行为变化时重跑对应矩阵 |
| T010.b | T010 | 同源 host qualification manifest 绑定命令、结果与清理 | VERIFIED_HOST_RECEIPT | [APP v33 host gate](evidence/host-minindn-v33.md)；receipt、shared validator/producer focused tests 和 profile consumer 均通过；qualification 明确为 `YOLO_HOST_GATE_COMPONENT_ONLY` | 不把 COMPONENT_ONLY 改成正式 PASS；正式 GPU/Tiger 资格仍需后续 gates | 生成清单不额外跑模型；不得把COMPONENT_ONLY改token冒充PASS |
...
| T011.b | T011 | 构建或复用基础SIF，在匹配SDK构建独立app并验证组合闭包 | VERIFIED_COMPOSITION | [APP v33 local composition](evidence/minindn-local-v91-v33.md)：base six-library manifest 与 external APP v33 闭包通过；同一只读 SIF/app 完成 MiniNDN 正常/负例和数值回执，远端 local-cpu v33i 也复现 PASS。 | GPU/Tiger资格仍未完成；job 210316 暴露 native V3 `resources:[]` 容量缺陷，需 APP-only 修复后重跑 | 基础库/ABI未变复用SIF；仅应用改动重建受影响targets |
