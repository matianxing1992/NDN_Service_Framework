...
The v56 layered candidate keeps the v23 base SIF and external APP bundle while
using the sealed v56 harness. Local CPU, single-node GPU, two-node normal,
negative dependency, and an independent second two-node normal allocation all
have authoritative retained verdicts. Earlier v22/v35/v49/v54/v55 records remain
immutable historical provenance and failure evidence.

## 2026-09-11 v56 final execution ledger

| Task / gate | Run and exact result | Qualification effect |
| --- | --- | --- |
| T011 / V13 | `tiger-local-cpu-v56-r1`, candidate `sha256:f885f1b4c9020a71f06655bb702ea9a382e665b45aed3d7e93077386283dde63`, verdict `sha256:2159969ea07e52265e3147f8c28b2afe3485a078648e21cfbd83a314b3c88e89`, 2 requests, 9 edges/request, shape `[1,50,6]`, `maxAbsError=0.0005340576171875` | `NORMAL_EXPERIMENT_PASS` exact-SIF local |
| T013 / V15 | Slurm `210471` on `itiger02`, `tiger-single-node-gpu-v56-r1`, verdict `sha256:43ad2d5729798ac4f03901dbab22e80c4db261e0d72e6d37457e2fb79036d032`; 1 warmup + 1 measured, model roles CUDA, Merge CPU, GPU UUID `GPU-0fab1e2a-dc0e-f3b0-62c0-f2dfab914341` | `NORMAL_EXPERIMENT_PASS` single-node GPU |
| T014 / V16 | Slurm `210472` on `itiger02,itiger03`, `tiger-two-node-gpu-v56-r1`, verdict `sha256:0972a0d69c4c866c45bf9c6228629746d3f0311b6fb74e8b5df61cfb95b4d03a`; 1 warmup + 3 measured, 4 roles, 9 edges/request, both model ranks CUDA and Merge CPU | `NORMAL_EXPERIMENT_PASS` first two-node |
| T015 / V17 | Slurm `210473` on `itiger02,itiger03`, `tiger-negative-dependency-v56-r1`, verdict `sha256:15cbf0df6f48e685aab3c15fdf2f0f3be90402fccc5a1f3ed0e98e575fdbd76b`; exact withheld DetectShard0→Merge edge, native failure, no response/reselection, bounded cleanup | `EXPECTED_REJECTION_PASS`, `DEPENDENCY_DATA_MISSING` |
| T016 / V18 | Independent Slurm `210474` on `itiger02,itiger03`, `tiger-two-node-gpu-v56-r2`, verdict `sha256:b8513ddf693b74a22d6214d19d91fe3d55ba81ff1a9454a4a3e5386f686039d6`; same profile/content/graph, new run/allocation/GPU UUIDs, 1 warmup + 3 measured | `NORMAL_EXPERIMENT_PASS` reuse |
| T017 / V19 | Closure, environment, integration evidence, operator guide and failure log updated; offline reanalysis and strict audit run after the v56 source fix | Final handoff complete |

The immutable v56 identities are base SIF `sha256:44b44d564c64387716a17588ea387ee2a255948ee744855291c8e7a0676907b0` (3,586,351,104 bytes), APP manifest `sha256:4a6c3af0c2cc24620c28519404f1a472074ebf354608372934339ae241942db7`, harness `sha256:09d8bb4d237453acf1a7a33048363712c5bfdde87e8623a2be359f7eff969ed`, dispatch `sha256:d18cf044576875ac2aebc6d6b67a98439ac42ee37dd68510abcf8ff1bdd65545`, inputs `sha256:acd62dd7c78323f89b2a47ac27348a4d672cb1907e3802816a4d650a31e00933`, and runtime `sha256:16cbf7db11888d43e5d7e1431bd30b03928776660f62869f0050e6eb78c8fed3`. The v55 r20 collector boundary exposed the native `dependency wait failed: failed to fetch signed exact Data:` wrapper; v56 accepts that exact bound wrapper and retains opaque identity checks. No SIF rebuild was needed.

## Historical v54/v55 execution ledger (superseded)
...
历史检查点：[APP v35 shared local-cpu](evidence/minindn-local-v35.md)。不变的
...
新增检查点：[v52 exact-SIF Y-B](evidence/minindn-v52-exact-sif-yb-v32.md)：固定
base SIF `sha256:2c07a9f1…0cb829b5` 与只读 v32 APP manifest
`sha256:3f81b1c5…6afbf4d` 在真实 MiniNDN 四 Provider 图完成一次 YOLO 推理，
`T010_DONE/returncode=0`，`shape=[1,50,6]`、`matched=true`、
`maxAbsError=0.0005340576171875`，清理无错误。v51 Y-N-D 的真实保留日志也已在
[v51 evidence](evidence/minindn-v51-exact-sif-yn-v32.md) 中绑定到
`SELECTION_COMMITTED` 后的两条 native withheld tensor 与
`DEPENDENCY_DATA_MISSING`。三场景 receipt 见
[T010 host-gate producer](evidence/t010-host-gate-producer.md)；该 receipt 仍是
`YOLO_HOST_GATE_COMPONENT_ONLY`，不能替代 GPU/Tiger 资格。

新增检查点：[v58 exact-SIF local collector PASS](evidence/minindn-local-v58-exact-sif-pass.md)。
同一 v22 base SIF 与只读 v32 APP 在 `submit.py local --case local-cpu` 中完成真实
MiniNDN 两请求（warmup + measured）：四个 native Provider、ACK/Selection、protected
grant、跨角色依赖和 terminal response 均有保留证据；两次均为 `shape=[1,50,6]`、
`matched=true`、`maxAbsError=0.0005340576171875`，每次 9 条依赖边配对，清理为
...
新增检查点：[v60 empty-HOME/scratch isolation](evidence/minindn-local-v60-empty-home-scratch.md)。
在同一 base SIF 与只读 v32 APP 内以空 HOME 启动，scratch 完成 4096-byte fsync、
整只 3,901,079,552-byte SIF 复制及 SHA-256 校验，v32 User `--help` exit 0，NFD
...
下表记录其已声明范围；v56 exact-SIF 与 Tiger receipts 已补齐正式 runtime 资格。
下表是当前执行入口；后文 checkpoint 是历史证据，不应把旧“下一步”当作当前指令。
`VERIFIED` 仅指该行声明的范围；v56 retained receipts are the runtime authority. `IMPLEMENTED` 表示代码/脚本存在；历史失败仍单独保留。
...
T010/T011 的本机证据仍单独标注；完整候选运输与 GPU 资格已由 T012–T016 v56 receipts 验收。
...
| T002.a | T002 | `runtime/yolo_profile.py` I/R/E closure and invalidation | VERIFIED_PASS | [integrity](evidence/t002-integrity.md), T007 audit and v56 prepare/transport | No blocker; rerun on input/ABI/harness change |
| T002.a2 | T002 | Explicit layered base/app I/R/E generator | VERIFIED_PASS | [layered plane render](evidence/layered-plane-render.md), v56 candidate identities | Base ABI changes invalidate I/R; app-only changes invalidate E |
| T002.b | T002 | Builder consumes Spec183 host receipt and preserves Spec175 | VERIFIED_PASS | [T010 producer](evidence/t010-host-gate-producer.md), v56 dispatch gate | Re-run on receipt/source change |
...
| T004.a | T004 | Profile/schema, frozen bundle, five commands and journal | VERIFIED_PASS | v56 prepare, submit, collect and transport receipts | Re-run on profile or launcher change |
| T004.b | T004 | Remote staging and run use real worker | VERIFIED_PASS | v56 jobs 210471–210474 | Re-run on worker/runtime change |
...
| T004.g | T004 | Single GPU batch→srun→rank→collector | VERIFIED_PASS | [single-node-gpu](evidence/single-node-gpu.md), job 210471 | Re-run on launcher or placement change |
| T004.h | T004 | Two ranks, independent allocation and final aggregation | VERIFIED_PASS | [two-node-first](evidence/two-node-first.md), jobs 210472/210474 | Re-run on profile/runtime change |
| T004.i | T004 | Capacity, SIF identity, node-local NFD and cleanup | VERIFIED_PASS | [tiger-environment](evidence/tiger-environment.md), jobs 210471–210474 | Recheck each allocation |
...
| T004.s | T004 | Negative User terminal, cutpoint, failure and cleanup collection | VERIFIED_PASS | [tiger-negative](evidence/tiger-negative.md), job 210473 | Re-run on negative harness change |
| T004.d | T004 | local→冻结 CLI→签发→两个 CPU 请求→清理→真实 collector | IMPLEMENTED | [local owner](evidence/t004-local-owner-wiring.md)；114 个唯一组件用例最终有通过记录；[v58 exact-SIF local PASS](evidence/minindn-local-v58-exact-sif-pass.md) 证实相同发行边界的真实签发/请求/清理和 collector 结果 | T007 全链审计后再将 local CLI 作为正式 promotion gate；v58 仅是本机 CPU 证据，不替代 empty HOME/scratch、GPU 或 Tiger gate | 首轮仅剩准备 fixture 漂移，修后只重跑该模块12项；base/app不变不重复 v58 |
| T005.a | T005 | Real User/Provider parameters, permissions and readiness | VERIFIED_PASS | v56 normal and negative receipts | Re-run on app/security change |
| T005.b | T005 | Role specs→DI assembler→reference→MODELROOT | VERIFIED_PASS | v56 four-role normal receipts and graph digest | Re-run on app/graph change |
...
| T005.c | T005 | Controller publication receipt in exact composition | VERIFIED_PASS | v56 local and Tiger receipts | Re-run on APP/base change |
| T006.a | T006 | Result oracle binds numeric, role, edge, GPU, exit and negative state | VERIFIED_PASS | v56 verdicts and focused collector suite | Re-run on collector contract change |
| T006.b | T006 | Collector reads independent per-request graph references | VERIFIED_PASS | v56 local/single/two verdicts | Re-run on evidence schema change |
...
| T008.c | T008 | Extensions, entrypoints, ldd/readelf/hash and relevant units | VERIFIED_PASS | [host-unit](evidence/host-unit.md) plus v56 exact-SIF loader/runtime evidence | Re-run on base/ABI/toolchain change |
| T009.a | T009 | Multi-process CPU normal ACK/Selection→four roles→numeric result | VERIFIED_PASS | [integration](evidence/integration.md), v56 local verdict | Re-run on app/security/graph change |
| T009.b | T009 | Epoch/permission denial, wrong selection, activation tamper and cleanup | VERIFIED_PASS | [integration](evidence/integration.md), v58–v60 and v56 negative evidence | Re-run on security/failure harness change |
| T010.a | T010 | MiniNDN normal, permission denial and missing dependency cases | VERIFIED_PASS | [minindn](evidence/minindn.md), v58/v59/v60 real receipts | Re-run on MiniNDN/app/base change |
| T010.b | T010 | Source/base/app-bound host qualification manifest | VERIFIED_PASS | [host-minindn-v33](evidence/host-minindn-v33.md) and v56 candidate dispatch | Component scope remains explicit |
...
| T011.b | T011 | Reused base SIF plus matching SDK-built external APP closure | VERIFIED_PASS | [local-sif](evidence/local-sif.md), v56 exact composition and base hash reuse | Base ABI change requires rebuild |
...
| T011.c | T011 | Exact-SIF local CPU YOLO and empty HOME/scratch | VERIFIED_PASS | v56 local verdict and [integration](evidence/integration.md) | Re-run on app/base/isolation change |
| T012.a | T012 | GPU/Apptainer/capacity substrate values | VERIFIED_PASS | [tiger-environment](evidence/tiger-environment.md), v56 allocations | Recheck every allocation |
| T012.b | T012 | Exact-SIF staging, nodes/GPU/routes and service readiness | VERIFIED_PASS | jobs 210471–210474 and environment evidence | Recheck every allocation |
...
| T013.a | T013 | One-node GPU, four providers, 1 warmup + 1 measured | VERIFIED_PASS | [single-node-gpu](evidence/single-node-gpu.md), job 210471 | One bounded gate; no matrix expansion |
| T014.a | T014 | First two-node normal, 1 warmup + 3 measured | VERIFIED_PASS | [two-node-first](evidence/two-node-first.md), job 210472 | Preserve as first normal allocation |
| T015.a | T015 | Negative dependency after Selection with one exact cutpoint | VERIFIED_PASS | [tiger-negative](evidence/tiger-negative.md), job 210473 | Preserve v55/v54 failed boundaries |
| T016.a | T016 | Second independent two-node allocation, unchanged profile/SIF, 1+3 requests | VERIFIED_PASS | [two-node-reuse](evidence/two-node-reuse.md), job 210474 | Requalify on any behavior/input change |
| T017.a | T017 | Final operator guide and README | VERIFIED_PASS | updated guide and README with v56 commands and IDs | Keep links aligned on future changes |
| T017.b | T017 | Offline reanalysis and closure evidence | VERIFIED_PASS | [closure](evidence/closure.md), validation matrix and runtime checkpoint | No new GPU campaign required |
...
T005 closure checkpoint：`runtime/yolo_operator.py` remains the sole rank-level production seam and never fabricates protocol or model data; v56 real User/Provider/collector receipts prove the production call path and secure ACK/Selection lifecycle.
- [x] T006 [US2] Implement authoritative collection and failure oracles in `Experiments/TigerCluster/runtime/yolo_result.py` and `tests/test_yolo_result.py`: associate every request/attempt/plan/role/node/GPU/edge digest, validate numeric output via the existing contract, require all finite child exit0 plus controlled service shutdown/reap, and separate local/single-node/two-node/expected-rejection verdicts. Reject stale/forged PASS, missing role/response/CUDA, shape/class/tolerance mismatch, timeout-as-auth-rejection, nonzero worker, forced cleanup or corrupt output. Retain first failure and immutable reanalysis. Depends on T005.
...
UAV仅复用同一部署边界；本Spec不实现UAV应用或Spec182。YOLO正式安全、数值与 runtime 门已由 v56 回执闭合。
...
- [x] T012 [US3] Qualify promotion and allocated environment through `Experiments/TigerCluster/jobs/yolo/submit.py` and `runtime/yolo_worker.py`, recording `specs/183-tiger-yolo-reusable-experiments/evidence/tiger-environment.md`: audit final script/profile closure before side effects; verify local/project/staged SIF equality, compute Apptainer, CUDA/ORT, writable capacity, roles/cwd/mounts, NFD and bidirectional signed Data/permission readiness. No build on Tiger/login-node inference, no runtime path patching, zero Provider launch on mismatch. Depends on T011 dispatch receipt; per-allocation subset repeats in every subsequent job.
...
- [x] T014 [US3] Execute first normal two-node allocation from the unchanged normal case in `Experiments/TigerCluster/profiles/yolo-two-node.json`, recording `specs/183-tiger-yolo-reusable-experiments/evidence/two-node-first.md`: actual distinct hosts, A backbone/merge and B heads, 1 warmup + 3 measured requests, both directions of dependency edges and full per-request identity/backend/numeric/terminal closure. Stop on first failure and preserve partial results. Depends on T013.
...
T005依赖T004已有输入/运行接口，不要求其尚待T005/T006才能实现的实际argv和collect提前完成。T004已由真实 v56 prepare/submit/collect 运行闭合，生产 bundle 包含真实应用、collector 和 run.sbatch；T007 审计确认 T002–T006 全部闭合。
T002 的真实启动边界来自 T004，receipt 语义校验来自 T006；原先要求 T002 全部完成才实现消费者会造成循环依赖。T002 的 receipt 与零副作用门已在 T007 和 v56 transport/preflight 中验收。T010才产生真实host receipt；之前仅可用明确标注的测试fixture验证拒错逻辑。
...
## Historical Checkpoint (superseded by the v56 ledger)
