...
| T007.b | T007 | T002/T004/T005/T006 生产接线收敛为 PASS | BLOCKED | 同上；剩host资格语义和MiniNDN wrapper两个控制项 | N1/N2共享producer/validator；T010/T011物理回执留在后续；不用旧Y-N子场景替代缺依赖Data | 只重审/重测变化边界；未PASS不开展正式T008+验收 |
...
| T010.a | T010 | MiniNDN 正常 CPU 图、权限拒绝、缺依赖三个注册场景 | BLOCKED | [输入绑定](evidence/t010-input-binding.md)、[取消](evidence/t010-cancellation.md)、[子进程reap](evidence/t010-reaping.md)已接；19项通过含实际进程；失败保留句柄与attempt；仅Y-B，V11 NOT_RUN | 剩三场景、外层总时限、后代/网络清理完整证据；T007前完成源码，T009后实跑 | 单次exit-status观测修正后仅复跑4项helper；复用其他证据，不重跑历史campaign |
