...
| T004.b | T004 | `jobs/yolo/submit.py` 的远端 staging、run 接真实 worker | BLOCKED | 共享目录接收端submit已接（T004.k），未暂存返回SHARED_STAGING_REQUIRED；NEGATIVE_RUNNER_NOT_WIRED仍在 | 完成跨机器运输/可移植前置证据与负例；在 T007 前完成审查 | 只验证新调用边界，复用既有 worker/barrier/journal 证据 |
...
| T004.m | T004 | sender submit使用本机解释器；两端同名project路径保留验收原字节 | IN_PROGRESS | [submit origin](evidence/t004-submit-origin.md)；入口修复及相关68项组件检查通过；运输合同已定 | 仍需闭包枚举、SSH协调与候选运输；接收组件见T004.n；本机project镜像尚未创建 | 仅重测提交解释器和接收边界；不修改旧回执、不重跑模型或下载SIF |
| T004.n | T004 | 显式文件inventory与不覆盖接收发布；中断后复用已匹配文件 | IMPLEMENTED | [transport receiver](evidence/t004-transport-receiver.md)；15组件检查通过；真实SSH两文件首次发布/复用通过 | 仅运输组件；仍缺语义闭包枚举、活动allocation排除、SSH协调及submit接线；不计运行资格 | 只测新增文件/发布边界；11KB工具与合成输入，不传模型/密钥/SIF |
...
| T008.c | T008 | `_ndnsf` 与 `_py_repoclient`、真实入口、ldd/readelf/hash、注册 unit | BLOCKED | [host-unit](evidence/host-unit.md) 已记录树外 import/ldd，Controller 到达 NFD socket；注册 unit 尚未完成 | 实际入口到 socket 不等于服务验收；unit 属 T008 原验收，不能移到 T009 代替 | 同候选注册集合集中验收一次；不重复已有 loader 探测 |
