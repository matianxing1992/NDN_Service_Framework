...
| T004.p | T004 | `submit --plan-transport`经原门验证后枚举candidate与前置验收文件 | IMPLEMENTED | [transport inventory](evidence/t004-transport-inventory.md)；80首组、21入口、9最终清单检查通过；fixture scope | 尚缺自动SSH协调/接收后提交；真实候选未导出；harness增至26文件须更新E | 无模型/SSH/Slurm；按原collector输出取所需文件，不复制角色HOME或重跑模型 |
