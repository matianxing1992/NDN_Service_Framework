...
| T007-A1 YOLO26n Y-A | `NOT_RUN_CURRENT_CANDIDATE` | local root MiniNDN owner; depends A0 | Current-candidate C++ native numerical oracle, terminal result, child exits and cleanup | Earlier `r51` result is stale for the refreshed framework/DI binaries; rerun root Y-A before restoring `PASS_FOR_ROW` |
| T007-A2 YOLO26n Y-B/Y-N | `NOT_RUN_CURRENT_CANDIDATE` | local root MiniNDN owner; depends A1 | Protected multi-provider terminal path and all declared negative permutations with first-failure classification and cleanup | Earlier Y-B `r37` and Y-N `r50` results are stale for the refreshed framework/DI binaries; rerun both against the same refreshed candidate |
