...
| Current `NativeOfferAdmission::verify` C++ negative and immutable-view coverage (`Spec182OfferAdmission`, 18 cases / 81 assertions) | 182:T008, PO-013, FR-001/002/004/005/009, CD-003/004/013, INV-003/004 | normal and unsuppressed ASan/UBSan admission selectors pass; two frozen historical names remain contract-mapping gaps; process/no-Python, real-model and external rows remain open | PARTIAL |
...
| Local 0.6B capability, external model inventory and legacy retirement checks | 182:T013, 182:T016, PO-010/011/012/016, FR-005/014/019, CD-010/012 | same-tree C++ tests and 30 focused compatibility checks pass; no actual 0.6B weights or current Spec184 27B qualification is run; the remote HF cache is only 1/15 shards, while a complete 27B artifact exists only under historical Spec175 with a failed multi-provider terminal; all 344 compatibility entries remain `removalEligible=false`, including 20 `external_use_unknown` entries | `WAITING_EXTERNAL_INPUT` / `PARTIAL` / `OPEN` |
...
| `T007-A3` Qwen3.6-27B | `WAITING_EXTERNAL_INPUT` | Exact `Qwen/Qwen3.6-27B` three-stage manifest, tokenizer, CUDA runtime and current-candidate experiment-owner result; local 0.6B is smoke-only; a complete artifact exists under historical Spec175 but its multi-provider terminal exited `1`, and the HF cache remains incomplete at 1/15 shards; host capability and selector boundaries are recorded in [T007 model capability evidence](../evidence/t007-model-capability-20260912.md) and [A4 external boundary](../evidence/a4-retirement-external-boundary-20260912.md) |
