...
- [ ] T189 [US1] Commit the audited image-contract repair under a new source revision, create and verify its exact source seal, and build a matching Foundation locally without publishing it because the GPU Dockerfile fail-closed source-revision binding forbids reusing the `01f730d` Foundation for changed source (missing, HIGH; depends on T188)
