...
- [ ] T158 [US1] Run the full offline/container/workflow static suite, source-secret scan, local six-repository seal creation/verification, YAML parse, and strict Spec Kit audit; record counts/digests without claiming a cloud build (depends on T157)
- [ ] T159 [US1] After local PASS, commit and push the workflow/packaging revision once so its existing Experimental-path trigger starts one GitHub build; monitor to a terminal result and preserve the run ID, exact failure, or immutable GHCR digest without automatic rerun (depends on T158)
- [ ] T160 [US1] After T159 PASS, render/review and submit exactly one bounded iTiger CPU materialization job through the crash-safe journal, build `runtime.sif` from the exact GHCR digest, verify checksum/static runtime, atomically promote evidence, and never auto-resubmit (depends on T159)
