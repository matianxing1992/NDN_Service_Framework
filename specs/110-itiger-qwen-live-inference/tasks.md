...
- [ ] T213 [US1] Retag and publish exactly the T212 local image once without rebuilding it, retain the immutable GHCR digest, manifest, source seal, secret scan, SBOM, signature/provenance or an explicit unavailable-tool result, checksums, push log, and anonymous digest-access proof; preserve the first terminal failure without automatic retry per FR-002 and runtime-release contract (missing, HIGH; depends on T212 and explicit authorization)
- [ ] T214 [US1] After T213 PASS, render/review and submit exactly one bounded iTiger CPU Slurm materialization job through the crash-safe journal, create `runtime.sif` from the exact anonymous GHCR digest in allocation scratch, verify static runtime and SHA-256, atomically promote under `/project/$USER/ndnsf-di/releases/<release-id>/`, and preserve failure without resubmission per US1/AC1 and runtime-release contract (missing, HIGH; controlling replacement for T047/T160; depends on T213)
- [ ] T215 [US1] After T214 PASS, render/review and submit exactly one bounded RTX 5000 GPU runtime probe, then validate Slurm GRES/UUID, compute scratch fsync, `apptainer exec --nv`, injected driver ABI, NFD lifecycle, NDNSF/NDNSF-DI imports, PyTorch CUDA operation, ONNX Runtime CUDA kernel execution, teardown, and durable promotion without Qwen inference claims per US1/AC1-AC4 and FR-003 (missing, HIGH; controlling replacement for T048; depends on T214)
