...
- [X] T194 [US1] Commit the host-driver-boundary repair under a new source identity, create/verify its seal, rebuild the matching local Foundation without publication, and then unlock T190 for one new local candidate build that may reuse BuildKit downloads but not the failed source identity (missing, HIGH; depends on T193)

## Phase 21: CUDA-only ONNX provider and component identity convergence

- [X] T195 [US1] Preserve source `228341e0ea1f28956015fbaa30d2bf58a56b7789` local GPU build as `EXECUTED_FAIL` after 20m10s at the optional ONNX Runtime TensorRT provider closure scan, prove the required CUDA provider build stages completed and the `libcuda.so.1` boundary passed, and record that no final image or external publication exists (contradicts, HIGH; depends on T194 attempt)
- [X] T196 [US1] Add RED regressions that lock removal of the unused TensorRT provider DSO while retaining CUDA provider acceptance, and that bind final OCI source revision separately from an immutable Foundation source revision so GPU-only repairs cannot masquerade as Foundation rebuilds or force unnecessary NDN/security recompilation (missing, HIGH; depends on T195)
- [X] T197 [US1] Implement the CUDA-only ONNX provider policy in `gpu.lock`, `Dockerfile.gpu`, and preflight; add explicit `FOUNDATION_SOURCE_REVISION` validation/label/workflow input while retaining the Foundation digest input; and pass focused/full offline, secret, workflow, and strict audit gates (missing, HIGH; depends on T196)
- [X] T198 [US1] Commit and seal the final GPU-layer repair under a new source identity, reuse only the immutable source-bound `228341e` local Foundation through the explicit component binding, and execute T190 as a new local candidate without GHCR publication or iTiger submission (missing, HIGH; depends on T197)

## Phase 22: Complete optional-provider closure coverage

- [X] T199 [US1] Preserve source `9ea1d9a252b9a60e535ca77ee5f073b59f59de72` as `EXECUTED_FAIL` after 20m29s at the independently downloaded ONNX Runtime C++ SDK TensorRT DSO, while retaining the successful Foundation binding, Python provider removal, 244-target build, native provider linkage, Python wheels, disk samples, and no-publication boundary (contradicts, HIGH; depends on T198 attempt)
...
- [X] T202 [US1] Commit and seal the dual-location repair under a new source identity, reuse the explicitly source-bound `228341e` Foundation, and execute T190 as one new local candidate without GHCR publication or iTiger submission (missing, HIGH; depends on T201)
...
- [X] T205 [US1] Implement missing-package-only runtime installation, enforce it in preflight, pass focused/full offline, integration, secret, workflow, and strict audit gates, commit and seal a new source identity, then execute T190 without GHCR publication or iTiger submission (missing, HIGH; depends on T204)
...
- [X] T208 [US1] Enforce current-state installed-package filtering with `${db:Status-Abbrev}`, pass all local gates, commit and seal a new source identity, then execute T190 once without GHCR publication or iTiger submission (missing, HIGH; depends on T207)
...
- [X] T211 [US1] Bring final runtime verification into parity with the audited derivation policy, enforce sibling and host-driver markers in preflight, pass all local gates, commit and seal a new source identity, then execute T190 once without GHCR publication or iTiger submission (missing, HIGH; depends on T210)

## Phase 26: Accepted local OCI publication and iTiger GPU convergence

- [X] T212 [US1] Freeze and review one replacement release record binding accepted local image `sha256:57f3804cf787bec41d69f4dadcf286d030419b3cbd247678c92e0d2313829b6a`, source `1a320e5d9e42f4f76e78aac62d9bb647e3b159f0`, seal `sha256:48b6832b59c128353f42c8c50027ba9c8b46a14beb8ee6c8a92c3d1095a4f58b`, immutable Foundation source `228341e0ea1f28956015fbaa30d2bf58a56b7789`, exact absent GHCR tag, user authorization, and no-rebuild/no-rerun boundary in `evidence/runtime-release-1a320e5-authorization.md` per FR-002, US1/AC1, and plan: runtime release (contradicts, HIGH; supersedes the failed GitHub-build dependency of T047/T160 without rewriting their evidence)
- [X] T213 [US1] Retag and publish exactly the T212 local image once without rebuilding it, retain the immutable GHCR digest, manifest, source seal, secret scan, SBOM, signature/provenance or an explicit unavailable-tool result, checksums, push log, and anonymous digest-access proof; preserve the first terminal failure without automatic retry per FR-002 and runtime-release contract (missing, HIGH; depends on T212 and explicit authorization)
- [X] T214 [US1] After T213 PASS, render/review and submit exactly one bounded iTiger CPU Slurm materialization job through the crash-safe journal, create `runtime.sif` from the exact anonymous GHCR digest in allocation scratch, verify static runtime and SHA-256, atomically promote under `/project/$USER/ndnsf-di/releases/<release-id>/`, and preserve failure without resubmission per US1/AC1 and runtime-release contract (executed once as job `149669`; `EXECUTED_FAIL` before OCI download because compute-node Apptainer could not resolve UID `64102`; no SIF promoted and the submission identity is frozen; HIGH; controlling replacement for T047/T160; depends on T213)
- [ ] T215 [US1] After replacement CPU materialization T218 PASS, render/review and submit exactly one bounded RTX 5000 GPU runtime probe, then validate Slurm GRES/UUID, compute scratch fsync, `apptainer exec --nv`, injected driver ABI, NFD lifecycle, NDNSF/NDNSF-DI imports, PyTorch CUDA operation, ONNX Runtime CUDA kernel execution, teardown, and durable promotion without Qwen inference claims per US1/AC1-AC4 and FR-003 (missing, HIGH; controlling replacement for T048; depends on T218 PASS)
- [ ] T216 [US1] Validate the promoted T213-T215 evidence, record the exact release PASS/FAIL and compatibility limits in `evidence/runtime-release-verdict.md`, and update T047-T049 only if their original acceptance boundaries are actually satisfied per SC-001, FR-002-FR-003, and plan: experiment sequence (partial, HIGH; depends on T215 execution)

## Phase 27: iTiger compute-node account resolution recovery

- [ ] T217 [US1] Obtain authoritative iTiger confirmation or remediation that UID `64102` (`tma1`) resolves inside `bigTiger` Slurm allocations and site Apptainer can start, then freeze a distinct replacement candidate/run/submission record for the same immutable OCI digest and stop for fresh explicit authorization; do not reuse job `149669` or submission `spec110-submission-78936191eda59b501440` (partial: exactly-once CPU preflight job `150145` PASSed on `itiger07` with UID resolution and Apptainer `1.3.3-1.el9`, isolating the prior failure to `itiger02` or a node-local/transient condition; replacement materialization identity is not yet frozen; CRITICAL; depends on T214 `EXECUTED_FAIL`)
- [ ] T218 [US1] After T217 and fresh explicit human authorization, submit exactly one bounded CPU replacement materialization under the new identity, require UID lookup and `apptainer version` preflight before OCI download, create/verify the SIF in allocation scratch, atomically promote it with SHA-256 and static-runtime evidence, and preserve any failure without resubmission (missing, HIGH; controlling replacement for failed T214; depends on T217 and explicit authorization)

## Phase 28: Post-Spec-111 Slurm/Apptainer deployment bridge

**Goal**: Consume the completed Spec 111 deployment/revision/request workflow in
one new iTiger candidate without relabeling the current release or conflating
Slurm, APPDeployment and request state.

- [ ] T219 [P] [US3] Add RED `RuntimeAllocationHandoff` mutation and scheduler-vs-deployment/request state tests in `tests/container/itiger-qwen-live/unit/test_spec111_runtime_handoff.py`, rejecting pre-Spec-111 source, mutable OCI, wrong revision/SIF/model/process-map/network/state and missing authorization per FR-038-FR-040
- [ ] T220 [P] [US3] Add RED least-privilege bind tests for read-only release/model/artifact/exact-role identity, identity-partitioned writable `/state`, shared node-local run path and scratch, including broad-project/cross-role/symlink/host-executable negatives in `tests/container/itiger-qwen-live/integration/test_spec111_apptainer_binds.sh` per FR-041
- [ ] T221 [P] [US3] Add RED v2 process-map tests deriving arbitrary Provider/role cardinality from a frozen Spec 111 revision, wrapping every project process in the exact SIF, binding one NFD per node/socket and validating per-Provider Slurm GPU UUID visibility while preserving all v1 fixture hashes in `tests/container/itiger-qwen-live/unit/test_spec111_allocation_topology.py` per FR-042-FR-043
- [ ] T222 [P] [US6] Add RED PENDING/RUNNING/PREEMPTED/TIMEOUT/CANCELLED/COMPLETED versus APP/request-state, bounded-drain, forced-kill, journal-reopen and no-auto-resubmit tests in `tests/container/itiger-qwen-live/unit/test_spec111_slurm_lifecycle.py` per FR-044
- [ ] T223 [US3] Implement versioned `RuntimeAllocationHandoff`, `InfrastructureAllocationHandle` and revision-derived allocation-process-map v2 validation/rendering in `packaging/ndnsf-di-container/lib/`, retaining the frozen v1 schema and evidence readers byte-for-byte (depends on T219-T221)
- [ ] T224 [US3] Extend the canonical `run-container.sh` and profile schema with validated identity-partitioned state and node-run binds, exact per-process identity, SIF and GPU mapping, while retaining read-only models/artifacts/releases and forbidding broad writable project/evidence mounts (depends on T220-T223)
- [ ] T225 [US3] Extend `run-allocation-topology.sh` so every NFD/controller/APPDeployment-coordinator/Provider/client project command runs through the canonical exact-SIF runner, Provider count derives from the revision, generic agents precede apply, and readiness includes apply/READY/ACTIVE/request/drain/INACTIVE before zero-survivor teardown (depends on T221-T224)
- [ ] T226 [US3] Extend `lib/adapters/slurm_apptainer.py` and the operations CLI to render/reconcile the immutable handoff and distinct infrastructure handle without treating sbatch/PENDING/RUNNING/COMPLETED as APP or request success; do not submit in this task (depends on T223-T225)
- [ ] T227 [US6] Implement TERM/preemption/time-limit grace handling that requests bounded APP drain, flushes persistent journal/evidence cursors, atomically promotes scratch evidence, preserves forced-termination uncertainty and never auto-resubmits in the Slurm supervisor/finalizer (depends on T222, T225-T226)
- [ ] T228 [US3] Run the complete offline post-Spec-111 handoff from a frozen revision fixture through sbatch/process-map render, same-SIF/bind/GPU/lifecycle/teardown validation with zero submission side effects and record it in `specs/110-itiger-qwen-live-inference/evidence/spec111-handoff-offline-gate.md` (depends on T219-T227)
- [ ] T229 [US1] After Spec 111 T201, T216 runtime-substrate PASS and T228 PASS, build/seal/publish/materialize one new post-Spec-111 OCI/SIF candidate containing the implemented APP/Core workflow under a fresh exactly-once identity; preserve any failure and never reuse the T213 image as deployment evidence (depends on Spec 111 completion, T216, T228 and explicit authorization for external publication/materialization)
- [ ] T230 [US3] After T229 PASS and fresh explicit authorization, render/review/submit exactly one bounded single-node small-Qwen job that proves APPDeployment apply/READY/ACTIVE, durable APPClient submit/result, actual CUDA/ONNX GPU, drain/INACTIVE, state reopen and zero survivors; preserve the first terminal result without rerun (depends on T229)
- [ ] T231 [US3] Only after T230 PASS plus T064 selected-transport PASS, render/review/submit exactly one bounded multi-node small-Qwen extension with one containerized NFD per node, exact faces/routes, a real cross-node dependency and the same APP lifecycle evidence (depends on T064, T230 and fresh explicit authorization)
- [ ] T232 [US6] Record the post-Spec-111 iTiger PASS/FAIL boundary, immutable identities, job IDs, Slurm/APP/request state separation, binds, model/GPU/network evidence and residual no-public-service/no-production-HA limits in `specs/110-itiger-qwen-live-inference/evidence/spec111-itiger-verdict.md`, update the handoff and docs, and do not mark Spec 110 complete while earlier size/matrix tasks remain open (depends on T230 and any executed T231)

**Checkpoint**: A PASS proves bounded Slurm/Apptainer deployment/use of the new
Spec 111 candidate. It does not prove an always-on Docker service, production HA
