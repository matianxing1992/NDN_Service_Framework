...
- [ ] T217 [US1] Obtain authoritative iTiger confirmation or remediation that UID `64102` (`tma1`) resolves inside `bigTiger` Slurm allocations and site Apptainer 1.3.4 can start, then freeze a distinct replacement candidate/run/submission record for the same immutable OCI digest and stop for fresh explicit authorization; do not reuse job `149669` or submission `spec110-submission-78936191eda59b501440` (external blocker, CRITICAL; depends on T214 `EXECUTED_FAIL`)
