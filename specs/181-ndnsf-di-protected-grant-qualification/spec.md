...
- **FR-003** — **Provider 双侧解包一致性**。Python Provider 装配路径与
  ...
  全部绑定字段、过期与撤销序列，解包内容密钥，注册明文租约并在清理
  时零化。错误收件人、跨请求/attempt/core/model/纪元绑定、过期或撤销
  的 grant 必须在认证层失败关闭（Python 与 native 行为一致，用同一
  跨语言向量验证）。
- **FR-004** — **Y-N-E 真实变异**。Y-N-E 子用例必须让一个真实的 grant
  变异（过期、撤销、错误收件人或伪造签名）到达已实现的 verifier 并
  断言其在授权边界被拒；合成纪元异常不得充当该证据。
