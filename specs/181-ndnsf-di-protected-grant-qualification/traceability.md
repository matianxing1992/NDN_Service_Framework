...
| `cpp/ndnsf-di/NativeProviderHandler.cpp`、`NativeProtectedProvider.{hpp,cpp}`、`ProtectedRuntime.{hpp,cpp}`、`NativeGrantVerifier.{hpp,cpp}`、`_ndnsf.cpp` | T002, T003 | partial（native Ed25519/P-256 正向链和 T006 负例已有真实证据；helper/factory 已有源码检查点；维护 integration 测试、剩余 handler 源码及全部资源/异常验收仍待闭合）| grant 精确名获取、权威校验、KeyChain 解包、parity |
