...
| `provider.py` 装配入口 | T001 | partial（维护进程测试验证真实 grant 获取/装配、错误收件人、磁盘变异/错误密钥拒绝及清理；全部异常/资源边界仍待补）| `evidence/t001-t002-process-integration-20260905.md`；保护纪元下规范名获取 + 解包 + 租约 + 零化 |
| `cpp/ndnsf-di/NativeProviderHandler.cpp`、`NativeProtectedProvider.{hpp,cpp}`、`NativeProtectedGrantCredentials.cpp`、`NativeProtectedGrantTransport.cpp`、`ProtectedRuntime.{hpp,cpp}`、`NativeGrantVerifier.{hpp,cpp}`、`_ndnsf.cpp` | T002, T003 | partial（native Ed25519/P-256 正向链和 T006 负例已有真实证据；维护进程集成与 P-256 ASAN PASS；剩余 handler 源码及全部资源/异常验收仍待闭合）| `evidence/t001-t002-process-integration-20260905.md`；grant 精确名获取、权威校验、KeyChain 解包、parity |
...
|---|---|---|
| SC-001 | T001/T002/T003/T005/T006 | 真实 production 往返/负例已有定向证据；完整资源/异常边界及正式同源 MiniNDN 未闭合；撤销延期 |
| SC-002 | T001/T002/T004/T005/T006/T007 | T004/T006 的真实入口/边界验收完成；维护集成补 Python/native grant 链，全部异常与 T007 审计仍开放 |
