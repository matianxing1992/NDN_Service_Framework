...
- [ ] **T001 [US1] 权威网络服务端**。在 controller 维护入口
  （`examples/python/NDNSF-DistributedInference/yolo_2x2/controller.py`）
  注册 `/<authority>/NDNSF-DI/KEY-GRANT/v1` 前缀：接收签名
  GrantRequestV1（Interest 参数），调用 `ArtifactPolicyAuthority`
  执行请求方签名验证、模型/纪元/驻留策略校验、收件人加密、权威
  签名，服务规范名 grant Data；同时签发并服务 RevocationStateV1
  Data。身份与公钥摘要取自 Spec 180 注册表 `artifactPolicyAuthority`
  条目；私钥位于 `~/.config/ndnsf/spec180/`（mode 0600，git 外）。
  验收：unit（策略负例：未授权模型/纪元/驻留层、请求方即 Provider、
  过期；请求字段校验）；integration（跨进程真实往返：requester
  进程签名 → 权威进程签发 → requester 按规范名验证获取；错误请求方
  签名与未授权模型在真实网络往返中被拒）；证据
...
- [ ] **T002 [US1] Python Provider 解包接入**。`provider.py` 装配入口
  ...
  `protection_epoch != "plaintext-v1"` 时：按规范名精确获取 grant
  Data → `verify_and_unwrap_grant`（权威签名、绑定、过期、撤销）→
  `PlaintextLeaseRegistry` 注册内容密钥 → 装配 → 清理零化；任何校验
  失败以 `DI_PROTECTION_EPOCH_REJECTED` 失败关闭。文件：
  `NDNSF-DistributedInference/ndnsf_distributed_inference/provider.py`、
  `core/protected_artifacts.py`、`tests/python/test_spec181_provider_grant.py`。
  验收：unit（绑定/过期/撤销/跨绑定负例，复用 Spec 180 的 29 个编码
  测试为回归基线）；integration（真实权威进程 + 真实 Provider 进程
  路径：正确 grant 装配成功；错误收件人 grant 在 verifier 内被拒且
  无明文落地）；MiniNDN 由 T006 的 Y-B 保护纪元子用例覆盖。

- [ ] **T003 [US1] native Provider 解包与跨语言 parity**。
  `NativeProviderHandler`/`ProtectedRuntime` 增加规范名精确获取、
  权威签名/绑定/过期/撤销校验与 KeyChain 解包；绑定失败维持现有
  `DI_PROTECTED_RUNTIME_BINDING_MISMATCH` 语义并新增 grant 级错误码。
  跨语言向量：同一 grant 字节分别由 Python 与 native 解包，必须得到
  同一内容密钥；向量文件
  `tests/fixtures/spec181/grant-vectors-v1.json`（含正例与全部负例）。
  文件：`cpp/ndnsf-di/NativeProviderHandler.cpp`、
  `cpp/ndnsf-di/ProtectedRuntime.{hpp,cpp}`、
  `pythonWrapper/src/ndnsf/_ndnsf.cpp`、
  `tests/integration-tests/ndnsf-di-protected-grant.t.cpp`、
  `tests/python/test_spec181_native_grant_parity.py`。
  验收：unit（C++ 校验负例）；integration（Python 端到端 native
  provider 真实解包 + parity 向量双侧一致）；[MiniNDN] 由 T006 覆盖。

- [ ] **T004 [US1] Y-N-E 真实 grant 变异**。Y-N-E 子用例构造真实
  grant 变异（过期、撤销序列推进、错误收件人、伪造权威签名）并断言
  已实现 verifier 在授权边界拒绝（错误码为 grant 级拒绝码）；删除
  合成纪元异常路径。文件：`Experiments/NDNSF_DI_YoloAckDriven_Minindn.py`
  的 Y-N-E 子用例与 `tests/python/test_spec181_y_n_e.py`。
  验收：unit（四种变异构造）；integration（每种变异到达真实 verifier
  并在装配前被拒）；[MiniNDN] T006 的 Y-N-E 子用例。
...
- [ ] **T005 [US1] 运行时就绪边界修复**。`pythonWrapper/ndnsf/service.py`
  ...
  `stop()`/取消后不得热转（取消检查 + io 停止后立即退出）。
  文件：`pythonWrapper/ndnsf/service.py`、
  `ndn-service-framework/ServiceController.cpp`、
  `tests/python/test_spec181_controller_readiness.py`。
  验收：unit（取消/停止路径）；integration（真实 controller 进程：
  超时余量下边界成功不被误报；取消路径及时退出且无热转）。
...
- [ ] **T006 [US2] [MiniNDN] Y-N 全矩阵语义重跑**。在 MiniNDN 小模型
  ...
  Y-N-E（真实 grant 变异，T004）、Y-N-L（明文字段注入，redaction
  违规检测）。每个子用例断言注册拒绝原因 + 边界位置；全量子进程
  退出与清理；无关失败不得充当预期结果。文件：
  `Experiments/NDNSF_DI_YoloAckDriven_Minindn.py`（语义判定已在 Spec
  180 修复，本任务消费并重跑）、`tests/python/test_spec181_y_n_matrix.py`
  （矩阵参数化 + 原因断言）。验收：[MiniNDN] 七子用例全部预期结果、
  零未收集存活进程；evidence `evidence/t006-y-n-matrix-current.md`
  记录每子用例的拒绝原因与边界。
...
- [ ] **T009 [US4] S1 候选封印**。以提交哈希封印候选：源、契约、
  ...
  oracle、runner 与测试清单。文件：`scripts/spec180_candidate.py`
  （沿用）。验收：unit（脏树/跨候选证据拒绝）；封印记录含提交哈希
  与全部平面摘要。
...
- [ ] **T010 [US4] S4 本地 SIF + exact-SIF Y-B replay**。在 T008
  ...
  replay 结果摘要）；evidence `evidence/t010-exact-sif-replay-current.md`。

- [ ] **T011 [US4] S5 一次 Tiger Y-B + 终局记录**。通过 host-NFD
  node-local launcher 提交一次 `yolo-functional`：一节点、一 RTX、
  四 Provider 进程、一次 cold Y-B、三模型角色 CUDA 证据 + Merge CPU；
  终局记录在单一候选身份下映射全部 FR 到代码/测试/证据并发出一个
  功能裁决。文件：Spec 180 的 `packaging/ndnsf-di-container/jobs/spec180/*`
  与 `scripts/` 工具链（路径沿用）。验收：一次提交的完整结构化证据
  （协议/数值/设备/子进程退出/清理 oracle 全通过）；
  `evidence/closure-record.md` 为唯一裁决来源。

## Dependencies & Execution Order

```text
T001 -> T002 -> T003 -> T004 -> T006（T005 可与 T001 并行 [P]）
