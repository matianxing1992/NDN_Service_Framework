...
- [ ] **T002 [US1] 权威撤销状态服务**。同一权威进程按
  `/<authority>/NDNSF-DI/REVOCATION/v1/EPOCH/<protection-epoch>` 签发并
  服务 RevocationStateV1 Data（撤销序列、被撤销 grant 摘要、过期
  时间）；Provider 侧撤销校验消费该记录。文件：controller 维护入口、
  `core/protected_artifacts.py`（复用）。验收：unit（撤销状态签名/
  序列推进负例）；integration（撤销 → Provider 侧校验看到撤销记录
  的完整网络往返）。证据 `evidence/t002-revocation-service-current.md`。

- [ ] **T003 [US1] Python Provider 解包接入**。`provider.py` 装配入口
  ...
  `verify_and_unwrap_grant`（权威签名、绑定、过期、撤销）→
  ...
  撤销/跨绑定负例，复用 Spec 180 的 29 个编码测试为回归基线）；
  integration（真实权威进程 + 真实 Provider 进程路径：正确 grant
  装配成功；错误收件人 grant 在 verifier 内被拒且无明文落地）；
  MiniNDN 由 T007 的 Y-B 保护纪元子用例覆盖。证据
  `evidence/t003-python-provider-grant-current.md`。

- [ ] **T004 [US1] native Provider 解包**。
  `NativeProviderHandler`/`ProtectedRuntime` 增加规范名精确获取、
  权威签名/绑定/过期/撤销校验与 KeyChain 解包（信封算法按收件人
  密钥类型：Ed25519 → X25519 转换、EC → ECDH-P256，与 Python 信封
  `alg` 字段一致）；绑定失败维持现有
  ...
  （C++ 校验负例：错误权威/收件人/绑定/过期/撤销）；integration
  （Python 端到端 native provider 真实解包，权威为真实网络进程）。
  证据 `evidence/t004-native-provider-grant-current.md`。

- [ ] **T005 [US1] 跨语言 parity 向量锁定**。固定向量文件
  ...
  过期、撤销、伪造签名）。文件：
  `tests/python/test_spec181_native_grant_parity.py`（消费 T004 的
  pybind 面）。验收：integration（双侧一致断言；向量文件随任何编码
  变更必须同步更新并重新双侧验证）。证据
  `evidence/t005-grant-parity-current.md`。

- [ ] **T006 [US1] 运行时就绪边界修复**。`pythonWrapper/ndnsf/service.py`
  ...
  `evidence/t006-readiness-boundary-current.md`。

## Phase 2: User Story 2 — 负面矩阵语义化（Priority: P1）

- [ ] **T007 [US2] [MiniNDN] Y-N 全矩阵语义重跑**。在 MiniNDN 小模型
  ...
  Y-N-E（真实 grant 变异，由 T008 构造）、Y-N-L（明文字段注入，
  ...
  `evidence/t007-y-n-matrix-current.md` 记录每子用例的拒绝原因与
  边界。

- [ ] **T008 [US2] Y-N-E 真实 grant 变异构造**。构造四种真实 grant
  变异（过期、撤销序列推进、错误收件人、伪造权威签名）供 T007 的
  Y-N-E 子用例使用，断言已实现 verifier 在授权边界以
  `DI_PROTECTED_GRANT_REJECTED` 拒绝；删除合成纪元异常路径。文件：
  `Experiments/NDNSF_DI_YoloAckDriven_Minindn.py` 的 Y-N-E 子用例、
  `tests/python/test_spec181_y_n_e.py`。验收：unit（四种变异构造）；
  integration（每种变异到达真实 verifier 并在装配前被拒）。证据
  `evidence/t008-y-n-e-grant-mutation-current.md`。

## Phase 3: User Story 3 — 收敛与本地资格（Priority: P2）

- [ ] **T009 [US3] 设计-代码收敛审计**。按 12 审计原则对真实生产链
...
- [ ] **T010 [US3] [MiniNDN] 本地资格认证**。从 T009 PASS 的同一源
  身份（提交哈希）执行：单元/集成选择器清单（local-suite inventory，
  逐项子进程监督）+ MiniNDN Y-A（原子候选、1 Provider）、Y-B
  （共享骨架、4 Provider、含 T003/T004 的保护纪元 grant 往返）、
  Y-N 全矩阵（T007 同源重跑）；CPU 后端证据记录，不得呈现为 GPU
...
- [ ] **T011 [US4] S1 候选封印**。以提交哈希封印候选：源、契约、
  ...
  `evidence/t011-candidate-seal-current.md`。

- [ ] **T012 [US4] S4 本地 SIF + exact-SIF Y-B replay**。在 T010
  ...
  replay 结果摘要）；证据 `evidence/t012-exact-sif-replay-current.md`。

- [ ] **T013 [US4] S5 一次 Tiger Y-B 提交**。通过 host-NFD node-local
  ...
  `evidence/t013-tiger-submission-current.md`。

- [ ] **T014 [US4] 终局记录**。在单一候选身份下映射全部 FR 到代码、
...
```text
T001 -> T002 -> T003 -> T004 -> T005 -> T008 -> T007
T006 可与 T001 并行 [P]
