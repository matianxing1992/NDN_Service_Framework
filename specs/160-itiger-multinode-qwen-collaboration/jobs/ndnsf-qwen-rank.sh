...
scratch="${base}/spec160-qwen-${SLURM_JOB_ID}-rank-${rank}"
rm -rf "$scratch"
...
apptainer exec --nv --cleanenv --containall \
  ...
  --env "SPEC160_POLICY=${SPEC160_POLICY}" \
  --env "SPEC160_RUNTIME_SUMMARY=${SPEC160_RUNTIME_SUMMARY}" \
  --env "SPEC160_REQUEST_ID=${SPEC160_REQUEST_ID}" \
  "$SPEC160_SIF" /source/ndnsf-qwen-rank-inner.sh
