...
用户要求将182未完成部分拆到183，避免旧 Spec 继续膨胀。本单元仅新建精简文档、迁移执行归属、
...
本轮文档审查范围为新183文件、182移交标记、feature pointer、Design ledger；核对14/14未完成父任务、
...
产品 OPEN_FOR_NEXT_BATCH，稳定退出是迁移无丢项、指针指向183、文档结构/链接一致。
...
- `.specify/feature.json` 与本机 AGENTS managed plan pointer 已指183；没有必需的 extension hook。
  可选 agent-context 更新使用已配置的 AGENTS.md 指针手动同步；该本机文件不纳入 Git。
- authority index 重建后 project/active health 均 `ok=true`，active feature 为 `specs/183-native-di-closure`。
