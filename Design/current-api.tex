...
\end{Verbatim}
\textbf{源码：}\path{ndn-service-framework/ServiceUser.hpp}，第 870 行。\par
...
ndn::Name RequestServiceTargeted(const ndn::Name& provider,
                                 ...
                                 ResponseHandler onResponseHandler);
\end{Verbatim}
\textbf{源码：}\path{ndn-service-framework/ServiceUser.hpp}，第 863 行。\par
...
ndn::Name RequestServiceTargeted(const ndn::Name& provider,
                                           ...
                                           int timeoutMs)
\end{Verbatim}
\textbf{源码：}\path{ndn-service-framework/ServiceUser.hpp}，第 1150 行。\par
...
\end{Verbatim}
\textbf{源码：}\path{ndn-service-framework/ServiceProvider.hpp}，第 868 行。\par
...
ServiceRegistration addScopedService(const ndn::Name& serviceName,
                                                 ...
                                                 ServiceMode mode);
\end{Verbatim}
\textbf{源码：}\path{ndn-service-framework/ServiceProvider.hpp}，第 872 行。\par
...
\end{Verbatim}
\textbf{源码：}\path{ndn-service-framework/ServiceProvider.hpp}，第 881 行。\par
...
\end{Verbatim}
\textbf{源码：}\path{ndn-service-framework/ServiceUser.hpp}，第 1003 行。\par
...
\end{Verbatim}
\textbf{源码：}\path{ndn-service-framework/ServiceUser.hpp}，第 1053 行。\par
...
std::shared_ptr<StreamedInvocationHandle<EventT, ResponseT>>
            RequestServiceStreaming(
                ...
                size_t strategy = ndn_service_framework::tlv::FirstResponding)
\end{Verbatim}
\textbf{源码：}\path{ndn-service-framework/ServiceUser.hpp}，第 924 行。\par
...
\end{Verbatim}
\textbf{源码：}\path{ndn-service-framework/InvocationStream.hpp}，第 349 行。\par
...
\end{Verbatim}
\textbf{源码：}\path{ndn-service-framework/InvocationStream.hpp}，第 360 行。\par
...
\end{Verbatim}
\textbf{源码：}\path{ndn-service-framework/ServiceUser.hpp}，第 782 行。\par
...
\end{Verbatim}
\textbf{源码：}\path{ndn-service-framework/ServiceUser.hpp}，第 805 行。\par
...
put(node, objectName, payload, options) 的本地重载接收 RepoNode 和字节，返回 RepoObjectManifest。StoreOptions 提供 objectType、replicationFactor、replicaNodes、policyEpoch。名字识别对象，摘要识别内容，segmentCount 描述分段数量，副本字段只是位置/策略信息，不能用其中一个替代全部验证。protected durable publication 的 opaque identity、protection epoch、key-reference、ciphertext manifest digest 和 serving locator 由 Core 提供并由 Repo manifest 原样保存；Repo 不创建、解包或验证密钥，也不替代当前请求授权。网络 helper 与本地静态调用不是同一传输或超时模型。
...
\end{Verbatim}
\textbf{源码：}\path{NDNSF-DistributedRepo/include/ndnsf-distributed-repo/RepoClient.hpp}，第 76 行。\par
...
\end{Verbatim}
\textbf{源码：}\path{NDNSF-DistributedRepo/include/ndnsf-distributed-repo/RepoCore.hpp}，第 91 行。\par
...
\end{Verbatim}
\textbf{源码：}\path{NDNSF-DistributedRepo/include/ndnsf-distributed-repo/RepoCore.hpp}，第 93 行。\par
...
\end{Verbatim}
\textbf{源码：}\path{NDNSF-DistributedRepo/include/ndnsf-distributed-repo/RepoCore.hpp}，第 95 行。\par
...
\subsection{原生输入与选项}
NativeApplicationInput 区分 Inline 和 RepositoryReference，携带任务及 schema 绑定；内联字节与 Repo 引用不能混填。NativeRequestOptions 的 timeoutMs 默认 30000，ackTimeoutMs 默认 5000，后者是前者之内的报价预算。生产路径由 Runtime::open → User::prepare → PreparedModel 绑定已验证模型包，再由 NativeInferenceClient 创建 handle 并调度准备、ACK、Selection、Provider 与 Response。没有 preparation/request contract 的兼容构造仍明确返回 \code{NATIVE_REQUEST_PIPELINE_NOT_READY}；不能把该兼容分支当作 prepared 路径的结果。
...
\end{Verbatim}
\textbf{源码：}\path{NDNSF-DistributedInference/ndnsf_distributed_inference/app_sdk/client.py}，第 1096 行。\par
...
\end{Verbatim}
\textbf{源码：}\path{NDNSF-DistributedInference/cpp/ndnsf-di/NativeInferenceClient.hpp}，第 219 行。\par
...
\end{Verbatim}
\textbf{源码：}\path{NDNSF-DistributedInference/cpp/ndnsf-di/NativeInferenceClient.hpp}，第 157 行。\par
...
\subsection{当前源码的描述符与候选增补}
当前工作树 NativeModelDescriptor 包含 NativeAdapterDescriptor 和 sourceRevision；adapter 描述符绑定 \code{name/version/stateDigest/abi}、格式/任务/backend/precision 能力及输入/options/result schema 摘要，\code{canonicalJson/modelDigest} 提供规范身份。\code{NativeRequestPreparation} 的 sameModel 按模型 canonicalJson 比较。当前 NativeSplitCandidate 还包含 fragmentsByRole、artifactsByRole、requirementsByRole、crossPartitionTensors、tensorDegreesByRole、\code{rankArtifactDigestsByRole}、输入/结果角色、mergeKind、postprocessingJson、estimatedCosts、可选 hybridPlan 与 candidateDigest；computedDigest 用于完整候选身份，不能只哈希部分角色名。这些是并行实现中的源码事实，完整语义、独立 oracle 与接线验收仍以 Spec182 为准；目标冻结声明不自动复制这些新增字段。
...
\end{Verbatim}
\textbf{源码：}\path{NDNSF-DistributedInference/cpp/ndnsf-di/NativePlanning.hpp}，第 303 行。\par
...
\end{Verbatim}
\textbf{源码：}\path{NDNSF-DistributedInference/cpp/ndnsf-di/NativePlanning.hpp}，第 314 行。\par
...
\end{Verbatim}
\textbf{源码：}\path{NDNSF-DistributedInference/cpp/ndnsf-di/NativeRequestPreparation.hpp}，第 111 行。\par
...
\end{Verbatim}
\textbf{源码：}\path{NDNSF-DistributedInference/cpp/ndnsf-di/NativeRequestPreparation.hpp}，第 119 行。\par
...
\end{Verbatim}
\textbf{源码：}\path{NDNSF-DistributedInference/cpp/ndnsf-di/NativeRequestPreparation.hpp}，第 127 行。\par
...
\end{Verbatim}
\textbf{源码：}\path{NDNSF-DistributedInference/cpp/ndnsf-di/NativeRequestPreparation.hpp}，第 134 行。\par
...
\end{Verbatim}
\textbf{源码：}\path{NDNSF-DistributedInference/cpp/ndnsf-di/NativePlanSealer.hpp}，第 117 行。\par

\Needspace{7\baselineskip}
\textbf{API-f5591fadba7d}\quad public\par
...
\end{Verbatim}
\textbf{源码：}\path{NDNSF-DistributedInference/cpp/ndnsf-di/NativePlanSealer.hpp}，第 128 行。\par
...
\end{Verbatim}
\textbf{源码：}\path{NDNSF-DistributedInference/cpp/ndnsf-di/NativePlanSealer.hpp}，第 143 行。\par
...
\end{Verbatim}
\textbf{源码：}\path{NDNSF-DistributedInference/cpp/ndnsf-di/NativePlanSealer.hpp}，第 147 行。\par
...
\end{Verbatim}
\textbf{源码：}\path{NDNSF-DistributedInference/cpp/ndnsf-di/NativeInferenceProvider.hpp}，第 103 行。\par
...
\end{Verbatim}
\textbf{源码：}\path{NDNSF-DistributedInference/cpp/ndnsf-di/NativeGrantVerifier.hpp}，第 59 行。\par
...
\end{Verbatim}
\textbf{源码：}\path{NDNSF-DistributedInference/cpp/ndnsf-di/NativeEpochCoordinator.hpp}，第 168 行。\par
...
\end{Verbatim}
\textbf{源码：}\path{NDNSF-DistributedInference/cpp/adapters/qwen/QwenGenerationSession.hpp}，第 146 行。\par
...
\end{Verbatim}
\textbf{源码：}\path{NDNSF-DistributedInference/cpp/adapters/qwen/QwenGenerationSession.hpp}，第 147 行。\par
...
void cancel();
\end{Verbatim}
\textbf{源码：}\path{NDNSF-DistributedInference/cpp/adapters/qwen/QwenGenerationSession.hpp}，第 154 行。\par
...
\end{Verbatim}
\textbf{源码：}\path{NDNSF-DistributedInference/cpp/adapters/qwen/QwenGenerationSession.hpp}，第 156 行。\par
...
\end{Verbatim}
\textbf{源码：}\path{NDNSF-DistributedInference/cpp/ndnsf-di/NativeConversationCoordinator.hpp}，第 109 行。\par
...
\end{Verbatim}
\textbf{源码：}\path{NDNSF-DistributedInference/cpp/ndnsf-di/NativeConversationCoordinator.hpp}，第 130 行。\par
...
\end{Verbatim}
\textbf{源码：}\path{NDNSF-DistributedInference/cpp/ndnsf-di/NativeConversationCoordinator.hpp}，第 132 行。\par
...
\end{Verbatim}
\textbf{源码：}\path{NDNSF-DistributedInference/cpp/ndnsf-di/NativeConversationCoordinator.hpp}，第 134 行。\par

\Needspace{7\baselineskip}
\textbf{API-7014d4b50f1d}\quad public\par
\textbf{所属符号：}\code{ndnsf::di::NativeConversationCoordinator::restore}\par
\begin{Verbatim}[fontsize=\footnotesize,breaklines,breakanywhere]
void restore();
\end{Verbatim}
\textbf{源码：}\path{NDNSF-DistributedInference/cpp/ndnsf-di/NativeConversationCoordinator.hpp}，第 135 行。\par
