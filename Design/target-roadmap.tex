...
\subsection{API 与身份}
Runtime::open 接收操作员固定的配置；User::prepare 接收模型、任务、目录 JSON 与一次性 source
定位。PreparedModel 持有已验证目录、splitter 与 source lease；manifest 和 receipt 只读。
request 接收 owning Input 和 RequestOptions；placement 只有 options 中一个覆盖入口。
RequestHandle 提供 wait()、wait(timeout)、cancel 和 observe；局部等待超时不取消请求。
...
\subsection{实施与证据}
按 Runtime、准备缓存、请求、会话、Provider、迁移、最终验收七阶段推进，细分九批、十四任务。
