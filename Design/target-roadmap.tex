...
\lead{PLANNED · Core 与 DI；接口与完整任务见 specs/185-prepared-model-runtime}

\subsection{Core 与应用边界修订}
现有 Core 已提供请求、ACK、Selection、Response、协作提交、流传输及作用域服务注册。
DI 仍持有通用执行器和等待通知状态；仅有现有协议复用还不足以满足运行时复用目标。
Spec185 的 C-09 新增前置 B0C（T017、T018）：在 Core 提供工作票据、调度、关闭与排空、
完成等待、退订和可靠事件读取；DI 的公开句柄委托这些机制。
模型准备、分片与放置评分、推理结果、token/KV、会话日志和终态事务仍由 DI 定义。
传输完成不能提前成为会话持久提交成功。验收同时要求仅链接 Core 的非模型消费者与真实 DI 调用链通过；
Core 不得反向依赖 DI、Python 或 ONNX。当前计划为18任务12批，均未实现；详细签名、字段、迁移及反例见 C-09。
...
\subsection{实施与证据}
共十一批、十六任务。先安装 C++ SDK/ABI 门，再 Runtime、扩展注册、准备、请求、会话、Provider、迁移；
...
\subsection{固定实现边界}
C-08 按类/文件增改、重要字段 owner、关键前后签名、调用/失败流程及独立判据细化实现；
每项任务的 Design binding 指向该契约，不以公开 API 表代替内部代码设计。
合作策略先于模型包构造；Package 保存精确配置与合作 splitter；会话经私有统一入口注入 continuation。
旧 request/planner 保留，新 cooperative 入口采用不同名字并共用同一实现，避免重载二义性。
接口、状态提交点和安全边界先确定；普通局部变量或等价实现保留自由，设计就绪不等于实现完成。
