...
\begin{description}
\item[版本] R2；本主体保留 R1 冻结基线及本轮勘误、行为补充，TG-01 至 TG-05 单独说明已接受的 PLANNED 改进。未被目标章节覆盖的内容沿用冻结行为；计划不代表已实现。
\item[基线] \SnapshotDate，\code{Experimental} 分支。冻结提交为 \texttt{\SourceBaseline}；逐文件摘要与独立补丁见 \code{target-source-baseline.json}。目标 API 查询使用 \code{api/target-inventory.json}，不随当前源码刷新。
...
\callout{关键现状}{已有 C++ Provider 执行组件不代表纯 C++ 请求全链路已经完成；当前原生 DI dispatch 仍返回 \code{NATIVE_REQUEST_PIPELINE_NOT_READY}。权限 grant 的局部取钥优化也不代表所有无关节点的运行状态完全不受影响。}
...
\subsection{目标 R0 的边界}
本版尚未采纳更新范围优化或 SVS 通知。因此目标 R0
保留本页全部现状。后续若修改目标设计，应明确影响判定、变更认证、版本兼容、
离线补齐和在途请求处置，再讨论实现；当前代码与验收门不随文档标题自动改变。
...
数据对象 & 关键身份与含义 \\
\midrule
RepoObjectManifest & objectName、objectType、SHA-256、size、segmentCount、replicationFactor、replicaNodes、policyEpoch 及生命周期 metadata。\\
...
\item[LOOKUP / QUERY] 查找对象位置，按类型、class、发布者、状态、标签和时间等 metadata 过滤；C++ 与 Python 注册能力需分别确认。
\end{description}
...
\lead{Python 编排仍在；原生执行组件与原生请求入口的完成度不同}
\code{ndnsf_distributed_inference.api} 当前为受审查的重新导出入口，实际实现仍在
\code{app_sdk} 等 owner 中。InferenceApplication 管理应用定义/部署；
InferenceClient/APPClient 接受请求并提供结果、取消、观察和恢复接口；
InferenceProvider 对接实际网络 Provider。导入名字短不代表内部已经没有 Python 控制逻辑。[D1]
...
\subsection{C++ 已有的能力}
NativeExecutionPlan、NativeProviderSession/Runtime、ProviderRoleWorker、
模型 runner、依赖传输、生成协调器及 Provider host 等组件已存在。
原生规划、grant、ONNX、tokenizer 和请求生命周期正在 Spec182 中逐步整合。[D3--D6]

\subsection{明确未闭合的路径}
当前 \code{NativeInferenceClient::request()} 会创建有 deadline 的 handle，调度原生操作；
\code{dispatchOperation()} 仍以 \code{NATIVE_REQUEST_PIPELINE_NOT_READY} 结束。
因此“纯 C++ 应用已完成默认端到端推理”不是当前可支持的结论。
真实 catalog/Repo 输入准备、完整报价消费及请求编排接线仍有未完成项。[D7]

\callout{目标 R0 的口径}{Spec182 的目标是同一原生实现配可选 Python 外壳；本文件的目标 R0 仍复制当前代码现状，不把这个尚未完成的目标写成已经实现。}
...
DI & Python 应用编排与 C++ Provider 执行组件并存；原生请求全链路仍未接通，不能声明迁移完成。\\
...
\subsection{活跃实现}
当前 active Spec 为 \code{182-native-di-python-bindings}。
真实来源/Repo 准备、报价消费、完整原生请求、流式接线、生成/会话与 Python caller 迁移
仍由其 tasks/contracts 控制。本文不改写这些任务的验收状态。
...
D7 & DI C++ \path{NativeInferenceClient.cpp}: dispatchOperation；active Spec182 tasks 与 evidence。\\
...
\code{module-inventory.json} 记录本次维护目录的文件清单，用于以后发现遗漏或新增组件。
...
下一步由用户指定目标设计修改，明确新旧差异后再讨论实现与验收。
