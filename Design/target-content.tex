...
\begin{description}
\item[版本] R3；主体保留独立冻结源码基线并补充已核对的行为与例子，TG-01 至 TG-05 说明已接受的 PLANNED 改进及待决接口。未被目标章节覆盖的内容沿用冻结行为，当前新增描述符/候选字段不自动进入目标基线。
...
\callout{目标与基线}{目标 TG-02 接受同一原生实现配可选 Python 外壳的方向；本章解释仍存在的 Python/C++ 分工，目标方向不构成 requester 已接通的证据。}
...
Core 在线权限以 \code{ServiceController::grant/revoke} 和第 9–13 章分支为依据；Repo 精确目录以 RepoCore::catalogLookup 为依据；UAV 最新帧以 \code{BoundedLatestFrameQueue::push/popLatest} 为依据；DI 请求句柄以 NativeInferenceClient 为依据。这些行为核对分别进入 AC-08、AC-13、AC-21、AC-14。生成/KV 组件行为在 AC-18，原生全链路、sampling parity、会话恢复及 Python caller 迁移仍由 \code{specs/182-native-di-python-bindings/tasks.md} 和 evidence 控制。Design/reviews 的修订记录只证明文档工作，不替代这些功能门。
