...
**先读 [highlevel-design.md](highlevel-design.md)**：四模块的用途、用户用例、当前设计与长期原则。所有后续变更首先核对其中的原则，再按 [管理规则](MANAGEMENT.md) 查阅下层契约和验证证据。
...
四章重复补充已合入对应 API；新增图解后当前/目标为 59/64 章。完整函数语义覆盖仍按清单逐项收敛，
...
- [图解源码与阅读入口](diagrams/README.md)：两份 PDF 各 G1--G9；当前第 83--91 页、目标第 88--96 页，包含四模块图、核心类图和关键时序/状态图。
