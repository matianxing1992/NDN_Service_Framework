...
- [目标设计](target-design.pdf)：R1 技术正文与当前设计完全一致，后续按用户指示独立修改。
- [覆盖矩阵](coverage-matrix.md)：章节与模块、核对入口的对应关系。
- [验证记录](validation.md)：构建、版面和正文一致性检查。
- [源码基线](source-baseline.json)：350 个文件的摘要、采样时间、提交与本地源码存档，包含全部 API 清单源文件。
...
运行 `python3 Design/verify-api-reference.py` 检查源码、快照、声明和契约 ID；运行 `python3 Design/verify-source-baseline.py` 检查 Git 可还原的源码身份。
