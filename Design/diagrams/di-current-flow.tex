...
\node[dstate] (prep) at (10,0) {PreparingInput\\调度进入此阶段};
\node[dstate] (fail) at (10,-3) {Failed\\原生请求链未就绪};
...
\draw[down] (prep)--node[dlabel,right]{默认 dispatch}(fail);
...
它们不是同一个枚举。进入 PreparingInput 不表示准备算法已被调用完成：
\code{dispatchOperation} 当前报告 \code{NATIVE_REQUEST_PIPELINE_NOT_READY}，
不能继续画成 ACK、规划、提交和成功链。}
...
\diagramnote{源码：\code{NativeInferenceClient.cpp} 的 request、dispatchOperation、
markTerminal、cancelOperation 和 result。声明中存在 Succeeded 等状态不代表默认路径已可达。
Python 应用入口与已实现的原生局部组件仍需按各自契约理解。}
