...
完整重载、数据字段和原始注释：\path{api/core-reference.md}。
...
完整重载、数据字段和原始注释：\path{api/repo-reference.md}。
...
\subsection{目录数据}
catalogStatus 报告本 Repo 的模式、epoch 和对象数；snapshot 是完整本地目录，delta 按目录 epoch 提供变化，lookup 以名称/类型/类别等字段过滤。目录 epoch 不是 Core ControllerVersion。
...
完整重载、数据字段和原始注释：\path{api/di-reference.md}。
...
void cancel();
\end{Verbatim}
\textbf{源码：}\path{NDNSF-DistributedInference/cpp/ndnsf-di/NativeInferenceClient.hpp}，第 96 行。\par
...
完整重载、数据字段和原始注释：\path{api/uav-reference.md}。
