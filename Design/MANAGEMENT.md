...
- SIF 交付对照 S1–S3：先验证并封存包含外部依赖和 SDK 的 base，在相同 base 容器环境内构建 NDNSF 并输出新的完整 runtime SIF；普通源码更新复用 base，依赖/ABI 变更生成新 base。复用 TigerCluster 维护入口，不将宿主产物混入镜像。
- 编译安装和 MiniNDN 变更同时对照高层设计的 B1–B3、M1–M3：NDNSF 自有原生目标归根 Waf，外部依赖使用各自构建系统；MiniNDN 消费系统已安装的软件，运行目录只承载配置、日志和数据，不引入临时软件闭包。
- [highlevel-design.md](highlevel-design.md) 是四模块变更必须首先对照的高层约束。Spec、代码审查和验收记录适用的 G1–G6 与模块原则、owner 影响、验证依据及未闭合项；现有实现缺口不得反写成原则。改变原则须经用户明确接受并记录迁移影响，不能为局部修复静默放宽。它的当前摘要不是完整实现或验收声明，具体契约和证据仍在下层文档维护。
...
图解配合正文维护模块关系、核心类/对象所有权、关键时序/状态三类视图，入口为
[diagrams/README.md](diagrams/README.md)。类图只列关键 API；持有/借用/调用须标明，
不能将所有类堆入一张图。新增类、所有权、调用或状态变化时同步相关图和来源索引。
当前图对照源码快照，目标新增流程标 PLANNED；共享图须核对两侧均成立。
双 PDF 构建后逐页检查所有新增图，递归 TeX 摘要绑定图源，不用截图替代可编辑源文件。
